...
export const build_report = () => {
  ...
  let md =
    ...
    "- 未转译(unsupported，C++ 语句超出转换能力，诚实失败): " + stats.unsupported + "\n" +
    "- 转译超时: " + stats.timeout + "\n" +
    "- 转译崩溃(信号，cpp2rust 内部缺陷): " + stats.crash + "\n" +
    "- 转译失败: " + stats.error + "\n" +
    ...
    "\n\n## 未转译(unsupported)模块\n\n" +
    modules
      .filter((m) => m.state === "unsupported")
      .map((m) => "- `" + m.file + "`: " + (m.reason || "").split("\n")[0] + " (日志: " + m.log + ")")
      .join("\n") +
    "\n\n## 转译失败/崩溃/超时模块\n\n" +
    modules
      .filter((m) => m.state === "error" || m.state === "timeout" || m.state === "crash")

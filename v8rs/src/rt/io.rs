...
thread_local! {
    static SAFE_STDIN: Value<File> = Rc::new(RefCell::new(File::from(
        io::stdin().as_fd().try_clone_to_owned().unwrap(),
    )));
    static SAFE_STDOUT: Value<File> = Rc::new(RefCell::new(File::from(
        io::stdout().as_fd().try_clone_to_owned().unwrap(),
    )));
    static SAFE_STDERR: Value<File> = Rc::new(RefCell::new(File::from(
        io::stderr().as_fd().try_clone_to_owned().unwrap(),
    )));
    static UNSAFE_STDIN: UnsafeCell<File> = unsafe {
        UnsafeCell::new(
            File::from_raw_fd(
                io::stdin()
                    .as_fd()
    ...
    static UNSAFE_STDOUT: UnsafeCell<File> = unsafe {
        UnsafeCell::new(
            File::from_raw_fd(
                io::stdout()
                    .as_fd()
    ...
    static UNSAFE_STDERR: UnsafeCell<File> = unsafe {
        UnsafeCell::new(
            File::from_raw_fd(
                io::stderr()
                    .as_fd()

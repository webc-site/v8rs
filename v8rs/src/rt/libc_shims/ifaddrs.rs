...
use nix::{ifaddrs::InterfaceAddress, sys::socket::SockaddrStorage as NixSockaddrStorage};
...
impl Ifaddrs {
  pub fn from_interface_address(ifa: &InterfaceAddress) -> Self {
    fn mk_addr(ss: Option<&NixSockaddrStorage>) -> Ptr<Sockaddr> {
      match ss {
        None => Ptr::null(),

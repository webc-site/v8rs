...
#[derive()]
pub struct heap_base_SuspendTagCheckingScope {}
impl heap_base_SuspendTagCheckingScope {
    pub fn heap_base_SuspendTagCheckingScope() -> Self {
        let __this: Value<heap_base_SuspendTagCheckingScope> = Rc::new(RefCell::new(Self {}));
        let this: Ptr<heap_base_SuspendTagCheckingScope> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl Clone for heap_base_SuspendTagCheckingScope {
    fn clone(&self) -> Self {
        let __this: Value<heap_base_SuspendTagCheckingScope> = Rc::new(RefCell::new(Self {}));
        let this: Ptr<heap_base_SuspendTagCheckingScope> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
...
impl Default for heap_base_SuspendTagCheckingScope {
    fn default() -> Self {
        { heap_base_SuspendTagCheckingScope::heap_base_SuspendTagCheckingScope() }
    }
}
impl ByteRepr for heap_base_SuspendTagCheckingScope {
    fn byte_size() -> usize {
        1
    }
    fn to_bytes(&self, buf: &mut [u8]) {}
...
pub trait heap_base_SuspendTagCheckingScopeImpl {
    fn destructor(&self);
}
impl heap_base_SuspendTagCheckingScopeImpl for Ptr<heap_base_SuspendTagCheckingScope> {
    fn destructor(&self) {}
}

...
thread_local!();
#[derive(Default)]
pub struct v8_internal_TracingFlags {}
impl v8_internal_TracingFlags {
    pub fn is_gc_enabled() -> bool {
        return (({ (*gc_21.with(Value::clone).borrow()).load_const(Some(0)) }) != 0_u32);
    }
    pub fn is_gc_stats_enabled() -> bool {
        return (({ (*gc_stats_22.with(Value::clone).borrow()).load_const(Some(0)) }) != 0_u32);
    }
    pub fn is_ic_stats_enabled() -> bool {
        return (({ (*ic_stats_23.with(Value::clone).borrow()).load_const(Some(0)) }) != 0_u32);
    }
    pub fn is_zone_stats_enabled() -> bool {
        return (({ (*zone_stats_24.with(Value::clone).borrow()).load_const(Some(0)) }) != 0_u32);
    }
}
impl Clone for v8_internal_TracingFlags {
    fn clone(&self) -> Self {
        let __this: Value<v8_internal_TracingFlags> = Rc::new(RefCell::new(Self {}));
        let this: Ptr<v8_internal_TracingFlags> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for v8_internal_TracingFlags {
    fn byte_size() -> usize {
        1
    }
    fn to_bytes(&self, buf: &mut [u8]) {}
...
thread_local!(
    pub static runtime_stats_20: Value<std_atomic_unsigned_int_> = Rc::new(RefCell::new(
        std_atomic_unsigned_int_::std_atomic_unsigned_int_1({ 0_u32 }),
    ));
);
thread_local!(
    pub static gc_21: Value<std_atomic_unsigned_int_> = Rc::new(RefCell::new(
        std_atomic_unsigned_int_::std_atomic_unsigned_int_1({ 0_u32 }),
    ));
);
thread_local!(
    pub static gc_stats_22: Value<std_atomic_unsigned_int_> = Rc::new(RefCell::new(
        std_atomic_unsigned_int_::std_atomic_unsigned_int_1({ 0_u32 }),
    ));
);
thread_local!(
    pub static ic_stats_23: Value<std_atomic_unsigned_int_> = Rc::new(RefCell::new(
        std_atomic_unsigned_int_::std_atomic_unsigned_int_1({ 0_u32 }),
    ));
);
thread_local!(
    pub static zone_stats_24: Value<std_atomic_unsigned_int_> = Rc::new(RefCell::new(
        std_atomic_unsigned_int_::std_atomic_unsigned_int_1({ 0_u32 }),
    ));
);

...
pub trait heap_base_StackVisitor {
    fn VisitPointer(&self, address: AnyPtr);
}
#[derive(Default)]
pub struct heap_base_Stack_Segment {
    pub start: Value<AnyPtr>,
    pub top: Value<AnyPtr>,
}
impl heap_base_Stack_Segment {
    pub fn heap_base_Stack_Segment1(stack_start: AnyPtr, stack_top: AnyPtr) -> Self {
        let stack_start: Value<AnyPtr> = Rc::new(RefCell::new(stack_start));
        let stack_top: Value<AnyPtr> = Rc::new(RefCell::new(stack_top));
        let __this: Value<heap_base_Stack_Segment> = Rc::new(RefCell::new(Self {
            start: Rc::new(RefCell::new((*stack_start.borrow()).clone())),
            top: Rc::new(RefCell::new((*stack_top.borrow()).clone())),
        }));
        let this: Ptr<heap_base_Stack_Segment> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl Clone for heap_base_Stack_Segment {
    fn clone(&self) -> Self {
        let __this: Value<heap_base_Stack_Segment> = Rc::new(RefCell::new(Self {
            start: Rc::new(RefCell::new((*self.start.borrow()).clone())),
            top: Rc::new(RefCell::new((*self.top.borrow()).clone())),
        }));
        let this: Ptr<heap_base_Stack_Segment> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for heap_base_Stack_Segment {
    fn byte_size() -> usize {
        16
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.start.borrow()).to_bytes(&mut buf[0..8]);
        (*self.top.borrow()).to_bytes(&mut buf[8..16]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            start: Rc::new(RefCell::new(<AnyPtr>::from_bytes(&buf[0..8]))),
            top: Rc::new(RefCell::new(<AnyPtr>::from_bytes(&buf[8..16]))),
        }
    }
}
#[derive()]
pub struct heap_base_Stack {
    current_segment_: Value<heap_base_Stack_Segment>,
    lock_: Value<v8_base_Mutex>,
    background_stacks_: Value<BTreeMap<i32, Value<heap_base_Stack_Segment>>>,
    scan_simulator_callback_: Value<FnPtr<fn(PtrDyn<dyn heap_base_StackVisitor>)>>,
}
impl Default for heap_base_Stack {
    fn default() -> Self {
        heap_base_Stack {
            current_segment_: <Value<heap_base_Stack_Segment>>::default(),
            lock_: <Value<v8_base_Mutex>>::default(),
            background_stacks_: Rc::new(RefCell::new(BTreeMap::new())),
            scan_simulator_callback_: Rc::new(RefCell::new(FnPtr::<
                fn(PtrDyn<dyn heap_base_StackVisitor>),
            >::null())),
        }
    }
}
impl ByteRepr for heap_base_Stack {
    fn byte_size() -> usize {
        56
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.current_segment_.borrow()).to_bytes(&mut buf[0..16]);
        (*self.lock_.borrow()).to_bytes(&mut buf[16..24]);
        (*self.background_stacks_.borrow()).to_bytes(&mut buf[24..48]);
        (*self.scan_simulator_callback_.borrow()).to_bytes(&mut buf[48..56]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            current_segment_: Rc::new(RefCell::new(<heap_base_Stack_Segment>::from_bytes(
                &buf[0..16],
            ))),
            lock_: Rc::new(RefCell::new(<v8_base_Mutex>::from_bytes(&buf[16..24]))),
            background_stacks_: Rc::new(RefCell::new(<BTreeMap<
                i32,
                Value<heap_base_Stack_Segment>,
            >>::from_bytes(&buf[24..48]))),
            scan_simulator_callback_: Rc::new(RefCell::new(<FnPtr<
                fn(PtrDyn<dyn heap_base_StackVisitor>),
            >>::from_bytes(
                &buf[48..56]
...
pub fn PushAllRegistersAndIterateStack_242(
    sp: Ptr<heap_base_Stack>,
    sv: PtrDyn<dyn heap_base_StackVisitor>,
    callback: FnPtr<fn(Ptr<heap_base_Stack>, PtrDyn<dyn heap_base_StackVisitor>, Ptr<i64>)>,
) {
    let sp: Value<Ptr<heap_base_Stack>> = Rc::new(RefCell::new(sp));
    let sv: Value<PtrDyn<dyn heap_base_StackVisitor>> = Rc::new(RefCell::new(sv));
    let callback: Value<
        FnPtr<fn(Ptr<heap_base_Stack>, PtrDyn<dyn heap_base_StackVisitor>, Ptr<i64>)>,
...
pub trait heap_base_StackImpl {
    fn SetStackStart(&self, stack_start: AnyPtr);
    fn stack_start(&self) -> AnyPtr;
    fn IsMarkerSet(&self) -> bool;
    fn IsMarkerSetForBackgroundThread(&self, thread: i32) -> bool;
    fn HasBackgroundStacks(&self) -> bool;
    fn SetScanSimulatorCallback(&self, callback: FnPtr<fn(PtrDyn<dyn heap_base_StackVisitor>)>);
}
impl heap_base_StackImpl for Ptr<heap_base_Stack> {
    fn SetStackStart(&self, stack_start: AnyPtr) {
        let stack_start: Value<AnyPtr> = Rc::new(RefCell::new(stack_start));
        (*(*(*(*self).upgrade().deref()).current_segment_.borrow())
            .start
            .borrow_mut()) = (*stack_start.borrow()).clone();
    }
    fn stack_start(&self) -> AnyPtr {
        return (*(*(*(*self).upgrade().deref()).current_segment_.borrow())
            .start
            .borrow())
        .clone();
    }
    ...
    fn IsMarkerSetForBackgroundThread(&self, thread: i32) -> bool {
        let thread: Value<i32> = Rc::new(RefCell::new(thread));
        ...
        let it: Value<RefcountMapIter<i32, heap_base_Stack_Segment>> =
            Rc::new(RefCell::new(RefcountMapIter::find_key(
                ((*(*self).upgrade().deref()).background_stacks_.as_pointer()
                    as Ptr<BTreeMap<i32, Value<heap_base_Stack_Segment>>>),
                &(*thread.borrow()),
            )));
        if (*it.borrow())
            == RefcountMapIter::end(
    ...
    fn HasBackgroundStacks(&self) -> bool {
        return !({ (*(*(*self).upgrade().deref()).background_stacks_.borrow()).empty() });
    }
    fn SetScanSimulatorCallback(&self, callback: FnPtr<fn(PtrDyn<dyn heap_base_StackVisitor>)>) {
        let callback: Value<FnPtr<fn(PtrDyn<dyn heap_base_StackVisitor>)>> =
            Rc::new(RefCell::new(callback));
        (*(*(*self).upgrade().deref())
            .scan_simulator_callback_
            .borrow_mut()) = (*callback.borrow()).clone();
    }
}

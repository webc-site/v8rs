...
use std::os::fd::AsFd;
use std::rc::{Rc, Weak};
pub type v8_internal_ProfilerStats_Reason = u32;
pub const v8_internal_ProfilerStats_Reason_kTickBufferFull: v8_internal_ProfilerStats_Reason = 0;
pub const v8_internal_ProfilerStats_Reason_kIsolateNotLocked: v8_internal_ProfilerStats_Reason = 1;
pub const v8_internal_ProfilerStats_Reason_kSimulatorFillRegistersFailed:
    v8_internal_ProfilerStats_Reason = 2;
pub const v8_internal_ProfilerStats_Reason_kNoFrameRegion: v8_internal_ProfilerStats_Reason = 3;
pub const v8_internal_ProfilerStats_Reason_kInCallOrApply: v8_internal_ProfilerStats_Reason = 4;
pub const v8_internal_ProfilerStats_Reason_kNoSymbolizedFrames: v8_internal_ProfilerStats_Reason =
    5;
pub const v8_internal_ProfilerStats_Reason_kNullPC: v8_internal_ProfilerStats_Reason = 6;
pub const v8_internal_ProfilerStats_Reason_kNumberOfReasons: v8_internal_ProfilerStats_Reason = 7;
#[derive()]
pub struct v8_internal_ProfilerStats {
    counts_: Value<Box<[std_atomic_int_]>>,
}
impl v8_internal_ProfilerStats {
    pub fn Instance() -> Ptr<v8_internal_ProfilerStats> {
        thread_local!(
            static stats_0: Value<v8_internal_ProfilerStats> =
                Rc::new(RefCell::new(<v8_internal_ProfilerStats>::default()));
        );
        return (stats_0.with(Value::clone).as_pointer());
    }
}
impl Default for v8_internal_ProfilerStats {
    fn default() -> Self {
        v8_internal_ProfilerStats {
            counts_: Rc::new(RefCell::new(
                (0..7)
                    .map(|_| <std_atomic_int_>::default())
                    .collect::<Box<[std_atomic_int_]>>(),
            )),
        }
    }
}
impl ByteRepr for v8_internal_ProfilerStats {
    fn byte_size() -> usize {
        28
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.counts_.borrow()).to_bytes(&mut buf[0..28]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            counts_: Rc::new(RefCell::new(<Box<[std_atomic_int_]>>::from_bytes(
                &buf[0..28],
            ))),
        }
    }
...
pub fn make_uint64_27(high: u32, low: u32) -> u64 {
...
pub fn IsAligned_30(value: u64, alignment: u64) -> bool {
...
pub type anon_31 = u8;
pub const anon_31_ONCE_STATE_UNINITIALIZED: anon_31 = 0;
pub const anon_31_ONCE_STATE_EXECUTING_FUNCTION: anon_31 = 1;
pub const anon_31_ONCE_STATE_DONE: anon_31 = 2;
pub fn CallOnce_32(once: Ptr<std_atomic_unsigned_char_>, init_func: std_function_void____) {
    ...
    if ((({ (*(*once.borrow()).upgrade().deref()).load_const(Some(2)) }) as i32)
        != (anon_31_ONCE_STATE_DONE as i32))
    {
        ({ CallOnceImpl_33((*once.borrow()).clone(), (*init_func.borrow()).clone()) });
    }
}
#[derive(Default)]
...
thread_local!(
    pub static kStackWalkError_35: Value<i32> = Rc::new(RefCell::new(-1_i32));
);
thread_local!(
    pub static kStackWalkMaxNameLen_36: Value<i32> = Rc::new(RefCell::new(256));
);
thread_local!(
    pub static kStackWalkMaxTextLen_37: Value<i32> = Rc::new(RefCell::new(256));
);
thread_local!(
    static msPerSecond_38: Value<i32> = Rc::new(RefCell::new(1000));
);
#[derive()]
...
impl v8_base_Stack {
    pub fn GetCurrentFrameAddress(
        ...
        let frame_address: Value<AnyPtr> = Rc::new(RefCell::new(
            frame_address.unwrap_or(({ __builtin_frame_address_40(0_u32) })),
        ));
...
impl v8_internal_ProfilerStats {
    fn ReasonToString(reason: v8_internal_ProfilerStats_Reason) -> Ptr<u8> {
        let reason: Value<v8_internal_ProfilerStats_Reason> = Rc::new(RefCell::new(reason));
        'switch: {
            let __match_cond = ((*reason.borrow()) as i32);
            match __match_cond {
                __v if __v == 0 => {
                    return Ptr::from_string_literal(b"kTickBufferFull");
                }
                __v if __v == 1 => {
                    return Ptr::from_string_literal(b"kIsolateNotLocked");
                }
                __v if __v == 2 => {
                    return Ptr::from_string_literal(b"kSimulatorFillRegistersFailed");
                }
                __v if __v == 3 => {
                    return Ptr::from_string_literal(b"kNoFrameRegion");
                }
                __v if __v == 4 => {
                    return Ptr::from_string_literal(b"kInCallOrApply");
                }
                __v if __v == 5 => {
                    return Ptr::from_string_literal(b"kNoSymbolizedFrames");
                }
                __v if __v == 6 => {
                    return Ptr::from_string_literal(b"kNullPC");
                }
                __v if __v == 7 => {
                    return Ptr::from_string_literal(b"kNumberOfReasons");
                }
                _ => {}
            }
        };
...
pub trait v8_internal_ProfilerStatsImpl {
    fn AddReason(&self, reason: v8_internal_ProfilerStats_Reason);
    fn Clear(&self);
    fn Print(&self);
}
impl v8_internal_ProfilerStatsImpl for Ptr<v8_internal_ProfilerStats> {
    fn AddReason(&self, reason: v8_internal_ProfilerStats_Reason) {
        let reason: Value<v8_internal_ProfilerStats_Reason> = Rc::new(RefCell::new(reason));
        ({
            (*(*(*self).upgrade().deref()).counts_.borrow())[(*reason.borrow()) as usize]
                .fetch_add_i32(1, Some(0))
        });
    }
    fn Clear(&self) {
        let i: Value<i32> = Rc::new(RefCell::new(0));
        'loop_: while ((*i.borrow()) < (v8_internal_ProfilerStats_Reason_kNumberOfReasons as i32)) {
            ({
                (*(*(*self).upgrade().deref()).counts_.borrow())[(*i.borrow()) as usize]
                    .store_i32(0, Some(0))
            });
    ...
    fn Print(&self) {
        ({ v8_base_OS::Print(Ptr::from_string_literal(b"ProfilerStats:\n"), &[]) });
        let i: Value<i32> = Rc::new(RefCell::new(0));
        'loop_: while ((*i.borrow()) < (v8_internal_ProfilerStats_Reason_kNumberOfReasons as i32)) {
            ({
                v8_base_OS::Print(
                    Ptr::from_string_literal(b"  %-30s\t\t %d\n"),
                    &[
                        ({
                            v8_internal_ProfilerStats::ReasonToString(
                                ((*i.borrow()) as v8_internal_ProfilerStats_Reason),
                            )
                        })
                        .into(),
                        ({

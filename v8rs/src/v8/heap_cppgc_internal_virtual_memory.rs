...
impl cppgc_internal_VirtualMemory {
    pub fn cppgc_internal_VirtualMemory1(
        ...
        let size: Value<usize> = Rc::new(RefCell::new(size));
        let alignment: Value<usize> = Rc::new(RefCell::new(alignment));
        let hint: Value<AnyPtr> = Rc::new(RefCell::new(hint.unwrap_or(AnyPtr::default())));
        let __this: Value<cppgc_internal_VirtualMemory> = Rc::new(RefCell::new(Self {
            page_allocator_: Rc::new(RefCell::new((*page_allocator.borrow()).clone())),
            start_: Rc::new(RefCell::new(AnyPtr::default())),
            size_: Rc::new(RefCell::new(0_usize)),
        }));
        let this: Ptr<cppgc_internal_VirtualMemory> = __this.as_pointer();
        (&(0));
        (&(0));
        let page_size: Value<usize> = Rc::new(RefCell::new(
            ({
        ...
        (*(*this.upgrade().deref()).start_.borrow_mut()) = ({
            let _length: usize =
                (({ RoundUp_28(((*size.borrow()) as u64), ((*page_size.borrow()) as i64)) })
                    as usize);
            let _alignment: usize = (({
                RoundUp_28(
                    ((*alignment.borrow()) as u64),
                    ((*page_size.borrow()) as i64),
                )
            }) as usize);
            (*(*page_allocator.borrow()).upgrade().deref())
                .AllocatePages_AnyPtr_usize_usize_v8_PageAllocator_Permission(
                    (*hint.borrow()).clone(),
                    _length,
                    _alignment,
                    v8_PageAllocator_Permission_kNoAccess,
                )
        });
        if !(*(*this.upgrade().deref()).start_.borrow()).is_null() {
            (*(*this.upgrade().deref()).size_.borrow_mut()) =
                (({ RoundUp_28(((*size.borrow()) as u64), ((*page_size.borrow()) as i64)) })
                    as usize);
        }
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
    pub fn cppgc_internal_VirtualMemory2(
        page_allocator: PtrDyn<dyn v8_PageAllocator>,
        reserved: AnyPtr,
        size: usize,
    ) -> Self {
        let page_allocator: Value<PtrDyn<dyn v8_PageAllocator>> =
            Rc::new(RefCell::new(page_allocator));
        let reserved: Value<AnyPtr> = Rc::new(RefCell::new(reserved));
        let size: Value<usize> = Rc::new(RefCell::new(size));
        let __this: Value<cppgc_internal_VirtualMemory> = Rc::new(RefCell::new(Self {
            page_allocator_: Rc::new(RefCell::new((*page_allocator.borrow()).clone())),
            start_: Rc::new(RefCell::new((*reserved.borrow()).clone())),
...
pub trait cppgc_internal_VirtualMemoryImpl {
    fn destructor(&self);
    fn operator_assign_pmutcppgc_internal_VirtualMemory(
        &self,
        other: Ptr<cppgc_internal_VirtualMemory>,
    ) -> Ptr<cppgc_internal_VirtualMemory>;
...
impl cppgc_internal_VirtualMemoryImpl for Ptr<cppgc_internal_VirtualMemory> {
    ...
    fn destructor(&self) {
        if ({ cppgc_internal_VirtualMemoryImpl::IsReserved(self) }) {
            ({
                let _address: AnyPtr = (*(*(*self).upgrade().deref()).start_.borrow()).clone();
                let _length: usize = (*(*(*self).upgrade().deref()).size_.borrow());
                (*(*(*(*self).upgrade().deref()).page_allocator_.borrow())
                    .upgrade()
                    .deref())
                .FreePages(_address, _length)
            });
        }
    }
    fn operator_assign_pmutcppgc_internal_VirtualMemory(
        ...
        (&(0));
        let __rhs = (*(*other.upgrade().deref()).page_allocator_.borrow()).clone();
        (*(*(*self).upgrade().deref()).page_allocator_.borrow_mut()) = __rhs;
        let __rhs = (*(*other.upgrade().deref()).start_.borrow()).clone();
        (*(*(*self).upgrade().deref()).start_.borrow_mut()) = __rhs;
        let __rhs = ((*(*other.upgrade().deref()).size_.borrow()) as usize);
        (*(*(*self).upgrade().deref()).size_.borrow_mut()) = __rhs;
        ({ cppgc_internal_VirtualMemoryImpl::Reset(&other) });
        return (*self).clone();
    }
    fn Reset(&self) {
        (*(*(*self).upgrade().deref()).start_.borrow_mut()) = AnyPtr::default();

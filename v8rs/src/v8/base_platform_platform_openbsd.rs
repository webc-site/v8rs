...
pub fn SharedMemoryHandleFromMachMemoryEntry_20(handle: u32) -> v8_SharedMemoryHandle {
    let handle: Value<u32> = Rc::new(RefCell::new(handle));
    return ({ v8_SharedMemoryHandle::FromPlatformHandle((*handle.borrow())) });
}
pub fn MachMemoryEntryFromSharedMemoryHandle_21(handle: v8_SharedMemoryHandle) -> u32 {
...
thread_local!(
    pub static kInvalidSharedMemoryHandle_22: Value<std_optional_v8_SharedMemoryHandle_> =
        Rc::new(RefCell::new(
            std_optional_v8_SharedMemoryHandle_::std_optional_v8_SharedMemoryHandle_1({
                (*nullopt_23.with(Value::clone).borrow()).clone()
            }),
        ));
);
...
pub fn operator_bitor_24(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
...
pub fn operator_bitand_25(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return ((((*lhs.borrow()) as i32) & ((*rhs.borrow()) as i32)) as v8_PagePermissions);
}
pub fn operator_bitor_assign_26(
    ...
    let __rhs = ({
        ...
        operator_bitor_24(_lhs, _rhs)
    });
    lhs.write(__rhs);
    return (lhs).clone();
}
pub fn IsSubset_27(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> bool {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return (({
        let _lhs: v8_PagePermissions = (*lhs.borrow());
        operator_bitand_25(_lhs, (*rhs.borrow()))
    }) == (*lhs.borrow()));
}
...
pub trait v8_Platform {
    ...
    fn CurrentClockTimeMilliseconds(&self) -> i64 {
        return (({ floor_28(({ self.CurrentClockTimeMillis() })) }) as i64);
    }
    ...
    fn GetHighAllocationThroughputObserver(&self) -> Ptr<v8_HighAllocationThroughputObserver> {
        thread_local!(
            static default_observer_29: Value<v8_HighAllocationThroughputObserver> = Rc::new(
                RefCell::new(<v8_HighAllocationThroughputObserver>::default()),
            );
        );
        return (default_observer_29.with(Value::clone).as_pointer());
    }
    fn CreateJobImpl(
        &self,
        priority: v8_TaskPriority,
...
impl v8_base_OS {
    pub fn GetSharedLibraryAddresses() -> Vec<v8_base_OS_SharedLibraryAddress> {
        ...
        let fp: Value<Ptr<CFile>> = Rc::new(RefCell::new(
            match CFile::open(
                &Ptr::from_string_literal(b"/proc/self/maps").to_rust_string(),
        ...
        if (*fp.borrow()).is_null() {
            return std::mem::take(&mut (*result.borrow_mut()));
        }
        let kLibNameLen: Value<i32> = Rc::new(RefCell::new((1024 + 1)));
        let lib_name: Value<Ptr<u8>> = Rc::new(RefCell::new(
            malloc_refcount(((*kLibNameLen.borrow()) as usize)).reinterpret_cast::<u8>(),
        ));
        'loop_: while true {
            let start: Value<u64> = <Value<u64>>::default();
            let end: Value<u64> = <Value<u64>>::default();
            let attr_r: Value<u8> = <Value<u8>>::default();
            let attr_w: Value<u8> = <Value<u8>>::default();
            let attr_x: Value<u8> = <Value<u8>>::default();
            let attr_p: Value<u8> = <Value<u8>>::default();
            if (({
                fscanf_40(
                    (*fp.borrow()).clone(),
                    Ptr::from_string_literal(b"%lx-%lx"),
                    &[(start.as_pointer()).into(), (end.as_pointer()).into()],
                )
            }) != 2)
            {
                break;
            ...
            if (({
                fscanf_40(
                    (*fp.borrow()).clone(),
                    Ptr::from_string_literal(b" %c%c%c%c"),
                    &[
                        (attr_r.as_pointer()).into(),
                        (attr_w.as_pointer()).into(),
                        (attr_x.as_pointer()).into(),
                        (attr_p.as_pointer()).into(),
                    ],
                )
            }) != 4)
            {
            ...
            if ((((*attr_r.borrow()) as i32) == (('r' as u8) as i32))
                && (((*attr_w.borrow()) as i32) != (('w' as u8) as i32)))
                && (((*attr_x.borrow()) as i32) == (('x' as u8) as i32))
            {
                let mut __do_while = true;
                'loop_: while __do_while
                    || ((((*c.borrow()) != (-1_i32)) && ((*c.borrow()) != (('\n' as u8) as i32)))
                        && ((*c.borrow()) != (('/' as u8) as i32)))
                {
                    __do_while = false;
                    (*c.borrow_mut()) = (*fp.borrow()).with_mut(|__f| __f.getc());
                }
                if ((*c.borrow()) == (-1_i32)) {
                    break;
                }
                if ((*c.borrow()) == (('/' as u8) as i32)) {
                    ({ ungetc_41((*c.borrow()), (*fp.borrow()).clone()) });
                    if ({
                        let __buf = (*lib_name.borrow()).clone();
                        let __n = (*kLibNameLen.borrow());
                        if __n <= 0 {
                            Ptr::null()
                        } else {
                    ...
                    (*lib_name.borrow())
                        .offset(
                            (((*lib_name.borrow()).to_c_string_iterator().count())
                    ...
                    {
                        let __s = format_c(
                            &Ptr::from_string_literal(b"%08lx-%08lx").to_rust_string(),
                            &[(*start.borrow()).into(), (*end.borrow()).into()],
                        );
                        let __b = __s.as_bytes();
                        if ((*kLibNameLen.borrow()) as usize) > 0 {
                            let __n =
                                ::std::cmp::min(__b.len(), ((*kLibNameLen.borrow()) as usize) - 1);
                            (*lib_name.borrow()).with_slice_mut(__n + 1, |__dst| {
                ...
                (*result.borrow_mut()).push(
                    v8_base_OS_SharedLibraryAddress::v8_base_OS_SharedLibraryAddress1(
                        {
        ...
        free_refcount(((*lib_name.borrow()).clone() as Ptr<u8>).to_any());
        {
...
impl v8_base_OS {
    pub fn SignalCodeMovingGC() {
        let size: Value<i64> = Rc::new(RefCell::new(({ sysconf_42(29) })));
        let f: Value<Ptr<CFile>> = Rc::new(RefCell::new(
            match CFile::open(
                &({ v8_base_OS::GetGCFakeMMapFile() }).to_rust_string(),
        ...
        if (*f.borrow()).is_null() {
            ({
                V8_Fatal_43(
                    Ptr::from_string_literal(b"Failed to open %s: %s"),
                    &[
                        ({ v8_base_OS::GetGCFakeMMapFile() }).into(),
                        ({ strerror_44((cpp2rust_errno().read())) }).into(),
                    ],
                )
            });
        }
        let addr: Value<AnyPtr> = Rc::new(RefCell::new(
            ({
                mmap_45(
                    AnyPtr::default(),
                    ((*size.borrow()) as usize),
                    (1 | 4),
                    2,
                    (*f.borrow()).with(|__f| __f.fd),
                    0_i64,
                )
            }),
        ));
        (&(0));
        ({ v8_base_OS::Free((*addr.borrow()).clone(), ((*size.borrow()) as usize)) });
        {
            let __r = (*f.borrow()).with(|__f| __f.close());
            (*f.borrow()).delete();
            __r
        };
    }

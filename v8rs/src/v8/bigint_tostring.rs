...
impl v8_bigint_ShiftedDigits {
    pub fn v8_bigint_ShiftedDigits1(
        ...
        let leading_zeros: Value<i32> = Rc::new(RefCell::new(
            ({ CountLeadingZeros_24(({ v8_bigint_DigitsImpl::msd(&original) })) }),
        ));
        if ((*shift.borrow()) < 0) {
...
pub fn RoundUp_94(x: i32, y: i32) -> i32 {
    let x: Value<i32> = Rc::new(RefCell::new(x));
    let y: Value<i32> = Rc::new(RefCell::new(y));
    return ((((*x.borrow()) + (*y.borrow())) - 1) & -(*y.borrow()));
}
pub fn CountTrailingZeros_95(value: u32) -> i32 {
    let value: Value<u32> = Rc::new(RefCell::new(value));
    return if ((*value.borrow()) == 0_u32) {
        32
    } else {
...
pub fn IsPowerOfTwo_96(value: i32) -> bool {
    let value: Value<i32> = Rc::new(RefCell::new(value));
    return ((*value.borrow()) > 0) && (((*value.borrow()) & ((*value.borrow()) - 1)) == 0);
}
pub fn AddAndReturnOverflow_97(Z: v8_bigint_RWDigits, X: v8_bigint_Digits) -> u64 {
    let Z: Value<v8_bigint_RWDigits> = Rc::new(RefCell::new(Z));
    let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
    ({ v8_bigint_DigitsImpl::Normalize(&X.as_pointer()) });
    if (({ v8_bigint_DigitsImpl::len(&X.as_pointer()) }) == 0_u32) {
...
pub fn SubAndReturnBorrow_98(Z: v8_bigint_RWDigits, X: v8_bigint_Digits) -> u64 {
    let Z: Value<v8_bigint_RWDigits> = Rc::new(RefCell::new(Z));
    let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
    ({ v8_bigint_DigitsImpl::Normalize(&X.as_pointer()) });
    if (({ v8_bigint_DigitsImpl::len(&X.as_pointer()) }) == 0_u32) {
...
pub fn IsBitNormalized_99(X: v8_bigint_Digits) -> bool {
    let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
    return ((({ v8_bigint_DigitsImpl::msd(&X.as_pointer()) }) >> (64 - 1)) == 1_u64);
}
pub fn BitLength_100(X: v8_bigint_Digits) -> u32 {
    let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
...
thread_local!(
    pub static kMaxBitsPerChar_101: Value<Box<[u8]>> = Rc::new(RefCell::new(Box::new([
        0_u8, 0_u8, 32_u8, 51_u8, 64_u8, 75_u8, 83_u8, 90_u8, 96_u8, 102_u8, 107_u8, 111_u8,
        115_u8, 119_u8, 122_u8, 126_u8, 128_u8, 131_u8, 134_u8, 136_u8, 139_u8, 141_u8, 143_u8,
        145_u8, 147_u8, 149_u8, 151_u8, 153_u8, 154_u8, 156_u8, 158_u8, 159_u8, 160_u8, 162_u8,
        163_u8, 165_u8, 166_u8,
    ])));
);
thread_local!(
    pub static kBitsPerCharTableShift_102: Value<i32> = Rc::new(RefCell::new(5));
);
thread_local!(
    pub static kBitsPerCharTableMultiplier_103: Value<usize> = Rc::new(RefCell::new(
        ((1_u32 << (*kBitsPerCharTableShift_102.with(Value::clone).borrow())) as usize),
    ));
);
thread_local!(
    pub static kConversionChars_104: Value<Box<[u8]>> = Rc::new(RefCell::new(Box::from(
        *b"0123456789abcdefghijklmnopqrstuvwxyz\0",
    )));
);
pub fn digit_pow_105(base: u64, exponent: u64) -> u64 {
    let base: Value<u64> = Rc::new(RefCell::new(base));
    let exponent: Value<u64> = Rc::new(RefCell::new(exponent));
    let result: Value<u64> = Rc::new(RefCell::new(1_u64));
    'loop_: while ((*exponent.borrow()) > 0_u64) {
        if (((*exponent.borrow()) & 1_u64) != 0) {
            {
                let rhs_0 = (*result.borrow()).wrapping_mul((*base.borrow()));
                (*result.borrow_mut()) = rhs_0
            };
        }
        (*exponent.borrow_mut()) >>= 1;
        {
            let rhs_0 = (*base.borrow()).wrapping_mul((*base.borrow()));
            (*base.borrow_mut()) = rhs_0
        };
    }
    return (*result.borrow());
}
pub fn digit_pow_rec_106(base: u64, exponent: u64) -> u64 {
    let base: Value<u64> = Rc::new(RefCell::new(base));
    let exponent: Value<u64> = Rc::new(RefCell::new(exponent));
    return if ((*exponent.borrow()) == 1_u64) {
        (*base.borrow())
    } else {
        (*base.borrow()).wrapping_mul(
            ({ digit_pow_rec_106((*base.borrow()), (*exponent.borrow()).wrapping_sub(1_u64)) }),
        )
    };
}
pub fn BasecaseFixedLast_107(chunk: u64, out: Ptr<u8>) -> Ptr<u8> {
    let chunk: Value<u64> = Rc::new(RefCell::new(chunk));
    let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
    'loop_: while ((*chunk.borrow()) != 0_u64) {
        (&(0));
        if (10 <= 10) {
            let __rhs = (((('0' as u8) as u64)
                .wrapping_add(((*chunk.borrow()).wrapping_rem((10 as u64)))))
                as u8);
            ((*out.borrow_mut()).prefix_dec()).write(__rhs);
        } else {
            let __rhs = (*kConversionChars_104.with(Value::clone).borrow())
                [((*chunk.borrow()).wrapping_rem((10 as u64))) as usize];
            ((*out.borrow_mut()).prefix_dec()).write(__rhs);
        }
        {
            let rhs_0 = (*chunk.borrow()).wrapping_div((10 as u64));
            (*chunk.borrow_mut()) = rhs_0
        };
    }
    return (*out.borrow()).clone();
}
pub fn DivideByMagic_108(
    rest: v8_bigint_RWDigits,
    input: v8_bigint_Digits,
    output: Ptr<u8>,
) -> Ptr<u8> {
    let rest: Value<v8_bigint_RWDigits> = Rc::new(RefCell::new(rest));
    let input: Value<v8_bigint_Digits> = Rc::new(RefCell::new(input));
    let output: Value<Ptr<u8>> = Rc::new(RefCell::new(output));
    let max_bits_per_char: Value<u8> = Rc::new(RefCell::new(107));
    let chunk_chars: Value<i32> = Rc::new(RefCell::new(9));
    let chunk_divisor: Value<u64> = Rc::new(RefCell::new(1000000000));
    let remainder: Value<u64> = Rc::new(RefCell::new(0_u64));
    let i: Value<u32> = Rc::new(RefCell::new(
        ({ v8_bigint_DigitsImpl::len(&input.as_pointer()) }),
    ));
    'loop_: while ((*i.borrow_mut()).postfix_dec() > 0_u32) {
        let d: Value<u64> = Rc::new(RefCell::new(
            ({ v8_bigint_DigitsImpl::operator_index(&input.as_pointer(), (*i.borrow())) }),
        ));
        let upper: Value<u64> = Rc::new(RefCell::new(
            (((*remainder.borrow()) << 32) | ((*d.borrow()) >> 32)),
        ));
        let u_result: Value<u64> =
            Rc::new(RefCell::new((*upper.borrow()).wrapping_div(1000000000)));
        (*remainder.borrow_mut()) = (*upper.borrow()).wrapping_rem(1000000000);
        let lower: Value<u64> = Rc::new(RefCell::new(
            (((*remainder.borrow()) << 32) | ((*d.borrow()) & 4294967295)),
        ));
        let l_result: Value<u64> =
            Rc::new(RefCell::new((*lower.borrow()).wrapping_div(1000000000)));
        (*remainder.borrow_mut()) = (*lower.borrow()).wrapping_rem(1000000000);
        ({
            v8_bigint_RWDigits_WritableDigitReferenceImpl::operator_assign_u64(
                &Rc::new(RefCell::new(
                    ({ v8_bigint_RWDigitsImpl::operator_index(&rest.as_pointer(), (*i.borrow())) }),
                ))
                .as_pointer(),
                (((*u_result.borrow()) << 32) | (*l_result.borrow())),
            )
        });
    }
    let i: Value<i32> = Rc::new(RefCell::new(0));
    'loop_: while ((*i.borrow()) < 9) {
        (&(0));
        if (10_u64 <= 10_u64) {
            let __rhs = (((('0' as u8) as u64)
                .wrapping_add(((*remainder.borrow()).wrapping_rem((10_u64 as u64)))))
                as u8);
            ((*output.borrow_mut()).prefix_dec()).write(__rhs);
        } else {
            let __rhs = (*kConversionChars_104.with(Value::clone).borrow())
                [((*remainder.borrow()).wrapping_rem((10_u64 as u64))) as usize];
            ((*output.borrow_mut()).prefix_dec()).write(__rhs);
        }
        {
            let rhs_0 = (*remainder.borrow()).wrapping_div((10_u64 as u64));
            (*remainder.borrow_mut()) = rhs_0
        };
        (*i.borrow_mut()).postfix_inc();
    }
    (&(0));
    return (*output.borrow()).clone();
}
#[derive(Default)]
pub struct v8_bigint_ToStringFormatter {
    digits_: Value<v8_bigint_Digits>,
    radix_: Value<i32>,
    chunk_chars_: Value<u32>,
    sign_: Value<bool>,
    out_start_: Value<Ptr<u8>>,
    out_end_: Value<Ptr<u8>>,
    out_: Value<Ptr<u8>>,
    chunk_divisor_: Value<u64>,
    processor_: Value<Ptr<v8_bigint_ProcessorImpl>>,
    platform_: Value<PtrDyn<dyn v8_bigint_Platform>>,
}
impl v8_bigint_ToStringFormatter {
    pub fn v8_bigint_ToStringFormatter(
        X: v8_bigint_Digits,
        radix: i32,
        sign: bool,
        out: Ptr<u8>,
        ...
        let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
        let radix: Value<i32> = Rc::new(RefCell::new(radix));
        let sign: Value<bool> = Rc::new(RefCell::new(sign));
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        let chars_available: Value<u32> = Rc::new(RefCell::new(chars_available));
        let processor: Value<Ptr<v8_bigint_ProcessorImpl>> = Rc::new(RefCell::new(processor));
        let __this: Value<v8_bigint_ToStringFormatter> = Rc::new(RefCell::new(Self {
            digits_: Rc::new(RefCell::new((*X.borrow()).clone())),
            radix_: Rc::new(RefCell::new((*radix.borrow()))),
            chunk_chars_: Rc::new(RefCell::new(0_u32)),
            sign_: Rc::new(RefCell::new((*sign.borrow()))),
            out_start_: Rc::new(RefCell::new((*out.borrow()).clone())),
            out_end_: Rc::new(RefCell::new(
                (*out.borrow()).offset((*chars_available.borrow()) as isize),
            )),
            out_: Rc::new(RefCell::new(
                (*(*this.upgrade().deref()).out_end_.borrow()).clone(),
            )),
            chunk_divisor_: Rc::new(RefCell::new(0_u64)),
            processor_: Rc::new(RefCell::new((*processor.borrow()).clone())),
            platform_: Rc::new(RefCell::new(
                ({ v8_bigint_ProcessorImpl::platform(&(*processor.borrow())) }),
            )),
        }));
        let this: Ptr<v8_bigint_ToStringFormatter> = __this.as_pointer();
        ({ v8_bigint_DigitsImpl::Normalize(&(*this.upgrade().deref()).digits_.as_pointer()) });
        (&(0));
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl Clone for v8_bigint_ToStringFormatter {
    fn clone(&self) -> Self {
        let __this: Value<v8_bigint_ToStringFormatter> = Rc::new(RefCell::new(Self {
            digits_: Rc::new(RefCell::new((*self.digits_.borrow()).clone())),
            radix_: Rc::new(RefCell::new((*self.radix_.borrow()))),
            chunk_chars_: Rc::new(RefCell::new((*self.chunk_chars_.borrow()))),
            sign_: Rc::new(RefCell::new((*self.sign_.borrow()))),
            out_start_: Rc::new(RefCell::new((*self.out_start_.borrow()).clone())),
            out_end_: Rc::new(RefCell::new((*self.out_end_.borrow()).clone())),
            out_: Rc::new(RefCell::new((*self.out_.borrow()).clone())),
            chunk_divisor_: Rc::new(RefCell::new((*self.chunk_divisor_.borrow()))),
            processor_: Rc::new(RefCell::new((*self.processor_.borrow()).clone())),
            platform_: Rc::new(RefCell::new((*self.platform_.borrow()).clone())),
        }));
        let this: Ptr<v8_bigint_ToStringFormatter> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
...
impl ByteRepr for v8_bigint_ToStringFormatter {
    fn byte_size() -> usize {
        80
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.digits_.borrow()).to_bytes(&mut buf[0..16]);
        (*self.radix_.borrow()).to_bytes(&mut buf[16..20]);
        (*self.chunk_chars_.borrow()).to_bytes(&mut buf[20..24]);
        (*self.sign_.borrow()).to_bytes(&mut buf[24..25]);
        (*self.out_start_.borrow()).to_bytes(&mut buf[32..40]);
        (*self.out_end_.borrow()).to_bytes(&mut buf[40..48]);
        (*self.out_.borrow()).to_bytes(&mut buf[48..56]);
        (*self.chunk_divisor_.borrow()).to_bytes(&mut buf[56..64]);
        (*self.processor_.borrow()).to_bytes(&mut buf[64..72]);
        (*self.platform_.borrow()).to_bytes(&mut buf[72..80]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            digits_: Rc::new(RefCell::new(<v8_bigint_Digits>::from_bytes(&buf[0..16]))),
            radix_: Rc::new(RefCell::new(<i32>::from_bytes(&buf[16..20]))),
            chunk_chars_: Rc::new(RefCell::new(<u32>::from_bytes(&buf[20..24]))),
            sign_: Rc::new(RefCell::new(<bool>::from_bytes(&buf[24..25]))),
            out_start_: Rc::new(RefCell::new(<Ptr<u8>>::from_bytes(&buf[32..40]))),
            out_end_: Rc::new(RefCell::new(<Ptr<u8>>::from_bytes(&buf[40..48]))),
            out_: Rc::new(RefCell::new(<Ptr<u8>>::from_bytes(&buf[48..56]))),
            chunk_divisor_: Rc::new(RefCell::new(<u64>::from_bytes(&buf[56..64]))),
...
#[derive(Default)]
pub struct v8_bigint_RecursionLevel {
    leading_zero_shift_: Value<i32>,
    char_count_: Value<u32>,
    is_toplevel_: Value<bool>,
    next_: Value<Ptr<v8_bigint_RecursionLevel>>,
    divisor_: Value<v8_bigint_ScratchDigits>,
    inverse_storage_: Value<Option<Value<v8_bigint_Storage>>>,
    inverse_: Value<v8_bigint_Digits>,
}
impl v8_bigint_RecursionLevel {
    fn v8_bigint_RecursionLevel1(
        base_divisor: u64,
        base_char_count: u32,
        platform: PtrDyn<dyn v8_bigint_Platform>,
    ) -> Self {
        let base_divisor: Value<u64> = Rc::new(RefCell::new(base_divisor));
        let base_char_count: Value<u32> = Rc::new(RefCell::new(base_char_count));
        let platform: Value<PtrDyn<dyn v8_bigint_Platform>> = Rc::new(RefCell::new(platform));
        let __this: Value<v8_bigint_RecursionLevel> = Rc::new(RefCell::new(Self {
            leading_zero_shift_: Rc::new(RefCell::new(0)),
            char_count_: Rc::new(RefCell::new((*base_char_count.borrow()))),
            is_toplevel_: Rc::new(RefCell::new(true)),
            next_: Rc::new(RefCell::new(Ptr::<v8_bigint_RecursionLevel>::null())),
            divisor_: Rc::new(RefCell::new(
                v8_bigint_ScratchDigits::v8_bigint_ScratchDigits({ 1_u32 }, {
                    (*platform.borrow()).clone()
                }),
            )),
            inverse_storage_: Rc::new(RefCell::new(None)),
            inverse_: Rc::new(RefCell::new(v8_bigint_Digits::v8_bigint_Digits4())),
        }));
        let this: Ptr<v8_bigint_RecursionLevel> = __this.as_pointer();
        ({
            v8_bigint_RWDigits_WritableDigitReferenceImpl::operator_assign_u64(
                &Rc::new(RefCell::new(
                    ({
                        v8_bigint_RWDigitsImpl::operator_index(
                            &(*this.upgrade().deref()).divisor_.as_pointer(),
                            0_u32,
                        )
                    }),
                ))
    ...
    fn v8_bigint_RecursionLevel2(
        next: Ptr<v8_bigint_RecursionLevel>,
        platform: PtrDyn<dyn v8_bigint_Platform>,
    ) -> Self {
        let next: Value<Ptr<v8_bigint_RecursionLevel>> = Rc::new(RefCell::new(next));
        let platform: Value<PtrDyn<dyn v8_bigint_Platform>> = Rc::new(RefCell::new(platform));
        let __this: Value<v8_bigint_RecursionLevel> = Rc::new(RefCell::new(Self {
            leading_zero_shift_: Rc::new(RefCell::new(0)),
            char_count_: Rc::new(RefCell::new(
                (*(*(*next.borrow()).upgrade().deref()).char_count_.borrow()).wrapping_mul(2_u32),
            )),
            is_toplevel_: Rc::new(RefCell::new(true)),
            next_: Rc::new(RefCell::new((*next.borrow()).clone())),
            divisor_: Rc::new(RefCell::new(
                v8_bigint_ScratchDigits::v8_bigint_ScratchDigits(
                    {
                        ({
                            v8_bigint_DigitsImpl::len(
                                &(*(*next.borrow()).upgrade().deref()).divisor_.as_pointer(),
                            )
                        })
                        .wrapping_mul(2_u32)
                    },
        ...
        let this: Ptr<v8_bigint_RecursionLevel> = __this.as_pointer();
        (*(*(*next.borrow()).upgrade().deref())
            .is_toplevel_
            .borrow_mut()) = false;
        if !((*(*this.upgrade().deref()).char_count_.borrow())
            < (<u32>::MAX as u32).wrapping_div(2_u32))
        {
            write!(cerr(), "tostring.cc:{:}: ", 344,);
            write!(
                cerr(),
                "Assertion failed: char_count_ < std::numeric_limits<uint32_t>::max() / 2\n",
            );
            std::process::abort();
        };
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
...
impl ByteRepr for v8_bigint_RecursionLevel {
    fn byte_size() -> usize {
        80
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.leading_zero_shift_.borrow()).to_bytes(&mut buf[0..4]);
        (*self.char_count_.borrow()).to_bytes(&mut buf[4..8]);
        (*self.is_toplevel_.borrow()).to_bytes(&mut buf[8..9]);
        (*self.next_.borrow()).to_bytes(&mut buf[16..24]);
        (*self.divisor_.borrow()).to_bytes(&mut buf[24..56]);
        (*self.inverse_storage_.borrow()).to_bytes(&mut buf[56..64]);
        (*self.inverse_.borrow()).to_bytes(&mut buf[64..80]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            leading_zero_shift_: Rc::new(RefCell::new(<i32>::from_bytes(&buf[0..4]))),
            char_count_: Rc::new(RefCell::new(<u32>::from_bytes(&buf[4..8]))),
            is_toplevel_: Rc::new(RefCell::new(<bool>::from_bytes(&buf[8..9]))),
            next_: Rc::new(RefCell::new(<Ptr<v8_bigint_RecursionLevel>>::from_bytes(
                &buf[16..24],
            ))),
            divisor_: Rc::new(RefCell::new(<v8_bigint_ScratchDigits>::from_bytes(
                &buf[24..56],
            ))),
            inverse_storage_: Rc::new(RefCell::new(
                <Option<Value<v8_bigint_Storage>>>::from_bytes(&buf[56..64]),
            )),
            inverse_: Rc::new(RefCell::new(<v8_bigint_Digits>::from_bytes(&buf[64..80]))),
        }
    }
}
impl v8_bigint_RecursionLevel {
    pub fn CreateLevels(
        base_divisor: u64,
        base_char_count: u32,
        target_bit_length: u32,
        processor: Ptr<v8_bigint_ProcessorImpl>,
    ) -> Ptr<v8_bigint_RecursionLevel> {
        let base_divisor: Value<u64> = Rc::new(RefCell::new(base_divisor));
        let base_char_count: Value<u32> = Rc::new(RefCell::new(base_char_count));
        let target_bit_length: Value<u32> = Rc::new(RefCell::new(target_bit_length));
        let processor: Value<Ptr<v8_bigint_ProcessorImpl>> = Rc::new(RefCell::new(processor));
        let platform: Value<PtrDyn<dyn v8_bigint_Platform>> = Rc::new(RefCell::new(
            ({ v8_bigint_ProcessorImpl::platform(&(*processor.borrow())) }),
        ));
        let level: Value<Ptr<v8_bigint_RecursionLevel>> = Rc::new(RefCell::new(Ptr::alloc(
            v8_bigint_RecursionLevel::v8_bigint_RecursionLevel1(
                { (*base_divisor.borrow()) },
                { (*base_char_count.borrow()) },
                { (*platform.borrow()).clone() },
            ),
        )));
        'loop_: while {
            let _lhs = (({
                BitLength_100((*(*(*level.borrow()).upgrade().deref()).divisor_.borrow()).clone())
            })
            .wrapping_mul(2_u32))
            .wrapping_sub(1_u32);
            _lhs <= (*target_bit_length.borrow())
        } {
            let prev: Value<Ptr<v8_bigint_RecursionLevel>> =
                Rc::new(RefCell::new((*level.borrow()).clone()));
            (*level.borrow_mut()) =
                Ptr::alloc(v8_bigint_RecursionLevel::v8_bigint_RecursionLevel2(
                    { (*prev.borrow()).clone() },
                    { (*platform.borrow()).clone() },
                ));
            ({
                let _Z: v8_bigint_RWDigits =
                    (*(*(*level.borrow()).upgrade().deref()).divisor_.borrow()).clone();
                let _X: v8_bigint_Digits =
                    (*(*(*prev.borrow()).upgrade().deref()).divisor_.borrow()).clone();
                let _Y: v8_bigint_Digits =
                    (*(*(*prev.borrow()).upgrade().deref()).divisor_.borrow()).clone();
                v8_bigint_ProcessorImplImpl::Multiply(&(*processor.borrow()), _Z, _X, _Y)
            });
            if ({ v8_bigint_ProcessorImplImpl::should_terminate(&(*processor.borrow())) }) {
                {
                    let __p = (*level.borrow());
                    __p.destructor();
                    __p.delete();
                };
                return Ptr::<v8_bigint_RecursionLevel>::null();
            }
            ({
                v8_bigint_DigitsImpl::Normalize(
                    &(*(*level.borrow()).upgrade().deref()).divisor_.as_pointer(),
                )
            });
            ({ v8_bigint_RecursionLevelImpl::LeftShiftDivisor(&(*prev.borrow())) });
            ({
                v8_bigint_RecursionLevelImpl::ComputeInverse(
                    &(*prev.borrow()),
        ...
        ({ v8_bigint_RecursionLevelImpl::LeftShiftDivisor(&(*level.borrow())) });
        return (*level.borrow()).clone();
    }
}
pub fn ToStringResultLength_109(X: v8_bigint_Digits, radix: i32, sign: bool) -> u32 {
    let X: Value<v8_bigint_Digits> = Rc::new(RefCell::new(X));
    let radix: Value<i32> = Rc::new(RefCell::new(radix));
    let sign: Value<bool> = Rc::new(RefCell::new(sign));
    let bit_length: Value<u32> = Rc::new(RefCell::new(({ BitLength_100((*X.borrow()).clone()) })));
    let result: Value<u32> = <Value<u32>>::default();
    if ({ IsPowerOfTwo_96((*radix.borrow())) }) {
        let bits_per_char: Value<u32> = Rc::new(RefCell::new(
            (({ CountTrailingZeros_95(((*radix.borrow()) as u32)) }) as u32),
        ));
        (*result.borrow_mut()) = ((((*bit_length.borrow()).wrapping_sub(1_u32))
            .wrapping_div((*bits_per_char.borrow())))
        .wrapping_add(1_u32))
        .wrapping_add(((*sign.borrow()) as u32));
    } else {
        let max_bits_per_char: Value<u8> = Rc::new(RefCell::new(
            (*kMaxBitsPerChar_101.with(Value::clone).borrow())[(*radix.borrow()) as usize],
        ));
        let min_bits_per_char: Value<u8> = Rc::new(RefCell::new(
            ((((*max_bits_per_char.borrow()) as i32) - 1) as u8),
        ));
        let chars_required: Value<u64> = Rc::new(RefCell::new(((*bit_length.borrow()) as u64)));
        {
            let rhs_0 = (*chars_required.borrow()).wrapping_mul(
                ((*kBitsPerCharTableMultiplier_103.with(Value::clone).borrow()) as u64),
            );
            (*chars_required.borrow_mut()) = rhs_0
        };
        let __rhs = ((((*chars_required.borrow()).wrapping_sub(1_u64))
            .wrapping_div(((*min_bits_per_char.borrow()) as u64)))
        .wrapping_add(1_u64));
        (*chars_required.borrow_mut()) = __rhs;
        (&(0));
        (*result.borrow_mut()) = ((*chars_required.borrow()) as u32);
    }
    {
        let rhs_0 = (*result.borrow()).wrapping_add(((*sign.borrow()) as u32));
        (*result.borrow_mut()) = rhs_0
    };
    return (*result.borrow());
}
...
impl v8_bigint_ProcessorImpl for Ptr<v8_bigint_Processor> {
    ...
    fn ToString(
        ...
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        let out_length: Value<Ptr<u32>> = Rc::new(RefCell::new(out_length));
        let radix: Value<i32> = Rc::new(RefCell::new(radix));
        let sign: Value<bool> = Rc::new(RefCell::new(sign));
        ...
        ({
            let _out: Ptr<u8> = (*out.borrow()).clone();
            let _out_length: Ptr<u32> = (*out_length.borrow()).clone();
            let _X: Ptr<v8_bigint_Digits> = (X).clone();
            let _radix: i32 = (*radix.borrow());
            let _sign: bool = (*sign.borrow());
            v8_bigint_ProcessorImplImpl::ToString(
                &(*impl_.borrow()),
                _out,
                _out_length,
                _X,
...
impl v8_bigint_ProcessorImplImpl for Ptr<v8_bigint_ProcessorImpl> {
    ...
    fn ToString(
        ...
        let radix: Value<i32> = Rc::new(RefCell::new(radix));
        let sign: Value<bool> = Rc::new(RefCell::new(sign));
        let use_fast_algorithm: Value<bool> = Rc::new(RefCell::new({
            let _lhs = ({ v8_bigint_DigitsImpl::len(&X) });
            _lhs >= 23
        }));
        ({
            ...
            let _radix: i32 = (*radix.borrow());
            let _sign: bool = (*sign.borrow());
            let _use_fast_algorithm: bool = (*use_fast_algorithm.borrow());
            v8_bigint_ProcessorImplImpl::ToStringImpl(
                self,
                _out,
                _out_length,
                _X,
    ...
    fn ToStringImpl(
        ...
        let radix: Value<i32> = Rc::new(RefCell::new(radix));
        let sign: Value<bool> = Rc::new(RefCell::new(sign));
        let fast: Value<bool> = Rc::new(RefCell::new(fast));
        let formatter: Value<v8_bigint_ToStringFormatter> = Rc::new(RefCell::new(
            v8_bigint_ToStringFormatter::v8_bigint_ToStringFormatter(
                { (*X.upgrade().deref()).clone() },
                { (*radix.borrow()) },
                { (*sign.borrow()) },
                { (*out.borrow()).clone() },
                { ((*out_length.borrow()).read()) },
                { (*self).clone() },
            ),
        ));
        if ({ IsPowerOfTwo_96((*radix.borrow())) }) {
            ({ v8_bigint_ToStringFormatterImpl::BasePowerOfTwo(&formatter.as_pointer()) });
        } else if (*fast.borrow()) {
            ({ v8_bigint_ToStringFormatterImpl::Start(&formatter.as_pointer()) });
            ({ v8_bigint_ToStringFormatterImpl::Fast(&formatter.as_pointer()) });
            if ({ v8_bigint_ProcessorImplImpl::should_terminate(self) }) {
                return;
            }
        } else {
            ({ v8_bigint_ToStringFormatterImpl::Start(&formatter.as_pointer()) });
            ({ v8_bigint_ToStringFormatterImpl::Classic(&formatter.as_pointer()) });
        }
        let excess: Value<u32> = Rc::new(RefCell::new(
            ({ v8_bigint_ToStringFormatterImpl::Finish(&formatter.as_pointer()) }),
        ));
        {
            let rhs_0 = ((*out_length.borrow()).read()).wrapping_sub((*excess.borrow()));
            (*out_length.borrow()).write(rhs_0)
        };
        {
            ((*out.borrow()).offset(((*out_length.borrow()).read()) as isize) as Ptr<u8>)
                .to_any()
                .memset((0) as u8, ((*excess.borrow()) as usize) as usize);
            ((*out.borrow()).offset(((*out_length.borrow()).read()) as isize) as Ptr<u8>).to_any()
        };
    }
}
...
pub trait v8_bigint_RecursionLevelImpl {
    fn destructor(&self);
    fn ComputeInverse(&self, processor: Ptr<v8_bigint_ProcessorImpl>, dividend_length: Option<u32>);
    fn GetInverse(&self, dividend_length: u32) -> v8_bigint_Digits;
    fn LeftShiftDivisor(&self);
}
impl v8_bigint_RecursionLevelImpl for Ptr<v8_bigint_RecursionLevel> {
    fn destructor(&self) {
        {
            let __p = (*(*(*self).upgrade().deref()).next_.borrow());
            __p.destructor();
            __p.delete();
        };
    }
    fn LeftShiftDivisor(&self) {
        (*(*(*self).upgrade().deref())
            .leading_zero_shift_
            .borrow_mut()) = ({
            CountLeadingZeros_24(
                ({
                    v8_bigint_DigitsImpl::msd(&(*(*self).upgrade().deref()).divisor_.as_pointer())
                }),
            )
        });
        ({
            let _Z: v8_bigint_RWDigits = (*(*(*self).upgrade().deref()).divisor_.borrow()).clone();
            let _X: v8_bigint_Digits = (*(*(*self).upgrade().deref()).divisor_.borrow()).clone();
            let _shift: i32 = (*(*(*self).upgrade().deref()).leading_zero_shift_.borrow());
            LeftShift_91(_Z, _X, _shift)
        });
    }
    fn ComputeInverse(
        &self,
        processor: Ptr<v8_bigint_ProcessorImpl>,
        dividend_length: Option<u32>,
    ) {
        let processor: Value<Ptr<v8_bigint_ProcessorImpl>> = Rc::new(RefCell::new(processor));
        let dividend_length: Value<u32> = Rc::new(RefCell::new(dividend_length.unwrap_or(0_u32)));
        let inverse_len: Value<u32> = Rc::new(RefCell::new(
            ({ v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).divisor_.as_pointer()) }),
        ));
        if ((*dividend_length.borrow()) != 0_u32) {
            (*inverse_len.borrow_mut()) = (*dividend_length.borrow()).wrapping_sub(
                ({
                    v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).divisor_.as_pointer())
                }),
            );
        ...
        let scratch_len: Value<u32> = Rc::new(RefCell::new(
            ({ InvertScratchSpace_89((*inverse_len.borrow())) }),
        ));
        let platform: Value<PtrDyn<dyn v8_bigint_Platform>> = Rc::new(RefCell::new(
            ({ v8_bigint_ProcessorImpl::platform(&(*processor.borrow())) }),
        ));
        let scratch: Value<v8_bigint_ScratchDigits> = Rc::new(RefCell::new(
            v8_bigint_ScratchDigits::v8_bigint_ScratchDigits({ (*scratch_len.borrow()) }, {
                (*platform.borrow()).clone()
            }),
        ));
        let inv_storage: Value<Ptr<v8_bigint_Storage>> = Rc::new(RefCell::new(Ptr::alloc(
            v8_bigint_Storage::v8_bigint_Storage(
                { (*inverse_len.borrow()).wrapping_add(1_u32) },
                { (*platform.borrow()).clone() },
            ),
        )));
        {
            let _p: Ptr<_> = (*inv_storage.borrow()).clone();
            (*(*(*self).upgrade().deref()).inverse_storage_.borrow_mut()) = _p.to_owned_opt()
        };
        let inverse_initializer: Value<v8_bigint_RWDigits> =
            Rc::new(RefCell::new(v8_bigint_RWDigits::v8_bigint_RWDigits1(
                { ({ v8_bigint_StorageImpl::get(&(*inv_storage.borrow())) }) },
                { (*inverse_len.borrow()).wrapping_add(1_u32) },
            )));
        let input: Value<v8_bigint_Digits> =
            Rc::new(RefCell::new(v8_bigint_Digits::v8_bigint_Digits3(
                { (*(*(*self).upgrade().deref()).divisor_.borrow()).clone() },
                {
                    ({
                        v8_bigint_DigitsImpl::len(
                            &(*(*self).upgrade().deref()).divisor_.as_pointer(),
                        )
                    })
                    .wrapping_sub((*inverse_len.borrow()))
                },
                { (*inverse_len.borrow()) },
            )));
        ({
            v8_bigint_ProcessorImplImpl::Invert(
                &(*processor.borrow()),
                (*inverse_initializer.borrow()).clone(),
                (*input.borrow()).clone(),
                (*scratch.borrow()).clone(),
            )
        });
        ({ v8_bigint_DigitsImpl::TrimOne(&inverse_initializer.as_pointer()) });
        (*(*(*self).upgrade().deref()).inverse_.borrow_mut()) =
            (*inverse_initializer.borrow()).clone();
    }
    fn GetInverse(&self, dividend_length: u32) -> v8_bigint_Digits {
        let dividend_length: Value<u32> = Rc::new(RefCell::new(dividend_length));
        (&(0));
        let inverse_len: Value<u32> =
            Rc::new(RefCell::new((*dividend_length.borrow()).wrapping_sub(
                ({
                    v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).divisor_.as_pointer())
                }),
            )));
        ...
        return ({
            let _i: u32 = (({
                v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).inverse_.as_pointer())
            })
            .wrapping_sub((*inverse_len.borrow())));
            v8_bigint_DigitsImpl::operator_add(
                &(*(*self).upgrade().deref()).inverse_.as_pointer(),
                _i,
            )
        });
    }
...
pub trait v8_bigint_ToStringFormatterImpl {
    fn Start(&self);
    fn Finish(&self) -> u32;
    fn Classic(&self);
    fn BasePowerOfTwo(&self);
    fn Fast(&self);
    fn FillWithZeros(
        &self,
        level: Ptr<v8_bigint_RecursionLevel>,
        right_boundary: Ptr<u8>,
        out: Ptr<u8>,
    ...
    fn ProcessLevel(
        &self,
        level: Ptr<v8_bigint_RecursionLevel>,
        chunk: v8_bigint_Digits,
        out: Ptr<u8>,
        is_last_on_level: bool,
    ) -> Ptr<u8>;
    fn BasecaseLast(&self, digit: u64, out: Ptr<u8>) -> Ptr<u8>;
    fn BasecaseMiddle(&self, digit: u64, out: Ptr<u8>) -> Ptr<u8>;
}
impl v8_bigint_ToStringFormatterImpl for Ptr<v8_bigint_ToStringFormatter> {
    fn Classic(&self) {
        if (({ v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).digits_.as_pointer()) })
            == 0_u32)
        {
            ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(('0' as u8));
            return;
        }
        if (({ v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).digits_.as_pointer()) })
            == 1_u32)
        {
            let __rhs = ({
                let _digit: u64 = ({
                    v8_bigint_DigitsImpl::operator_index(
                        &(*(*self).upgrade().deref()).digits_.as_pointer(),
                        0_u32,
                    )
                });
                let _out: Ptr<u8> = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
                v8_bigint_ToStringFormatterImpl::BasecaseLast(self, _digit, _out)
            });
            (*(*(*self).upgrade().deref()).out_.borrow_mut()) = __rhs;
            return;
        }
        let rest: Value<v8_bigint_ScratchDigits> = Rc::new(RefCell::new(
            v8_bigint_ScratchDigits::v8_bigint_ScratchDigits(
                {
                    ({
                        v8_bigint_DigitsImpl::len(
                ...
                { (*(*(*self).upgrade().deref()).platform_.borrow()).clone() },
            ),
        ));
        let dividend: Value<v8_bigint_Digits> = Rc::new(RefCell::new(
            (*(*(*self).upgrade().deref()).digits_.borrow()).clone(),
        ));
        let mut __do_while = true;
        'loop_: while __do_while || (({ v8_bigint_DigitsImpl::len(&rest.as_pointer()) }) > 1_u32) {
            __do_while = false;
            if ((*(*(*self).upgrade().deref()).radix_.borrow()) == 10) {
                let __rhs = ({
                    DivideByMagic_108(
                        (*rest.borrow()).clone(),
                        (*dividend.borrow()).clone(),
                        (*(*(*self).upgrade().deref()).out_.borrow()).clone(),
                    )
                });
                (*(*(*self).upgrade().deref()).out_.borrow_mut()) = __rhs;
                (&(0));
            } else {
                let chunk: Value<u64> = <Value<u64>>::default();
                ({
                    DivideSingle_60(
                        (*rest.borrow()).clone(),
                        (chunk.as_pointer()),
                        (*dividend.borrow()).clone(),
                        (*(*(*self).upgrade().deref()).chunk_divisor_.borrow()),
                    )
                });
                let __rhs = ({
                    let _out: Ptr<u8> = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
                    v8_bigint_ToStringFormatterImpl::BasecaseMiddle(self, (*chunk.borrow()), _out)
                });
                (*(*(*self).upgrade().deref()).out_.borrow_mut()) = __rhs;
                (&(0));
            }
            (&(0));
            ({ v8_bigint_DigitsImpl::Normalize(&rest.as_pointer()) });
            (*dividend.borrow_mut()) = (*rest.borrow()).clone();
        }
        let __rhs = ({
            let _out: Ptr<u8> = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
            v8_bigint_ToStringFormatterImpl::BasecaseLast(
                self,
                ({
                    v8_bigint_RWDigits_WritableDigitReferenceImpl::operator_digit_t(
                        &Rc::new(RefCell::new(
                            ({ v8_bigint_RWDigitsImpl::operator_index(&rest.as_pointer(), 0_u32) }),
                        ))
                        .as_pointer(),
                    )
                }),
    ...
    fn BasecaseLast(&self, digit: u64, out: Ptr<u8>) -> Ptr<u8> {
        ...
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        if ((*(*(*self).upgrade().deref()).radix_.borrow()) == 10) {
            return ({ BasecaseFixedLast_107((*digit.borrow()), (*out.borrow()).clone()) });
        }
        let mut __do_while = true;
        'loop_: while __do_while || ((*digit.borrow()) > 0_u64) {
            __do_while = false;
            (&(0));
            let __rhs = (*kConversionChars_104.with(Value::clone).borrow())[((*digit.borrow())
                .wrapping_rem(((*(*(*self).upgrade().deref()).radix_.borrow()) as u64)))
                as usize];
            ((*out.borrow_mut()).prefix_dec()).write(__rhs);
            {
                let rhs_0 = (*digit.borrow())
                    .wrapping_div(((*(*(*self).upgrade().deref()).radix_.borrow()) as u64));
                (*digit.borrow_mut()) = rhs_0
            };
        }
        return (*out.borrow()).clone();
    }
    fn BasecaseMiddle(&self, digit: u64, out: Ptr<u8>) -> Ptr<u8> {
        ...
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        let i: Value<u32> = Rc::new(RefCell::new(0_u32));
        'loop_: while ((*i.borrow()) < (*(*(*self).upgrade().deref()).chunk_chars_.borrow())) {
            (&(0));
            let __rhs = (*kConversionChars_104.with(Value::clone).borrow())[((*digit.borrow())
                .wrapping_rem(((*(*(*self).upgrade().deref()).radix_.borrow()) as u64)))
                as usize];
    ...
    fn Start(&self) {
        let max_bits_per_char: Value<u32> = Rc::new(RefCell::new(
            ((*kMaxBitsPerChar_101.with(Value::clone).borrow())
                [(*(*(*self).upgrade().deref()).radix_.borrow()) as usize] as u32),
        ));
        (*(*(*self).upgrade().deref()).chunk_chars_.borrow_mut()) = ((((64 as usize)
            .wrapping_mul((*kBitsPerCharTableMultiplier_103.with(Value::clone).borrow()))
            as usize)
            .wrapping_div(((*max_bits_per_char.borrow()) as usize)))
            as u32);
        (*(*(*self).upgrade().deref()).chunk_divisor_.borrow_mut()) = ({
            let _base: u64 = ((*(*(*self).upgrade().deref()).radix_.borrow()) as u64);
            let _exponent: u64 = ((*(*(*self).upgrade().deref()).chunk_chars_.borrow()) as u64);
            digit_pow_105(_base, _exponent)
        });
        (&(0));
    }
    fn Finish(&self) -> u32 {
        ...
        (&(0));
        'loop_: while ({
            let _lhs = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
            _lhs < (*(*(*self).upgrade().deref()).out_end_.borrow()).clone()
        }) && ((((*(*(*self).upgrade().deref()).out_.borrow()).read()) as i32)
            == (('0' as u8) as i32))
        {
            (*(*(*self).upgrade().deref()).out_.borrow_mut()).postfix_inc();
        }
        if (*(*(*self).upgrade().deref()).sign_.borrow()) {
            ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(('-' as u8));
        }
        let excess: Value<u32> = Rc::new(RefCell::new(0_u32));
        if {
            let _lhs = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
            _lhs > (*(*(*self).upgrade().deref()).out_start_.borrow()).clone()
        } {
            let actual_length: Value<usize> = Rc::new(RefCell::new(
                ((((*(*(*self).upgrade().deref()).out_end_.borrow()).clone()
                    - (*(*(*self).upgrade().deref()).out_.borrow()).clone())
                    as i64) as usize),
            ));
            (*excess.borrow_mut()) = ((((*(*(*self).upgrade().deref()).out_.borrow()).clone()
                - (*(*(*self).upgrade().deref()).out_start_.borrow()).clone())
                as i64) as u32);
            {
                ((*(*(*self).upgrade().deref()).out_start_.borrow()).clone() as Ptr<u8>)
                    .to_any()
                    .memcpy(
                        &((*(*(*self).upgrade().deref()).out_.borrow()).clone() as Ptr<u8>)
                            .to_any(),
                        (*actual_length.borrow()) as usize,
                    );
                ((*(*(*self).upgrade().deref()).out_start_.borrow()).clone() as Ptr<u8>).to_any()
            };
        }
        return (*excess.borrow());
    }
    fn BasePowerOfTwo(&self) {
        let bits_per_char: Value<i32> = Rc::new(RefCell::new(
            ({ CountTrailingZeros_95(((*(*(*self).upgrade().deref()).radix_.borrow()) as u32)) }),
        ));
        let char_mask: Value<i32> = Rc::new(RefCell::new(
            ((*(*(*self).upgrade().deref()).radix_.borrow()) - 1),
        ));
        let digit: Value<u64> = Rc::new(RefCell::new(0_u64));
        let available_bits: Value<i32> = Rc::new(RefCell::new(0));
        let i: Value<u32> = Rc::new(RefCell::new(0_u32));
        'loop_: while ((*i.borrow())
            < ({ v8_bigint_DigitsImpl::len(&(*(*self).upgrade().deref()).digits_.as_pointer()) })
                .wrapping_sub(1_u32))
        {
            let new_digit: Value<u64> = Rc::new(RefCell::new(
                ({
                    v8_bigint_DigitsImpl::operator_index(
                        &(*(*self).upgrade().deref()).digits_.as_pointer(),
                        (*i.borrow()),
            ...
            let current: Value<i32> = Rc::new(RefCell::new(
                ((((*digit.borrow()) | ((*new_digit.borrow()) << (*available_bits.borrow())))
                    & ((*char_mask.borrow()) as u64)) as i32),
            ));
            let __rhs =
                (*kConversionChars_104.with(Value::clone).borrow())[(*current.borrow()) as usize];
            ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(__rhs);
            let consumed_bits: Value<i32> = Rc::new(RefCell::new(
                ((*bits_per_char.borrow()) - (*available_bits.borrow())),
            ));
            (*digit.borrow_mut()) = ((*new_digit.borrow()) >> (*consumed_bits.borrow()));
            (*available_bits.borrow_mut()) = (64 - (*consumed_bits.borrow()));
            'loop_: while ((*available_bits.borrow()) >= (*bits_per_char.borrow())) {
                let __rhs = (*kConversionChars_104.with(Value::clone).borrow())
                    [((*digit.borrow()) & ((*char_mask.borrow()) as u64)) as usize];
                ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(__rhs);
                (*digit.borrow_mut()) >>= (*bits_per_char.borrow());
                (*available_bits.borrow_mut()) -= (*bits_per_char.borrow());
            }
            (*i.borrow_mut()).postfix_inc();
        }
        let msd: Value<u64> = Rc::new(RefCell::new(
            ({ v8_bigint_DigitsImpl::msd(&(*(*self).upgrade().deref()).digits_.as_pointer()) }),
        ));
        let current: Value<i32> = Rc::new(RefCell::new(
            ((((*digit.borrow()) | ((*msd.borrow()) << (*available_bits.borrow())))
                & ((*char_mask.borrow()) as u64)) as i32),
        ));
        let __rhs =
            (*kConversionChars_104.with(Value::clone).borrow())[(*current.borrow()) as usize];
        ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(__rhs);
        (*digit.borrow_mut()) =
            ((*msd.borrow()) >> ((*bits_per_char.borrow()) - (*available_bits.borrow())));
        'loop_: while ((*digit.borrow()) != 0_u64) {
            let __rhs = (*kConversionChars_104.with(Value::clone).borrow())
                [((*digit.borrow()) & ((*char_mask.borrow()) as u64)) as usize];
            ((*(*(*self).upgrade().deref()).out_.borrow_mut()).prefix_dec()).write(__rhs);
            (*digit.borrow_mut()) >>= (*bits_per_char.borrow());
    ...
    fn Fast(&self) {
        let recursion_levels: Value<Option<Value<v8_bigint_RecursionLevel>>> =
            Rc::new(RefCell::new(
                ({
                    let _base_divisor: u64 =
                        (*(*(*self).upgrade().deref()).chunk_divisor_.borrow());
                    let _base_char_count: u32 =
                        (*(*(*self).upgrade().deref()).chunk_chars_.borrow());
                    let _target_bit_length: u32 = ({
                        BitLength_100((*(*(*self).upgrade().deref()).digits_.borrow()).clone())
                    });
                    let _processor: Ptr<v8_bigint_ProcessorImpl> =
                        (*(*(*self).upgrade().deref()).processor_.borrow()).clone();
                    v8_bigint_RecursionLevel::CreateLevels(
                        _base_divisor,
                        _base_char_count,
                        _target_bit_length,
                        _processor,
        ...
        let __rhs = ({
            let _chunk: v8_bigint_Digits = (*(*(*self).upgrade().deref()).digits_.borrow()).clone();
            let _out: Ptr<u8> = (*(*(*self).upgrade().deref()).out_.borrow()).clone();
            v8_bigint_ToStringFormatterImpl::ProcessLevel(
                self,
                (*recursion_levels.borrow()).as_pointer(),
                _chunk,
                _out,
                true,
            )
    ...
    fn FillWithZeros(
        ...
        let level: Value<Ptr<v8_bigint_RecursionLevel>> = Rc::new(RefCell::new(level));
        let right_boundary: Value<Ptr<u8>> = Rc::new(RefCell::new(right_boundary));
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        let is_last_on_level: Value<bool> = Rc::new(RefCell::new(is_last_on_level));
        if (*is_last_on_level.borrow()) {
            return (*out.borrow()).clone();
        }
        let chunk_chars: Value<u32> = Rc::new(RefCell::new(if (*level.borrow()).is_null() {
            (*(*(*self).upgrade().deref()).chunk_chars_.borrow())
        } else {
            (*(*(*level.borrow()).upgrade().deref()).char_count_.borrow()).wrapping_mul(2_u32)
        }));
        let end: Value<Ptr<u8>> = Rc::new(RefCell::new(
            (*right_boundary.borrow()).offset(-((*chunk_chars.borrow()) as isize)),
        ));
        (&(0));
        'loop_: while {
            let _lhs = (*out.borrow()).clone();
            _lhs > (*end.borrow()).clone()
        } {
            ((*out.borrow_mut()).prefix_dec()).write(('0' as u8));
        }
        return (*out.borrow()).clone();
    }
    fn ProcessLevel(
        ...
        let level: Value<Ptr<v8_bigint_RecursionLevel>> = Rc::new(RefCell::new(level));
        let chunk: Value<v8_bigint_Digits> = Rc::new(RefCell::new(chunk));
        let out: Value<Ptr<u8>> = Rc::new(RefCell::new(out));
        let is_last_on_level: Value<bool> = Rc::new(RefCell::new(is_last_on_level));
        let normalized: Value<v8_bigint_Digits> = Rc::new(RefCell::new((*chunk.borrow()).clone()));
        ({ v8_bigint_DigitsImpl::Normalize(&normalized.as_pointer()) });
        if (({ v8_bigint_DigitsImpl::len(&normalized.as_pointer()) }) <= 1_u32) {
            let right_boundary: Value<Ptr<u8>> = Rc::new(RefCell::new((*out.borrow()).clone()));
            if (({ v8_bigint_DigitsImpl::len(&normalized.as_pointer()) }) == 1_u32) {
                let __rhs = ({
                    v8_bigint_ToStringFormatterImpl::BasecaseLast(
                        self,
                        ({ v8_bigint_DigitsImpl::operator_index(&normalized.as_pointer(), 0_u32) }),
                        (*out.borrow()).clone(),
                    )
                });
            ...
            return ({
                v8_bigint_ToStringFormatterImpl::FillWithZeros(
                    self,
                    (*level.borrow()).clone(),
                    (*right_boundary.borrow()).clone(),
                    (*out.borrow()).clone(),
                    (*is_last_on_level.borrow()),
                )
            });
        }
        if {
            let _lhs = ({ v8_bigint_DigitsImpl::len(&normalized.as_pointer()) });
            _lhs < ({
                v8_bigint_DigitsImpl::len(
                    &(*(*level.borrow()).upgrade().deref()).divisor_.as_pointer(),
                )
            ...
            let __rhs = ({
                v8_bigint_ToStringFormatterImpl::ProcessLevel(
                    self,
                    (*(*(*level.borrow()).upgrade().deref()).next_.borrow()).clone(),
                    (*chunk.borrow()).clone(),
                    (*out.borrow()).clone(),
                    (*is_last_on_level.borrow()),
                )
        ...
        let allow_inplace_modification: Value<bool> = Rc::new(RefCell::new(
            (({ v8_bigint_DigitsImpl::digits(&chunk.as_pointer()) })
                != ({
                    v8_bigint_DigitsImpl::digits(&(*(*self).upgrade().deref()).digits_.as_pointer())
                })),
        ));
        let original_chunk: Value<v8_bigint_Digits> =
            Rc::new(RefCell::new((*chunk.borrow()).clone()));
        let chunk_shifted: Value<v8_bigint_ShiftedDigits> = Rc::new(RefCell::new(
            v8_bigint_ShiftedDigits::v8_bigint_ShiftedDigits1(
                { chunk.as_pointer() },
                { (*(*(*self).upgrade().deref()).platform_.borrow()).clone() },
                {
                    Some(
                        (*(*(*level.borrow()).upgrade().deref())
                            .leading_zero_shift_
                            .borrow()),
                    )
                },
                { Some((*allow_inplace_modification.borrow())) },
            ),
        ));
        (*chunk.borrow_mut()) = (*chunk_shifted.borrow()).clone();
        ({ v8_bigint_DigitsImpl::Normalize(&chunk.as_pointer()) });
        ...
        if ((*comparison.borrow()) <= 0) {
            let right_boundary: Value<Ptr<u8>> = Rc::new(RefCell::new((*out.borrow()).clone()));
            if ((*comparison.borrow()) < 0) {
                ({ v8_bigint_ShiftedDigitsImpl::Reset(&chunk_shifted.as_pointer()) });
                (*chunk.borrow_mut()) = (*original_chunk.borrow()).clone();
                let __rhs = ({
                    v8_bigint_ToStringFormatterImpl::ProcessLevel(
                        self,
                        (*(*(*level.borrow()).upgrade().deref()).next_.borrow()).clone(),
                ...
                (*out.borrow_mut()) = __rhs;
                ((*out.borrow_mut()).prefix_dec()).write(('1' as u8));
            }
            return ({
                v8_bigint_ToStringFormatterImpl::FillWithZeros(
                    self,
        ...
        let right: Value<v8_bigint_ScratchDigits> = Rc::new(RefCell::new(
            v8_bigint_ScratchDigits::v8_bigint_ScratchDigits(
                {
                    ({
                        v8_bigint_DigitsImpl::len(
        ...
        let left: Value<v8_bigint_ScratchDigits> = Rc::new(RefCell::new(
            v8_bigint_ScratchDigits::v8_bigint_ScratchDigits(
                {
                    (({ v8_bigint_DigitsImpl::len(&chunk.as_pointer()) }).wrapping_sub(
                        ({
                            v8_bigint_DigitsImpl::len(
                                &(*(*level.borrow()).upgrade().deref()).divisor_.as_pointer(),
                            )
        ...
        if ((*inverse_len.borrow()) == 0_u32) {
            ({
                v8_bigint_ProcessorImplImpl::DivideSchoolbook(
                    &(*(*(*self).upgrade().deref()).processor_.borrow()),
                    left.as_pointer(),
                    right.as_pointer(),
                    chunk.as_pointer(),
            ...
            v8_bigint_DigitsImpl::len(&(*(*level.borrow()).upgrade().deref()).divisor_.as_pointer())
        }) == 1_u32)
        {
            ({
                DivideSingle_60(
                    (*left.borrow()).clone(),
                    ({ v8_bigint_RWDigitsImpl::digits(&right.as_pointer()) }),
                    (*chunk.borrow()).clone(),
                    ({
                        v8_bigint_RWDigits_WritableDigitReferenceImpl::operator_digit_t(
                            &Rc::new(RefCell::new(
            ...
            let i: Value<u32> = Rc::new(RefCell::new(1_u32));
            'loop_: while ((*i.borrow()) < ({ v8_bigint_DigitsImpl::len(&right.as_pointer()) })) {
                ({
                    v8_bigint_RWDigits_WritableDigitReferenceImpl::operator_assign_u64(
                        &Rc::new(RefCell::new(
                            ({
            ...
            let scratch: Value<v8_bigint_ScratchDigits> = Rc::new(RefCell::new(
                v8_bigint_ScratchDigits::v8_bigint_ScratchDigits(
                    {
                        ({
                            DivideBarrettScratchSpace_86(
                                ({ v8_bigint_DigitsImpl::len(&chunk.as_pointer()) }),
                            )
                        })
                    },
                    { (*(*(*self).upgrade().deref()).platform_.borrow()).clone() },
            ...
            if (*(*(*level.borrow()).upgrade().deref()).is_toplevel_.borrow()) {
                ({
                    v8_bigint_RecursionLevelImpl::ComputeInverse(
                        &(*level.borrow()),
                        (*(*(*self).upgrade().deref()).processor_.borrow()).clone(),
                        Some(({ v8_bigint_DigitsImpl::len(&chunk.as_pointer()) })),
                    )
                });
                if ({
                    v8_bigint_ProcessorImplImpl::should_terminate(
            ...
            let inverse: Value<v8_bigint_Digits> = Rc::new(RefCell::new(
                ({
                    v8_bigint_RecursionLevelImpl::GetInverse(
                        &(*level.borrow()),
                        ({ v8_bigint_DigitsImpl::len(&chunk.as_pointer()) }),
                    )
                }),
            ...
            ({
                v8_bigint_ProcessorImplImpl :: DivideBarrett_v8_bigint_RWDigits_v8_bigint_RWDigits_v8_bigint_Digits_v8_bigint_Digits_v8_bigint_Digits_v8_bigint_RWDigits ( &(*(*(* self ) .upgrade().deref()) . processor_ .borrow())  , ((*left.borrow()) ).clone()  , ((*right.borrow()) ).clone()  , ((*chunk.borrow()) ).clone()  , ((*(*(*level.borrow()) .upgrade().deref()) . divisor_ .borrow()) ).clone()  , ((*inverse.borrow()) ).clone()  , ((*scratch.borrow()) ).clone()  , )
            });
            if ({
                v8_bigint_ProcessorImplImpl::should_terminate(
                    &(*(*(*self).upgrade().deref()).processor_.borrow()),
        ...
        ({
            let _Z: v8_bigint_RWDigits = (*right.borrow()).clone();
            let _X: v8_bigint_Digits = (*right.borrow()).clone();
            RightShift_92(
                _Z,
                _X,
                (*(*(*level.borrow()).upgrade().deref())
        ...
        let end_of_right_part: Value<Ptr<u8>> = Rc::new(RefCell::new(
            ({
                v8_bigint_ToStringFormatterImpl::ProcessLevel(
                    self,
                    (*(*(*level.borrow()).upgrade().deref()).next_.borrow()).clone(),
        ...
        (&(0));
        (&(*end_of_right_part.borrow_mut()));
        return ({
            let _level: Ptr<v8_bigint_RecursionLevel> =
                (*(*(*level.borrow()).upgrade().deref()).next_.borrow()).clone();
            let _out: Ptr<u8> = (*out.borrow())
                .offset(-((*(*(*level.borrow()).upgrade().deref()).char_count_.borrow()) as isize));
            v8_bigint_ToStringFormatterImpl::ProcessLevel(
                self,
                _level,
                (*left.borrow()).clone(),

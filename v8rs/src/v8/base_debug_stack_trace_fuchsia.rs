...
pub fn SharedMemoryHandleFromMachMemoryEntry_5(handle: u32) -> v8_SharedMemoryHandle {
    let handle: Value<u32> = Rc::new(RefCell::new(handle));
    return ({ v8_SharedMemoryHandle::FromPlatformHandle((*handle.borrow())) });
}
pub fn MachMemoryEntryFromSharedMemoryHandle_6(handle: v8_SharedMemoryHandle) -> u32 {
...
thread_local!(
    pub static kInvalidSharedMemoryHandle_7: Value<std_optional_v8_SharedMemoryHandle_> =
        Rc::new(RefCell::new(
            std_optional_v8_SharedMemoryHandle_::std_optional_v8_SharedMemoryHandle_1({
                (*nullopt_8.with(Value::clone).borrow()).clone()
            }),
        ));
);
...
pub fn operator_bitor_9(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
...
pub fn operator_bitand_10(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return ((((*lhs.borrow()) as i32) & ((*rhs.borrow()) as i32)) as v8_PagePermissions);
}
pub fn operator_bitor_assign_11(
    ...
    let __rhs = ({
        ...
        operator_bitor_9(_lhs, _rhs)
    });
    lhs.write(__rhs);
    return (lhs).clone();
}
pub fn IsSubset_12(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> bool {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return (({
        let _lhs: v8_PagePermissions = (*lhs.borrow());
        operator_bitand_10(_lhs, (*rhs.borrow()))
    }) == (*lhs.borrow()));
}
...
pub trait v8_Platform {
    ...
    fn CurrentClockTimeMilliseconds(&self) -> i64 {
        return (({ floor_13(({ self.CurrentClockTimeMillis() })) }) as i64);
    }
    ...
    fn GetHighAllocationThroughputObserver(&self) -> Ptr<v8_HighAllocationThroughputObserver> {
        thread_local!(
            static default_observer_14: Value<v8_HighAllocationThroughputObserver> = Rc::new(
                RefCell::new(<v8_HighAllocationThroughputObserver>::default()),
            );
        );
        return (default_observer_14.with(Value::clone).as_pointer());
    }
    fn CreateJobImpl(
        &self,
        priority: v8_TaskPriority,
...
thread_local!();
pub fn ControlledCrashesAreHarmless_16() -> bool {
    return ((*g_abort_mode_15.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_15.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn DcheckFailuresAreIgnored_17() -> bool {
    return ((*g_abort_mode_15.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_15.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn FatalErrorsWithNoSecurityImpactShouldExit_18() -> bool {
    return ((*g_abort_mode_15.with(Value::clone).borrow())
...
pub fn EnableInProcessStackDumping_41() -> bool {
    return false;
}
pub fn DisableSignalStackDump_42() {}
...
impl v8_base_debug_StackTraceImpl for Ptr<v8_base_debug_StackTrace> {
    fn Print(&self) {
        let backtrace: Value<Vec<u8>> = Rc::new(RefCell::new(
            ({ v8_base_debug_StackTraceImpl::ToString(self) }),
        ));
        ({
            v8_base_OS::Print(
                Ptr::from_string_literal(b"%s\n"),
                &[(backtrace.as_pointer() as Ptr<u8>).into()],
            )
        });
    }
    fn OutputToStream(&self, os: Ptr<std::fs::File>) {
        ...
        'loop_: while ((*i.borrow()) < (*(*(*self).upgrade().deref()).count_.borrow())) {
            write!(
                (*os.borrow()),
                "#{:}{:}{:}\n",
                2,
                (*i.borrow()),
                (*(*(*self).upgrade().deref()).trace_.borrow())[(*i.borrow()) as usize],
            );

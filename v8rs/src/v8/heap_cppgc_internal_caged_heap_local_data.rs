...
use std::rc::{Rc, Weak};
thread_local!(
    pub static kKB_0: Value<usize> = Rc::new(RefCell::new(1024));
);
thread_local!(
    pub static kMB_1: Value<usize> = Rc::new(RefCell::new(1048576));
);
thread_local!(
    pub static kGB_2: Value<usize> = Rc::new(RefCell::new(1073741824));
);
thread_local!(
    pub static kFullyConstructedBitFieldOffsetFromPayload_3: Value<usize> =
        Rc::new(RefCell::new(4));
);
thread_local!(
    pub static kFullyConstructedBitMask_4: Value<u16> = Rc::new(RefCell::new(32768));
);
thread_local!(
    pub static kPageSizeBits_5: Value<usize> = Rc::new(RefCell::new(17));
);
thread_local!(
    pub static kPageSize_6: Value<usize> = Rc::new(RefCell::new(131072));
);
thread_local!(
    pub static kLargeObjectSizeThreshold_7: Value<usize> = Rc::new(RefCell::new(65536));
);
thread_local!(
    pub static kPointerCompressionShift_8: Value<u32> = Rc::new(RefCell::new(3));
);
thread_local!(
    pub static kCagedHeapDefaultReservationSize_9: Value<usize> = Rc::new(RefCell::new(4294967296));
);
thread_local!(
    pub static kCagedHeapMaxReservationSize_10: Value<usize> = Rc::new(RefCell::new(17179869184));
);
thread_local!(
    pub static kCagedHeapReservationAlignment_11: Value<usize> = Rc::new(RefCell::new(17179869184));
);
thread_local!(
    pub static kDefaultAlignment_12: Value<usize> = Rc::new(RefCell::new(8));
);
thread_local!(
    pub static kMaxSupportedAlignment_13: Value<usize> = Rc::new(RefCell::new(16));
);
thread_local!(
    pub static kAllocationGranularity_14: Value<usize> = Rc::new(RefCell::new(8));
);
thread_local!(
    pub static kCachelineSize_15: Value<usize> = Rc::new(RefCell::new(64));
);
#[derive(Default)]
...
impl cppgc_internal_CagedHeapBase {
    ...
    pub fn IsWithinCage(address: AnyPtr) -> bool {
        ...
        return {
            ...
            _lhs == (*g_heap_base__16.with(Value::clone).borrow())
        };
    }
    pub fn AreWithinCage(addr1: AnyPtr, addr2: AnyPtr) -> bool {
        ...
        thread_local!(
            static kHeapBaseShift_18: Value<usize> = Rc::new(RefCell::new(34));
        );
        (&(cppgc_internal_EatParams_const_char__ref_arr_arr_ {}));
        return !(({
            let _lhs = ({
    ...
    pub fn GetBase() -> u64 {
        return (*g_heap_base__16.with(Value::clone).borrow());
    }
    pub fn GetAgeTableSize() -> usize {
        return (*g_age_table_size__17.with(Value::clone).borrow());
    }
}
...
pub fn SharedMemoryHandleFromMachMemoryEntry_19(handle: u32) -> v8_SharedMemoryHandle {
    let handle: Value<u32> = Rc::new(RefCell::new(handle));
    return ({ v8_SharedMemoryHandle::FromPlatformHandle((*handle.borrow())) });
}
pub fn MachMemoryEntryFromSharedMemoryHandle_20(handle: v8_SharedMemoryHandle) -> u32 {
...
thread_local!(
    pub static kInvalidSharedMemoryHandle_21: Value<std_optional_v8_SharedMemoryHandle_> =
        Rc::new(RefCell::new(
            std_optional_v8_SharedMemoryHandle_::std_optional_v8_SharedMemoryHandle_1({
                (*nullopt_22.with(Value::clone).borrow()).clone()
            }),
        ));
);
...
pub fn operator_bitor_23(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
...
pub fn operator_bitand_24(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> v8_PagePermissions {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return ((((*lhs.borrow()) as i32) & ((*rhs.borrow()) as i32)) as v8_PagePermissions);
}
pub fn operator_bitor_assign_25(
    ...
    let __rhs = ({
        ...
        operator_bitor_23(_lhs, _rhs)
    });
    lhs.write(__rhs);
    return (lhs).clone();
}
pub fn IsSubset_26(lhs: v8_PagePermissions, rhs: v8_PagePermissions) -> bool {
    let lhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(lhs));
    let rhs: Value<v8_PagePermissions> = Rc::new(RefCell::new(rhs));
    return (({
        let _lhs: v8_PagePermissions = (*lhs.borrow());
        operator_bitand_24(_lhs, (*rhs.borrow()))
    }) == (*lhs.borrow()));
}
...
pub trait v8_Platform {
    ...
    fn CurrentClockTimeMilliseconds(&self) -> i64 {
        return (({ floor_27(({ self.CurrentClockTimeMillis() })) }) as i64);
    }
    ...
    fn GetHighAllocationThroughputObserver(&self) -> Ptr<v8_HighAllocationThroughputObserver> {
        thread_local!(
            static default_observer_28: Value<v8_HighAllocationThroughputObserver> = Rc::new(
                RefCell::new(<v8_HighAllocationThroughputObserver>::default()),
            );
        );
        return (default_observer_28.with(Value::clone).as_pointer());
    }
    fn CreateJobImpl(
        &self,
        priority: v8_TaskPriority,
...
thread_local!(
    static kRequiredSize_29: Value<usize> = Rc::new(RefCell::new(1048576));
);
thread_local!(
    static kAllocationGranularity_30: Value<usize> = Rc::new(RefCell::new(8));
);
pub type cppgc_internal_AgeTable_Age = u8;
pub const cppgc_internal_AgeTable_Age_kOld: cppgc_internal_AgeTable_Age = 0;
pub const cppgc_internal_AgeTable_Age_kYoung: cppgc_internal_AgeTable_Age = 1;
pub const cppgc_internal_AgeTable_Age_kMixed: cppgc_internal_AgeTable_Age = 2;
pub type cppgc_internal_AgeTable_AdjacentCardsPolicy = u8;
pub const cppgc_internal_AgeTable_AdjacentCardsPolicy_kConsider:
    cppgc_internal_AgeTable_AdjacentCardsPolicy = 0;
pub const cppgc_internal_AgeTable_AdjacentCardsPolicy_kIgnore:
    cppgc_internal_AgeTable_AdjacentCardsPolicy = 1;
thread_local!(
    pub static kCardSizeInBytes_31: Value<usize> = Rc::new(RefCell::new(4096));
);
#[derive()]
pub struct cppgc_internal_AgeTable {
    table_: Value<Box<[cppgc_internal_AgeTable_Age]>>,
}
impl cppgc_internal_AgeTable {
    pub fn CalculateAgeTableSizeForHeapSize(heap_size: usize) -> usize {
        let heap_size: Value<usize> = Rc::new(RefCell::new(heap_size));
        return (*heap_size.borrow()).wrapping_div(4096);
    }
}
impl Clone for cppgc_internal_AgeTable {
    fn clone(&self) -> Self {
        let __this: Value<cppgc_internal_AgeTable> = Rc::new(RefCell::new(Self {
            table_: Rc::new(RefCell::new(
                (0..0)
                    .map(|_| <cppgc_internal_AgeTable_Age>::default())
                    .collect::<Box<[cppgc_internal_AgeTable_Age]>>(),
            )),
        }));
        let this: Ptr<cppgc_internal_AgeTable> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl Default for cppgc_internal_AgeTable {
    fn default() -> Self {
        cppgc_internal_AgeTable {
            table_: Rc::new(RefCell::new(
                (0..0)
                    .map(|_| <cppgc_internal_AgeTable_Age>::default())
                    .collect::<Box<[cppgc_internal_AgeTable_Age]>>(),
...
impl ByteRepr for cppgc_internal_AgeTable {
    fn byte_size() -> usize {
        0
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.table_.borrow()).to_bytes(&mut buf[0..0]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            table_: Rc::new(RefCell::new(
                <Box<[cppgc_internal_AgeTable_Age]>>::from_bytes(&buf[0..0]),
            )),
        }
    }
}
#[derive(Default)]
pub struct cppgc_internal_CagedHeapLocalData {
    pub age_table: Value<cppgc_internal_AgeTable>,
}
impl cppgc_internal_CagedHeapLocalData {
    pub fn Get() -> Ptr<cppgc_internal_CagedHeapLocalData> {
        return Rc::new(RefCell::new(({ cppgc_internal_CagedHeapBase::GetBase() })))
            .as_pointer()
            .reinterpret_cast::<cppgc_internal_CagedHeapLocalData>();
    }
    pub fn CalculateLocalDataSizeForHeapSize(heap_size: usize) -> usize {
        let heap_size: Value<usize> = Rc::new(RefCell::new(heap_size));
        return ({
            cppgc_internal_AgeTable::CalculateAgeTableSizeForHeapSize((*heap_size.borrow()))
        });
    }
}
impl Clone for cppgc_internal_CagedHeapLocalData {
    fn clone(&self) -> Self {
        let __this: Value<cppgc_internal_CagedHeapLocalData> = Rc::new(RefCell::new(Self {
            age_table: Rc::new(RefCell::new((*self.age_table.borrow()).clone())),
        }));
        let this: Ptr<cppgc_internal_CagedHeapLocalData> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for cppgc_internal_CagedHeapLocalData {
    fn byte_size() -> usize {
        0
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.age_table.borrow()).to_bytes(&mut buf[0..0]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            age_table: Rc::new(RefCell::new(<cppgc_internal_AgeTable>::from_bytes(
                &buf[0..0],
            ))),
        }
    }
...
thread_local!();
pub fn ControlledCrashesAreHarmless_33() -> bool {
    return ((*g_abort_mode_32.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_32.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn DcheckFailuresAreIgnored_34() -> bool {
    return ((*g_abort_mode_32.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_32.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn FatalErrorsWithNoSecurityImpactShouldExit_35() -> bool {
    return ((*g_abort_mode_32.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitIfNoSecurityImpact);
}
thread_local!(
    pub static kReturnAddressStackSlotCount_36: Value<i32> = Rc::new(RefCell::new(0));
);
thread_local!(
    pub static kPageSizeBits_37: Value<i32> = Rc::new(RefCell::new(18));
);
thread_local!(
    pub static kRegularPageSize_38: Value<i32> = Rc::new(RefCell::new(262144));
);
thread_local!(
    pub static kMinimumOSPageSize_39: Value<i32> = Rc::new(RefCell::new(16384));
);
thread_local!(
    pub static kUnimplementedCodeMessage_40: Value<Ptr<u8>> = Rc::new(RefCell::new(
        Ptr::from_string_literal(b"unimplemented code"),
    ));
);
thread_local!(
    pub static kUnreachableCodeMessage_41: Value<Ptr<u8>> =
...
thread_local!(
    pub static is_enum_42: Value<bool> = Rc::new(RefCell::new(false));
);
#[derive(Default)]
...
thread_local!(
    pub static is_enum_43: Value<bool> = Rc::new(RefCell::new(false));
);
#[derive(Default)]
...
thread_local!(
    pub static value_44: Value<bool> = Rc::new(RefCell::new(false));
);
#[derive(Default)]
...
thread_local!(
    pub static value_45: Value<bool> = Rc::new(RefCell::new(true));
);
#[derive(Default)]
...
thread_local!(
    pub static value_46: Value<bool> = Rc::new(RefCell::new(false));
);
#[derive(Default)]
...
thread_local!(
    pub static value_47: Value<bool> = Rc::new(RefCell::new(false));
);
#[derive(Default)]
...
pub fn make_uint64_48(high: u32, low: u32) -> u64 {
...
pub fn IsAligned_51(value: u64, alignment: u64) -> bool {
...
pub trait cppgc_internal_AgeTableImpl {
    fn SetAge(&self, cage_offset: u64, age: cppgc_internal_AgeTable_Age);
    fn GetAge(&self, cage_offset: u64) -> cppgc_internal_AgeTable_Age;
    fn SetAgeForRange(
        &self,
        offset_begin: u64,
        offset_end: u64,
        age: cppgc_internal_AgeTable_Age,
        adjacent_cards_policy: cppgc_internal_AgeTable_AdjacentCardsPolicy,
    );
    fn GetAgeForRange(&self, offset_begin: u64, offset_end: u64) -> cppgc_internal_AgeTable_Age;
    fn ResetForTesting(&self);
    fn card(&self, offset: u64) -> usize;
}
impl cppgc_internal_AgeTableImpl for Ptr<cppgc_internal_AgeTable> {
    fn SetAge(&self, cage_offset: u64, age: cppgc_internal_AgeTable_Age) {
        let cage_offset: Value<u64> = Rc::new(RefCell::new(cage_offset));
        let age: Value<cppgc_internal_AgeTable_Age> = Rc::new(RefCell::new(age));
        (*(*(*self).upgrade().deref()).table_.borrow_mut())
            [({ cppgc_internal_AgeTableImpl::card(self, (*cage_offset.borrow())) }) as usize] =
            (*age.borrow());
    }
    fn GetAge(&self, cage_offset: u64) -> cppgc_internal_AgeTable_Age {
        let cage_offset: Value<u64> = Rc::new(RefCell::new(cage_offset));
        return (*(*(*self).upgrade().deref()).table_.borrow())
            [({ cppgc_internal_AgeTableImpl::card(self, (*cage_offset.borrow())) }) as usize];
    }
    fn card(&self, offset: u64) -> usize {
        let offset: Value<u64> = Rc::new(RefCell::new(offset));
        let kGranularityBits: Value<usize> = Rc::new(RefCell::new(12));
        let entry: Value<usize> = Rc::new(RefCell::new((((*offset.borrow()) >> 12) as usize)));
        (&(cppgc_internal_EatParams_const_char__ref_arr_arr_ {}));
        return (*entry.borrow());
    }
    fn SetAgeForRange(
        ...
        let offset_begin: Value<u64> = Rc::new(RefCell::new(offset_begin));
        let offset_end: Value<u64> = Rc::new(RefCell::new(offset_end));
        let age: Value<cppgc_internal_AgeTable_Age> = Rc::new(RefCell::new(age));
        let adjacent_cards_policy: Value<cppgc_internal_AgeTable_AdjacentCardsPolicy> =
            Rc::new(RefCell::new(adjacent_cards_policy));
        let inner_card_offset_begin: Value<u64> = Rc::new(RefCell::new(
            ({ RoundUp_50((*offset_begin.borrow()), (4096 as i64)) }),
        ));
        let outer_card_offset_end: Value<u64> = Rc::new(RefCell::new(
            ({ RoundDown_49((*offset_end.borrow()), (4096 as i64)) }),
        ));
        let inner_offset: Value<u64> = Rc::new(RefCell::new((*inner_card_offset_begin.borrow())));
        'loop_: while ((*inner_offset.borrow()) < (*outer_card_offset_end.borrow())) {
            ({
                cppgc_internal_AgeTableImpl::SetAge(self, (*inner_offset.borrow()), (*age.borrow()))
            });
            {
                let rhs_0 = (*inner_offset.borrow()).wrapping_add((4096 as u64));
                (*inner_offset.borrow_mut()) = rhs_0
            };
        }
        let set_age_for_outer_card: Value<_> = Rc::new(RefCell::new(
            (|offset: u64| {
                let offset: Value<u64> = Rc::new(RefCell::new(offset));
                if ({ IsAligned_51((*offset.borrow()), (4096 as u64)) }) {
                    return;
                }
                if ((*adjacent_cards_policy.borrow())
                    == cppgc_internal_AgeTable_AdjacentCardsPolicy_kIgnore)
                {
                    ({
                        cppgc_internal_AgeTableImpl::SetAge(
                            &(*self),
                            (*offset.borrow()),
                            (*age.borrow()),
                        )
                    });
                } else if (({ cppgc_internal_AgeTableImpl::GetAge(&(*self), (*offset.borrow())) })
                    != (*age.borrow()))
                {
                    ({
                        cppgc_internal_AgeTableImpl::SetAge(
                            &(*self),
                            (*offset.borrow()),
                            cppgc_internal_AgeTable_Age_kMixed,
                        )
                    });
                }
            }),
        ));
        ({ (*set_age_for_outer_card.borrow_mut())((*offset_begin.borrow())) });
        ({ (*set_age_for_outer_card.borrow_mut())((*offset_end.borrow())) });
    }
    fn GetAgeForRange(&self, offset_begin: u64, offset_end: u64) -> cppgc_internal_AgeTable_Age {
        let offset_begin: Value<u64> = Rc::new(RefCell::new(offset_begin));
        let offset_end: Value<u64> = Rc::new(RefCell::new(offset_end));
        let result: Value<cppgc_internal_AgeTable_Age> = Rc::new(RefCell::new(
            ({ cppgc_internal_AgeTableImpl::GetAge(self, (*offset_begin.borrow())) }),
        ));
        let offset: Value<u64> = Rc::new(RefCell::new(
            (*offset_begin.borrow()).wrapping_add((4096 as u64)),
        ));
        'loop_: while ((*offset.borrow()) < (*offset_end.borrow())) {
            if ((*result.borrow())
                != ({ cppgc_internal_AgeTableImpl::GetAge(self, (*offset.borrow())) }))
            {
                (*result.borrow_mut()) = cppgc_internal_AgeTable_Age_kMixed;
            }
            {
                let rhs_0 = (*offset.borrow()).wrapping_add((4096 as u64));
    ...
    fn ResetForTesting(&self) {
        ({
            let ___first: Ptr<cppgc_internal_AgeTable_Age> =
                (((*(*self).upgrade().deref()).table_.as_pointer()
                    as Ptr<cppgc_internal_AgeTable_Age>)
                    .offset(0));
            let ___last: Ptr<cppgc_internal_AgeTable_Age> =
                (((*(*self).upgrade().deref()).table_.as_pointer()
                    as Ptr<cppgc_internal_AgeTable_Age>)
                    .offset(({ cppgc_internal_CagedHeapBase::GetAgeTableSize() })));
            let ___value: Value<cppgc_internal_AgeTable_Age> =
                Rc::new(RefCell::new(cppgc_internal_AgeTable_Age_kOld));
            fill_52(___first, ___last, ___value.as_pointer())
        });
    }
}

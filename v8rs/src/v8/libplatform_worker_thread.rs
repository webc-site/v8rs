...
#[derive(Default)]
pub struct v8_platform_WorkerThread {
    queue_: Value<Ptr<v8_platform_TaskQueue>>,
}
impl v8_platform_WorkerThread {
    pub fn v8_platform_WorkerThread(queue: Ptr<v8_platform_TaskQueue>) -> Self {
        let queue: Value<Ptr<v8_platform_TaskQueue>> = Rc::new(RefCell::new(queue));
        let __this: Value<v8_platform_WorkerThread> = Rc::new(RefCell::new(Self {
            queue_: Rc::new(RefCell::new((*queue.borrow()).clone())),
        }));
        let this: Ptr<v8_platform_WorkerThread> = __this.as_pointer();
...
impl v8_base_Thread for v8_platform_WorkerThread {
    fn Run(&self) {
        'loop_: while ({ (*task.borrow())() }) {
            ({ (*(*task.borrow()).as_ref().unwrap().borrow()).Run() });
        }
    }
}
impl ByteRepr for v8_platform_WorkerThread {
    fn byte_size() -> usize {
        56
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.queue_.borrow()).to_bytes(&mut buf[48..56]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            queue_: Rc::new(RefCell::new(<Ptr<v8_platform_TaskQueue>>::from_bytes(
                &buf[48..56],
            ))),
        }
    }
...
thread_local!(
    pub static kLowZeroBits_42: Value<i32> = Rc::new(RefCell::new(8));
);
thread_local!(
    pub static kAlignment_43: Value<i32> = Rc::new(RefCell::new(256));
);
...
thread_local!(
    pub static kToBePaddedSize_44: Value<usize> = Rc::new(RefCell::new(33));
);
#[derive(Default)]
...
pub fn is_constant_evaluated_47() -> bool {
    return ({ is_constant_evaluated_48() });
}
...
pub fn LogSeverities_49() -> Vec<absl_LogSeverity> {
    return vec![
...
thread_local!(
    pub static kLogDebugFatal_50: Value<absl_LogSeverity> = Rc::new(RefCell::new(2));
);
pub fn LogSeverityName_51(s: absl_LogSeverity) -> Ptr<u8> {
...
pub fn NormalizeLogSeverity_52(s: absl_LogSeverity) -> absl_LogSeverity {
    let s: Value<absl_LogSeverity> = Rc::new(RefCell::new(s));
...
pub fn NormalizeLogSeverity_53(s: i32) -> absl_LogSeverity {
    let s: Value<i32> = Rc::new(RefCell::new(s));
    return ({ NormalizeLogSeverity_52(((*s.borrow()) as absl_LogSeverity)) });
}
...
pub fn operator_gt_54(lhs: absl_LogSeverityAtLeast, rhs: absl_LogSeverity) -> bool {
...
pub fn operator_lt_55(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtLeast) -> bool {
...
pub fn operator_le_56(lhs: absl_LogSeverityAtLeast, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtLeast> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) <= (*rhs.borrow()));
}
pub fn operator_ge_57(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtLeast) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtLeast> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) >= ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn operator_lt_58(lhs: absl_LogSeverityAtMost, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) < (*rhs.borrow()));
}
pub fn operator_gt_59(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtMost) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) > ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn operator_ge_60(lhs: absl_LogSeverityAtMost, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) >= (*rhs.borrow()));
}
pub fn operator_le_61(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtMost) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) <= ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn Basename_62(fname: Ptr<u8>, offset: i32) -> Ptr<u8> {
            ...
            == (('\\' as u8) as i32))
    {
        ...
        ({ Basename_62((*fname.borrow()).clone(), ((*offset.borrow()) - 1)) })
    };
}
thread_local!();
pub fn ClippedSubstr_64(s: Vec<u8>, pos: usize, n: Option<usize>) -> Vec<u8> {
...
pub fn NullSafeStringView_65(p: Ptr<u8>) -> Vec<u8> {
...
pub fn days_per_century_68(yi: i32) -> i32 {
    let yi: Value<i32> = Rc::new(RefCell::new(yi));
    return (36524 + ((((*yi.borrow()) == 0) || ((*yi.borrow()) > 300)) as i32));
}
pub fn days_per_4years_69(yi: i32) -> i32 {
    let yi: Value<i32> = Rc::new(RefCell::new(yi));
    return (1460
        + (((((*yi.borrow()) == 0) || ((*yi.borrow()) > 300))
            || ((((*yi.borrow()) - 1) % 100) < 96)) as i32));
}
pub fn days_per_year_70(y: i64, m: i8) -> i32 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
    return if ({ is_leap_year_66(((*y.borrow()) + ((((*m.borrow()) as i32) > 2) as i64))) }) {
        366
    } else {
        365
    };
}
pub fn days_per_month_71(y: i64, m: i8) -> i32 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
    ...
    return ((*k_days_per_month.borrow())[(*m.borrow()) as usize]
        + (((((*m.borrow()) as i32) == 2) && ({ is_leap_year_66((*y.borrow())) })) as i32));
}
pub fn n_day_72(
    y: i64,
    m: i8,
    ...
    if ((*d.borrow()) > 0_i64) {
        ...
        if ((*d.borrow()) > (-365_i32 as i64)) {
            (*ey.borrow_mut()) -= 1_i64;
            (*d.borrow_mut()) += (({ days_per_year_70((*ey.borrow()), (*m.borrow())) }) as i64);
        } else {
            (*ey.borrow_mut()) -= 400_i64;
            (*d.borrow_mut()) += 146097_i64;
        }
    ...
    if ((*d.borrow()) > 365_i64) {
        let yi: Value<i32> = Rc::new(RefCell::new(
            ({ year_index_67((*ey.borrow()), (*m.borrow())) }),
        ));
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(({ days_per_century_68((*yi.borrow())) })));
        ...
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(({ days_per_4years_69((*yi.borrow())) })));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
            (*d.borrow_mut()) -= ((*n.borrow()) as i64);
        ...
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(
                ({ days_per_year_70((*ey.borrow()), (*m.borrow())) }),
            ));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
    ...
    if ((*d.borrow()) > 28_i64) {
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(
                ({ days_per_month_71((*ey.borrow()), (*m.borrow())) }),
            ));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
...
pub fn step_78(
    ...
    return ({
        ...
        n_min_75(_y, _m, _d, _hh, 0_i64, _mm, _ss)
    });
}
pub fn step_79(
    ...
    return ({
        ...
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_hour_74(_y, _m, _d, 0_i64, _hh, _mm, _ss)
    });
}
pub fn step_80(
    ...
    return ({
        ...
        let _mm: i8 = (*(*f.borrow()).mm.borrow());
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_day_72(_y, _m, _d, (*n.borrow()), _hh, _mm, _ss)
    });
}
pub fn step_81(
    ...
    return ({
        ...
        let _mm: i8 = (*(*f.borrow()).mm.borrow());
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_mon_73(_y, _m, _d, 0_i64, _hh, _mm, _ss)
    });
}
pub fn step_82(
...
pub fn scale_add_83(v: i64, f: i64, a: i64) -> i64 {
...
pub fn ymd_ord_84(y: i64, m: i8, d: i8) -> i64 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
...
pub fn day_difference_85(y1: i64, m1: i8, d1: i8, y2: i64, m2: i8, d2: i8) -> i64 {
    ...
    let delta: Value<i64> = Rc::new(RefCell::new(
        (({ ymd_ord_84((*a_c4_off.borrow()), (*m1.borrow()), (*d1.borrow())) })
            - ({ ymd_ord_84((*b_c4_off.borrow()), (*m2.borrow()), (*d2.borrow())) })),
    ));
...
pub fn difference_88(
    ...
    return ({
        ...
        day_difference_85(_y1, _m1, _d1, _y2, _m2, _d2)
    });
}
pub fn difference_89(
    ...
    return ({
        ...
        scale_add_83(_v, 24_i64, _a)
    });
}
pub fn difference_90(
    _a0: absl_time_internal_cctz_detail_minute_tag,
    f1: absl_time_internal_cctz_detail_fields,
    f2: absl_time_internal_cctz_detail_fields,
) -> i64 {
    ...
    return ({
        ...
        scale_add_83(_v, 60_i64, _a)
    });
}
pub fn difference_91(
    _a0: absl_time_internal_cctz_detail_second_tag,
    f1: absl_time_internal_cctz_detail_fields,
    f2: absl_time_internal_cctz_detail_fields,
) -> i64 {
...
pub fn operator_add_98(
    ...
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_80 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , (*n.borrow())  , ) } )    } , )   ;
}
pub fn operator_sub_99(
    a: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_,
    n: i64,
) -> absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ {
    let a: Value<
    ...
    return if ((*n.borrow()) != <i64>::MIN) {
        absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_80 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , - (*n.borrow())  , ) } )    } , )
    } else {
        absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_80 ( absl_time_internal_cctz_detail_day_tag { }  , (  { step_80 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , - ( ( (*n.borrow()) + 1_i64 ) )  , ) } )   , 1_i64  , ) } )    } , )
    };
}
#[derive()]
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_10(
        ...
        > = Rc::new(RefCell::new(Self {
            f_: Rc::new(RefCell::new(
                ({ align_93(absl_time_internal_minute_tag {}, (*f.borrow()).clone()) }),
            )),
        }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_13(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_94(absl_time_internal_hour_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_17(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_95(absl_time_internal_day_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_20(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_96(absl_time_internal_month_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_23(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_97(absl_time_internal_year_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_> =
            __this.as_pointer();
...
pub fn next_weekday_101(
    ...
    let base: Value<absl_time_internal_cctz_detail_weekday> = Rc::new(RefCell::new(
        ({
            ...
            get_weekday_100(_cs.as_pointer())
        }),
    ));
            ...
            'loop_: while true {
                if ((*wd.borrow()) == (*k_weekdays_forw.borrow())[(*j.borrow()) as usize]) {
                    return ({
                        ...
                        operator_add_98(_a, (((*j.borrow()) - (*i.borrow())) as i64))
                    });
                }
        ...
        if ((*base.borrow()) == (*k_weekdays_back.borrow())[(*i.borrow()) as usize]) {
            ...
            'loop_: while true {
                if ((*wd.borrow()) == (*k_weekdays_back.borrow())[(*j.borrow()) as usize]) {
                    return ({
                        let _a: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_   = ((*cd.borrow()) ).clone()  ;
                        operator_sub_99(_a, (((*j.borrow()) - (*i.borrow())) as i64))
                    });
                }
                (*j.borrow_mut()).prefix_inc();
            }
...
pub fn GetYearDay_107(
    ...
    return ({
        let _cs : Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_ > = Rc::new(RefCell::new(absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_5 ( {  cs .as_pointer()   } , None , ) ));
        get_yearday_103(_cs.as_pointer())
    });
}
pub fn operator_eq_108(
...
pub fn operator_ne_109(
    ...
    return !({
        ...
        operator_eq_108(_lhs, (*rhs.borrow()).clone())
    });
}
#[derive(Default)]
...
}pub  fn split_seconds_111 ( tp : Ptr< std_chrono_time_point_std_chrono_system_clock__std_chrono_duration_long_long__std_ratio________  > , ) -> (Value<std_chrono_time_point_std_chrono_steady_clock__std_chrono_duration_long_long__std_ratio________>, Value<std_chrono_duration_long_long__std_ratio_______>){
    return (
        Rc::new(RefCell::new(
            (*tp.upgrade().deref())
                .clone()
...
thread_local!(
    pub static kTicksPerNanosecond_113: Value<i64> = Rc::new(RefCell::new(4));
);
thread_local!(
    pub static kTicksPerSecond_114: Value<i64> = Rc::new(RefCell::new(4000000000));
);
pub fn FromInt64_115(v: i64, _a1: std_ratio______) -> absl_Duration {
    ...
    return ({
        ...
        let _ticks: i64 = (((((((*v.borrow()) % 1000000000_i64) * 4) * 1000_i64) * 1000_i64)
            * 1000_i64)
            / 1000000000_i64);
        MakeNormalizedDuration_116(_sec, _ticks)
    });
}
...
pub fn FromInt64_118(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({
...
pub fn FromInt64_119(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({
...
pub fn operator_gt_122(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    ...
    return ({
        ...
        operator_lt_123(_lhs, (*lhs.borrow()).clone())
    });
}
pub fn operator_ge_124(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*lhs.borrow()).clone();
        operator_lt_123(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_le_125(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*rhs.borrow()).clone();
...
pub fn operator_ne_126(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*lhs.borrow()).clone();
        operator_eq_121(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_add_127(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn operator_sub_128(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn operator_div_129(lhs: absl_Duration, rhs: absl_Duration) -> i64 {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return ({
        let _num: absl_Duration = (*lhs.borrow()).clone();
        let _rem: Ptr<absl_Duration> = (lhs.as_pointer());
        IDivDuration_130(_num, (*rhs.borrow()).clone(), _rem)
    });
}
pub fn operator_rem_131(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn ZeroDuration_132() -> absl_Duration {
    return absl_Duration::absl_Duration1();
}
pub fn AbsDuration_133(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if ({
        let _lhs: absl_Duration = (*d.borrow()).clone();
        operator_lt_123(_lhs, ({ ZeroDuration_132() }))
    }) {
        ({
...
pub fn Nanoseconds_135(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_115(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Nanoseconds_136(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_115((*n.borrow()), std_ratio______ {}) });
}
pub fn Microseconds_137(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_117(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Microseconds_138(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_117((*n.borrow()), std_ratio______ {}) });
}
pub fn Milliseconds_139(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_118(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Milliseconds_140(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_118((*n.borrow()), std_ratio______ {}) });
}
pub fn Seconds_141(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_119((*n.borrow()), std_ratio______ {}) });
}
pub fn Seconds_142(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_119((*n.borrow()), std_ratio______ {}) });
}
pub fn Minutes_143(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_144(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Hours_145(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_146(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn operator_shl_147(os: Ptr<std::fs::File>, d: absl_Duration) -> Ptr<std::fs::File> {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return os.write_all(
        &([(&({ FormatDuration_148((*d.borrow()).clone()) })
            .iter()
            .take(({ FormatDuration_148((*d.borrow()).clone()) }).len() - 1)
            .map(|&c| c as u8)
...
pub fn operator_cmp_149(lhs: absl_Time, rhs: absl_Time) -> std::cmp::Ordering {
    ...
    return ({
        ...
        operator_cmp_120(_lhs, _rhs)
    });
}
pub fn operator_lt_151(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_lt_123(_lhs, _rhs)
    });
}
pub fn operator_gt_152(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Time = (*rhs.borrow()).clone();
        operator_lt_151(_lhs, (*lhs.borrow()).clone())
    });
}
pub fn operator_ge_153(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*lhs.borrow()).clone();
        operator_lt_151(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_le_154(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*rhs.borrow()).clone();
...
pub fn operator_eq_150(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_eq_121(_lhs, _rhs)
    });
}
pub fn operator_ne_155(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*lhs.borrow()).clone();
        operator_eq_150(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_add_156(lhs: absl_Time, rhs: absl_Duration) -> absl_Time {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
...
pub fn operator_add_157(lhs: absl_Duration, rhs: absl_Time) -> absl_Time {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
...
pub fn operator_sub_158(lhs: absl_Time, rhs: absl_Duration) -> absl_Time {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
...
pub fn operator_sub_159(lhs: absl_Time, rhs: absl_Time) -> absl_Duration {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_sub_128(_lhs, _rhs)
    });
}
pub fn UnixEpoch_160() -> absl_Time {
    return <absl_Time>::default();
}
pub fn UniversalEpoch_161() -> absl_Time {
    return absl_Time::absl_Time1({
        ({ MakeDuration_162((((-24_i32 * 719162) as i64) * 3600_i64), 0_u32) })
    });
}
pub fn InfiniteFuture_163() -> absl_Time {
    return absl_Time::absl_Time1({ ({ MakeDuration_162(<i64>::MAX, !0_u32) }) });
}
pub fn InfinitePast_164() -> absl_Time {
    return absl_Time::absl_Time1({ ({ MakeDuration_162(<i64>::MIN, !0_u32) }) });
}
pub fn operator_eq_165(a: absl_TimeZone, b: absl_TimeZone) -> bool {
    ...
    return ({
        ...
        operator_eq_108(_lhs, _rhs)
    });
}
pub fn operator_ne_166(a: absl_TimeZone, b: absl_TimeZone) -> bool {
    let a: Value<absl_TimeZone> = Rc::new(RefCell::new(a));
    let b: Value<absl_TimeZone> = Rc::new(RefCell::new(b));
    return ({
        let _lhs: absl_time_internal_cctz_time_zone = (*(*a.borrow()).cz_.borrow()).clone();
        let _rhs: absl_time_internal_cctz_time_zone = (*(*b.borrow()).cz_.borrow()).clone();
        operator_ne_109(_lhs, _rhs)
    });
}
pub fn operator_shl_167(os: Ptr<std::fs::File>, tz: absl_TimeZone) -> Ptr<std::fs::File> {
...
pub fn LoadTimeZone_168(name: Vec<u8>, tz: Ptr<absl_TimeZone>) -> bool {
    ...
    if (*name.borrow()).clone()
        == Ptr::from_string_literal(b"localtime")
            ...
            .collect::<Vec<u8>>()
    {
        let __rhs = absl_TimeZone::absl_TimeZone1({ ({ local_time_zone_169() }) });
    ...
    let b: Value<bool> = Rc::new(RefCell::new(
        ({
            ...
            load_time_zone_170(_name.as_pointer(), (cz.as_pointer()))
        }),
    ));
...
pub fn FixedTimeZone_171(seconds: i32) -> absl_TimeZone {
    ...
    return absl_TimeZone::absl_TimeZone1({
        ({
            ...
            fixed_time_zone_172(_offset.as_pointer())
        })
    });
}
pub fn UTCTimeZone_173() -> absl_TimeZone {
    return absl_TimeZone::absl_TimeZone1({ ({ utc_time_zone_174() }) });
}
pub fn LocalTimeZone_175() -> absl_TimeZone {
    return absl_TimeZone::absl_TimeZone1({ ({ local_time_zone_169() }) });
}
pub fn ToCivilSecond_176(
    t: absl_Time,
    tz: absl_TimeZone,
) -> absl_time_internal_cctz_detail_civil_time_absl_time_internal_second_tag_ {
...
thread_local!();
thread_local!();
pub fn operator_shl_189(os: Ptr<std::fs::File>, t: absl_Time) -> Ptr<std::fs::File> {
    let t: Value<absl_Time> = Rc::new(RefCell::new(t));
    return os.write_all(
        &([(&({ FormatTime_190((*t.borrow()).clone()) })
            .iter()
            .take(({ FormatTime_190((*t.borrow()).clone()) }).len() - 1)
            .map(|&c| c as u8)
            .collect::<Vec<u8>>()[..] as &[u8])]
        .concat()),
    );
}
pub fn MakeDuration_162(hi: i64, lo: Option<u32>) -> absl_Duration {
    let hi: Value<i64> = Rc::new(RefCell::new(hi));
    let lo: Value<u32> = Rc::new(RefCell::new(lo.unwrap_or(0_u32)));
    return absl_Duration::absl_Duration2({ (*hi.borrow()) }, { (*lo.borrow()) });
}
pub fn MakeDuration_191(hi: i64, lo: i64) -> absl_Duration {
    let hi: Value<i64> = Rc::new(RefCell::new(hi));
    let lo: Value<i64> = Rc::new(RefCell::new(lo));
    return ({ MakeDuration_162((*hi.borrow()), Some(((*lo.borrow()) as u32))) });
}
pub fn MakePosDoubleDuration_192(n: f64) -> absl_Duration {
    ...
    return if (((*ticks.borrow()) as i64) < 4000000000) {
        ({ MakeDuration_162((*int_secs.borrow()), Some((*ticks.borrow()))) })
    } else {
        ({
            MakeDuration_191(
...
pub fn MakeNormalizedDuration_116(sec: i64, ticks: i64) -> absl_Duration {
    let sec: Value<i64> = Rc::new(RefCell::new(sec));
    let ticks: Value<i64> = Rc::new(RefCell::new(ticks));
    return if ((*ticks.borrow()) < 0_i64) {
        ({ MakeDuration_191(((*sec.borrow()) - 1_i64), ((*ticks.borrow()) + 4000000000)) })
    } else {
        ({ MakeDuration_191((*sec.borrow()), (*ticks.borrow())) })
    };
}
pub fn GetRepHi_193(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return ({ absl_Duration_HiRepImpl::Get(&(*d.borrow()).rep_hi_.as_pointer()) });
}
pub fn GetRepLo_194(d: absl_Duration) -> u32 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return (*(*d.borrow()).rep_lo_.borrow());
}
pub fn IsInfiniteDuration_195(d: absl_Duration) -> bool {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return (({ GetRepLo_194((*d.borrow()).clone()) }) == !0_u32);
}
pub fn OppositeInfinity_196(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if (({ GetRepHi_193((*d.borrow()).clone()) }) < 0_i64) {
        ({ MakeDuration_162(<i64>::MAX, Some(!0_u32)) })
    } else {
        ({ MakeDuration_162(<i64>::MIN, Some(!0_u32)) })
    };
}
pub fn NegateAndSubtractOne_197(n: i64) -> i64 {
    let n: Value<i64> = Rc::new(RefCell::new(n));
...
pub fn FromUnixDuration_198(d: absl_Duration) -> absl_Time {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return absl_Time::absl_Time1({ (*d.borrow()).clone() });
}
pub fn ToUnixDuration_199(t: absl_Time) -> absl_Duration {
    let t: Value<absl_Time> = Rc::new(RefCell::new(t));
...
pub fn FromInt64_144(v: i64, _a1: std_ratio______) -> absl_Duration {
    ...
    {
        ({ MakeDuration_162(((*v.borrow()) * 60_i64), None) })
    } else {
        if ((*v.borrow()) > 0_i64) {
            ({ InfiniteDuration_200() })
        } else {
            ({
                let _d: absl_Duration = ({ InfiniteDuration_200() });
                operator_neg_134(_d)
            })
        }
    };
}
pub fn FromInt64_146(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return if (((*v.borrow()) <= (<i64>::MAX / 3600_i64))
        && ((*v.borrow()) >= (<i64>::MIN / 3600_i64)))
    {
        ({ MakeDuration_162(((*v.borrow()) * 3600_i64), None) })
    } else {
        if ((*v.borrow()) > 0_i64) {
            ({ InfiniteDuration_200() })
        } else {
...
pub fn IsValidRep64_201(_a0: i32) -> bool {
    let _a0: Value<i32> = Rc::new(RefCell::new(_a0));
    return true;
}
pub fn IsValidRep64_202(_a0: i32) -> bool {
    let _a0: Value<i32> = Rc::new(RefCell::new(_a0));
    return true;
}
...
pub fn FromChrono_205(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_118(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_206(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_119(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_207(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_144(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_208(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_146(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn ToInt64_209(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Nanoseconds_210((*d.borrow()).clone()) });
}
pub fn ToInt64_211(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Microseconds_212((*d.borrow()).clone()) });
}
pub fn ToInt64_213(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Milliseconds_214((*d.borrow()).clone()) });
}
pub fn ToInt64_215(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Seconds_216((*d.borrow()).clone()) });
}
pub fn ToInt64_217(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Minutes_218((*d.borrow()).clone()) });
}
pub fn ToInt64_219(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Hours_220((*d.borrow()).clone()) });
}
pub fn operator_lt_123(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return if (({ GetRepHi_193((*lhs.borrow()).clone()) })
        != ({ GetRepHi_193((*rhs.borrow()).clone()) }))
    {
        (({ GetRepHi_193((*lhs.borrow()).clone()) }) < ({ GetRepHi_193((*rhs.borrow()).clone()) }))
    } else {
        if (({ GetRepHi_193((*lhs.borrow()).clone()) }) == <i64>::MIN) {
            (({ GetRepLo_194((*lhs.borrow()).clone()) }).wrapping_add(1_u32)
                < ({ GetRepLo_194((*rhs.borrow()).clone()) }).wrapping_add(1_u32))
        } else {
            (({ GetRepLo_194((*lhs.borrow()).clone()) })
                < ({ GetRepLo_194((*rhs.borrow()).clone()) }))
        }
    };
}
pub fn operator_cmp_120(lhs: absl_Duration, rhs: absl_Duration) -> std::cmp::Ordering {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    let lhs_hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_193((*lhs.borrow()).clone()) })));
    let rhs_hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_193((*rhs.borrow()).clone()) })));
    {
    ...
    let lhs_lo: Value<u32> = Rc::new(RefCell::new(({ GetRepLo_194((*lhs.borrow()).clone()) })));
    let rhs_lo: Value<u32> = Rc::new(RefCell::new(({ GetRepLo_194((*rhs.borrow()).clone()) })));
...
pub fn operator_eq_121(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (({ GetRepHi_193((*lhs.borrow()).clone()) })
        == ({ GetRepHi_193((*rhs.borrow()).clone()) }))
        && (({ GetRepLo_194((*lhs.borrow()).clone()) })
            == ({ GetRepLo_194((*rhs.borrow()).clone()) }));
}
pub fn operator_neg_134(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if (({ GetRepLo_194((*d.borrow()).clone()) }) == 0_u32) {
        if (({ GetRepHi_193((*d.borrow()).clone()) }) == <i64>::MIN) {
            ({ InfiniteDuration_200() })
        } else {
            ({ MakeDuration_162(-({ GetRepHi_193((*d.borrow()).clone()) }), None) })
        }
    } else {
        if ({ IsInfiniteDuration_195((*d.borrow()).clone()) }) {
            ({ OppositeInfinity_196((*d.borrow()).clone()) })
        } else {
            ({
                let _hi: i64 =
                    ({ NegateAndSubtractOne_197(({ GetRepHi_193((*d.borrow()).clone()) })) });
                let _lo: i64 = (4000000000 - (({ GetRepLo_194((*d.borrow()).clone()) }) as i64));
                MakeDuration_191(_hi, _lo)
            })
        }
    };
}
pub fn InfiniteDuration_200() -> absl_Duration {
    return ({ MakeDuration_162(<i64>::MAX, Some(!0_u32)) });
}
...
pub fn FromChrono_223(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_205((d).clone()) });
}
pub fn FromChrono_224(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_206((d).clone()) });
}
pub fn FromChrono_225(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_207((d).clone()) });
}
pub fn FromChrono_226(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_208((d).clone()) });
}
pub fn FromUnixNanos_227(ns: i64) -> absl_Time {
    let ns: Value<i64> = Rc::new(RefCell::new(ns));
    return ({ FromUnixDuration_198(({ Nanoseconds_136((*ns.borrow())) })) });
}
pub fn FromUnixMicros_228(us: i64) -> absl_Time {
    let us: Value<i64> = Rc::new(RefCell::new(us));
    return ({ FromUnixDuration_198(({ Microseconds_138((*us.borrow())) })) });
}
pub fn FromUnixMillis_229(ms: i64) -> absl_Time {
    let ms: Value<i64> = Rc::new(RefCell::new(ms));
    return ({ FromUnixDuration_198(({ Milliseconds_140((*ms.borrow())) })) });
}
pub fn FromUnixSeconds_230(s: i64) -> absl_Time {
    let s: Value<i64> = Rc::new(RefCell::new(s));
    return ({ FromUnixDuration_198(({ Seconds_141((*s.borrow())) })) });
}
pub fn FromTimeT_231(t: i64) -> absl_Time {
    let t: Value<i64> = Rc::new(RefCell::new(t));
    return ({ FromUnixDuration_198(({ Seconds_142((*t.borrow())) })) });
}
pub fn ToInt64Nanoseconds_210(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_193((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_193((*d.borrow()).clone()) }) >> 33) == 0_i64)
    {
        return ((((({ GetRepHi_193((*d.borrow()).clone()) }) * 1000_i64) * 1000_i64) * 1000_i64)
            + ((({ GetRepLo_194((*d.borrow()).clone()) }) as i64) / 4));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_129(_lhs, ({ Nanoseconds_135(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Microseconds_212(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_193((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_193((*d.borrow()).clone()) }) >> 43) == 0_i64)
    {
        return (((({ GetRepHi_193((*d.borrow()).clone()) }) * 1000_i64) * 1000_i64)
            + ((({ GetRepLo_194((*d.borrow()).clone()) }) as i64) / (4 * 1000_i64)));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_129(_lhs, ({ Microseconds_137(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Milliseconds_214(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_193((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_193((*d.borrow()).clone()) }) >> 53) == 0_i64)
    {
        return ((({ GetRepHi_193((*d.borrow()).clone()) }) * 1000_i64)
            + ((({ GetRepLo_194((*d.borrow()).clone()) }) as i64) / ((4 * 1000_i64) * 1000_i64)));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_129(_lhs, ({ Milliseconds_139(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Seconds_216(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_193((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_195((*d.borrow()).clone()) }) {
        return (*hi.borrow());
    }
    if ((*hi.borrow()) < 0_i64) && (({ GetRepLo_194((*d.borrow()).clone()) }) != 0_u32) {
        (*hi.borrow_mut()).prefix_inc();
    }
    return (*hi.borrow());
}
pub fn ToInt64Minutes_218(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_193((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_195((*d.borrow()).clone()) }) {
        return (*hi.borrow());
...
pub fn ToInt64Hours_220(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_193((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_195((*d.borrow()).clone()) }) {
        return (*hi.borrow());
...
pub fn SleepFor_232(duration: absl_Duration) {
    let duration: Value<absl_Duration> = Rc::new(RefCell::new(duration));
    ({ AbslInternalSleepFor_233((*duration.borrow()).clone()) });
}
thread_local!(
    static kNoTimeout_234: Value<u64> = Rc::new(RefCell::new(18446744073709551615));
);
thread_local!(
    static kMaxNanos_235: Value<i64> = Rc::new(RefCell::new(9223372036854775807));
);
#[derive()]
...
pub fn GetOrCreateCurrentThreadIdentity_236() -> Ptr<absl_base_internal_ThreadIdentity> {
    let identity: Value<Ptr<absl_base_internal_ThreadIdentity>> =
        Rc::new(RefCell::new(({ CurrentThreadIdentityIfPresent_237() })));
    if ((((false) || ((*identity.borrow()).is_null())) as i64) != 0) {
        return ({ CreateThreadIdentity_238() });
    }
...
impl absl_synchronization_internal_PerThreadSem {
    fn Init(identity: Ptr<absl_base_internal_ThreadIdentity>) {
        ...
        ({ AbslInternalPerThreadSemInit_239((*identity.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {
    fn Post(identity: Ptr<absl_base_internal_ThreadIdentity>) {
        let identity: Value<Ptr<absl_base_internal_ThreadIdentity>> =
            Rc::new(RefCell::new(identity));
        ({ AbslInternalPerThreadSemPost_240((*identity.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {
    fn Wait(t: absl_synchronization_internal_KernelTimeout) -> bool {
        ...
        return ({ AbslInternalPerThreadSemWait_241((*t.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {

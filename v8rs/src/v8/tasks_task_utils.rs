...
thread_local!(
    pub static kLowZeroBits_31: Value<i32> = Rc::new(RefCell::new(8));
);
thread_local!(
    pub static kAlignment_32: Value<i32> = Rc::new(RefCell::new(256));
);
...
thread_local!(
    pub static kToBePaddedSize_33: Value<usize> = Rc::new(RefCell::new(33));
);
#[derive(Default)]
...
pub fn is_constant_evaluated_36() -> bool {
    return ({ is_constant_evaluated_37() });
}
...
pub fn LogSeverities_38() -> Vec<absl_LogSeverity> {
    return vec![
...
thread_local!(
    pub static kLogDebugFatal_39: Value<absl_LogSeverity> = Rc::new(RefCell::new(2));
);
pub fn LogSeverityName_40(s: absl_LogSeverity) -> Ptr<u8> {
...
pub fn NormalizeLogSeverity_41(s: absl_LogSeverity) -> absl_LogSeverity {
    let s: Value<absl_LogSeverity> = Rc::new(RefCell::new(s));
...
pub fn NormalizeLogSeverity_42(s: i32) -> absl_LogSeverity {
    let s: Value<i32> = Rc::new(RefCell::new(s));
    return ({ NormalizeLogSeverity_41(((*s.borrow()) as absl_LogSeverity)) });
}
...
pub fn operator_gt_43(lhs: absl_LogSeverityAtLeast, rhs: absl_LogSeverity) -> bool {
...
pub fn operator_lt_44(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtLeast) -> bool {
...
pub fn operator_le_45(lhs: absl_LogSeverityAtLeast, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtLeast> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) <= (*rhs.borrow()));
}
pub fn operator_ge_46(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtLeast) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtLeast> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) >= ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn operator_lt_47(lhs: absl_LogSeverityAtMost, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) < (*rhs.borrow()));
}
pub fn operator_gt_48(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtMost) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) > ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn operator_ge_49(lhs: absl_LogSeverityAtMost, rhs: absl_LogSeverity) -> bool {
    let lhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(rhs));
    return (((*lhs.borrow()) as absl_LogSeverity) >= (*rhs.borrow()));
}
pub fn operator_le_50(lhs: absl_LogSeverity, rhs: absl_LogSeverityAtMost) -> bool {
    let lhs: Value<absl_LogSeverity> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_LogSeverityAtMost> = Rc::new(RefCell::new(rhs));
    return ((*lhs.borrow()) <= ((*rhs.borrow()) as absl_LogSeverity));
}
pub fn Basename_51(fname: Ptr<u8>, offset: i32) -> Ptr<u8> {
            ...
            == (('\\' as u8) as i32))
    {
        ...
        ({ Basename_51((*fname.borrow()).clone(), ((*offset.borrow()) - 1)) })
    };
}
thread_local!();
pub fn ClippedSubstr_53(s: Vec<u8>, pos: usize, n: Option<usize>) -> Vec<u8> {
...
pub fn NullSafeStringView_54(p: Ptr<u8>) -> Vec<u8> {
...
pub fn days_per_century_57(yi: i32) -> i32 {
    let yi: Value<i32> = Rc::new(RefCell::new(yi));
    return (36524 + ((((*yi.borrow()) == 0) || ((*yi.borrow()) > 300)) as i32));
}
pub fn days_per_4years_58(yi: i32) -> i32 {
    let yi: Value<i32> = Rc::new(RefCell::new(yi));
    return (1460
        + (((((*yi.borrow()) == 0) || ((*yi.borrow()) > 300))
            || ((((*yi.borrow()) - 1) % 100) < 96)) as i32));
}
pub fn days_per_year_59(y: i64, m: i8) -> i32 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
    return if ({ is_leap_year_55(((*y.borrow()) + ((((*m.borrow()) as i32) > 2) as i64))) }) {
        366
    } else {
        365
    };
}
pub fn days_per_month_60(y: i64, m: i8) -> i32 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
    ...
    return ((*k_days_per_month.borrow())[(*m.borrow()) as usize]
        + (((((*m.borrow()) as i32) == 2) && ({ is_leap_year_55((*y.borrow())) })) as i32));
}
pub fn n_day_61(
    y: i64,
    m: i8,
    ...
    if ((*d.borrow()) > 0_i64) {
        ...
        if ((*d.borrow()) > (-365_i32 as i64)) {
            (*ey.borrow_mut()) -= 1_i64;
            (*d.borrow_mut()) += (({ days_per_year_59((*ey.borrow()), (*m.borrow())) }) as i64);
        } else {
            (*ey.borrow_mut()) -= 400_i64;
            (*d.borrow_mut()) += 146097_i64;
        }
    ...
    if ((*d.borrow()) > 365_i64) {
        let yi: Value<i32> = Rc::new(RefCell::new(
            ({ year_index_56((*ey.borrow()), (*m.borrow())) }),
        ));
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(({ days_per_century_57((*yi.borrow())) })));
        ...
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(({ days_per_4years_58((*yi.borrow())) })));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
            (*d.borrow_mut()) -= ((*n.borrow()) as i64);
        ...
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(
                ({ days_per_year_59((*ey.borrow()), (*m.borrow())) }),
            ));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
    ...
    if ((*d.borrow()) > 28_i64) {
        'loop_: while true {
            let n: Value<i32> = Rc::new(RefCell::new(
                ({ days_per_month_60((*ey.borrow()), (*m.borrow())) }),
            ));
            if ((*d.borrow()) <= ((*n.borrow()) as i64)) {
                break;
            }
...
pub fn step_67(
    ...
    return ({
        ...
        n_min_64(_y, _m, _d, _hh, 0_i64, _mm, _ss)
    });
}
pub fn step_68(
    ...
    return ({
        ...
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_hour_63(_y, _m, _d, 0_i64, _hh, _mm, _ss)
    });
}
pub fn step_69(
    ...
    return ({
        ...
        let _mm: i8 = (*(*f.borrow()).mm.borrow());
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_day_61(_y, _m, _d, (*n.borrow()), _hh, _mm, _ss)
    });
}
pub fn step_70(
    ...
    return ({
        ...
        let _mm: i8 = (*(*f.borrow()).mm.borrow());
        let _ss: i8 = (*(*f.borrow()).ss.borrow());
        n_mon_62(_y, _m, _d, 0_i64, _hh, _mm, _ss)
    });
}
pub fn step_71(
...
pub fn scale_add_72(v: i64, f: i64, a: i64) -> i64 {
...
pub fn ymd_ord_73(y: i64, m: i8, d: i8) -> i64 {
    let y: Value<i64> = Rc::new(RefCell::new(y));
    let m: Value<i8> = Rc::new(RefCell::new(m));
...
pub fn day_difference_74(y1: i64, m1: i8, d1: i8, y2: i64, m2: i8, d2: i8) -> i64 {
    ...
    let delta: Value<i64> = Rc::new(RefCell::new(
        (({ ymd_ord_73((*a_c4_off.borrow()), (*m1.borrow()), (*d1.borrow())) })
            - ({ ymd_ord_73((*b_c4_off.borrow()), (*m2.borrow()), (*d2.borrow())) })),
    ));
...
pub fn difference_76(
    ...
    return ({
        ...
        scale_add_72(_v, 12_i64, _a)
    });
}
pub fn difference_77(
    _a0: absl_time_internal_cctz_detail_day_tag,
    f1: absl_time_internal_cctz_detail_fields,
    f2: absl_time_internal_cctz_detail_fields,
) -> i64 {
    ...
    return ({
        ...
        day_difference_74(_y1, _m1, _d1, _y2, _m2, _d2)
    });
}
...
pub fn operator_add_87(
    ...
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_69 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , (*n.borrow())  , ) } )    } , )   ;
}
pub fn operator_sub_88(
    a: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_,
    n: i64,
) -> absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ {
    let a: Value<
    ...
    return if ((*n.borrow()) != <i64>::MIN) {
        absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_69 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , - (*n.borrow())  , ) } )    } , )
    } else {
        absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_2 ( {  (  { step_69 ( absl_time_internal_cctz_detail_day_tag { }  , (  { step_69 ( absl_time_internal_cctz_detail_day_tag { }  , ((*(*a.borrow()) . f_ .borrow()) ).clone()  , - ( ( (*n.borrow()) + 1_i64 ) )  , ) } )   , 1_i64  , ) } )    } , )
    };
}
#[derive()]
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_10(
        ...
        > = Rc::new(RefCell::new(Self {
            f_: Rc::new(RefCell::new(
                ({ align_82(absl_time_internal_minute_tag {}, (*f.borrow()).clone()) }),
            )),
        }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_minute_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_13(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_83(absl_time_internal_hour_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_hour_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_17(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_84(absl_time_internal_day_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_day_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_20(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_85(absl_time_internal_month_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_month_tag_> =
            __this.as_pointer();
...
impl absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_ {
    fn absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_23(
        ...
        let __this: Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_> =
            Rc::new(RefCell::new(Self {
                f_: Rc::new(RefCell::new(
                    ({ align_86(absl_time_internal_year_tag {}, (*f.borrow()).clone()) }),
                )),
            }));
        let this: Ptr<absl_time_internal_cctz_detail_civil_time_absl_time_internal_year_tag_> =
            __this.as_pointer();
            ...
            'loop_: while true {
                if ((*wd.borrow()) == (*k_weekdays_forw.borrow())[(*j.borrow()) as usize]) {
                    return ({
                        ...
                        operator_add_87(_a, (((*j.borrow()) - (*i.borrow())) as i64))
                    });
                }
        ...
        if ((*base.borrow()) == (*k_weekdays_back.borrow())[(*i.borrow()) as usize]) {
            ...
            'loop_: while true {
                if ((*wd.borrow()) == (*k_weekdays_back.borrow())[(*j.borrow()) as usize]) {
                    return ({
                        let _a: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_day_tag_   = ((*cd.borrow()) ).clone()  ;
                        operator_sub_88(_a, (((*j.borrow()) - (*i.borrow())) as i64))
                    });
                }
                (*j.borrow_mut()).prefix_inc();
            }
...
pub fn GetYearDay_96(
    ...
    return ({
        let _cs : Value<absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_ > = Rc::new(RefCell::new(absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_ :: absl_time_internal_cctz_detail_civil_time_absl_time_internal_cctz_detail_second_tag_5 ( {  cs .as_pointer()   } , None , ) ));
        get_yearday_92(_cs.as_pointer())
    });
}
pub fn operator_eq_97(
...
pub fn operator_ne_98(
    ...
    return !({
        ...
        operator_eq_97(_lhs, (*rhs.borrow()).clone())
    });
}
#[derive(Default)]
...
}pub  fn split_seconds_100 ( tp : Ptr< std_chrono_time_point_std_chrono_system_clock__std_chrono_duration_long_long__std_ratio________  > , ) -> (Value<std_chrono_time_point_std_chrono_steady_clock__std_chrono_duration_long_long__std_ratio________>, Value<std_chrono_duration_long_long__std_ratio_______>){
    return (
        Rc::new(RefCell::new(
            (*tp.upgrade().deref())
                .clone()
...
thread_local!(
    pub static kTicksPerNanosecond_102: Value<i64> = Rc::new(RefCell::new(4));
);
thread_local!(
    pub static kTicksPerSecond_103: Value<i64> = Rc::new(RefCell::new(4000000000));
);
pub fn FromInt64_104(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    ...
    return ({
        ...
        let _ticks: i64 = (((((((*v.borrow()) % 1000000000_i64) * 4) * 1000_i64) * 1000_i64)
            * 1000_i64)
            / 1000000000_i64);
        MakeNormalizedDuration_105(_sec, _ticks)
    });
}
pub fn FromInt64_106(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({
...
pub fn FromInt64_108(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({
...
pub fn operator_gt_111(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    ...
    return ({
        ...
        operator_lt_112(_lhs, (*lhs.borrow()).clone())
    });
}
pub fn operator_ge_113(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*lhs.borrow()).clone();
        operator_lt_112(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_le_114(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*rhs.borrow()).clone();
...
pub fn operator_ne_115(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Duration = (*lhs.borrow()).clone();
        operator_eq_110(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_add_116(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn operator_sub_117(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn operator_div_118(lhs: absl_Duration, rhs: absl_Duration) -> i64 {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return ({
        let _num: absl_Duration = (*lhs.borrow()).clone();
        let _rem: Ptr<absl_Duration> = (lhs.as_pointer());
        IDivDuration_119(_num, (*rhs.borrow()).clone(), _rem)
    });
}
pub fn operator_rem_120(lhs: absl_Duration, rhs: absl_Duration) -> absl_Duration {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (*({
...
pub fn ZeroDuration_121() -> absl_Duration {
    return absl_Duration::absl_Duration1();
}
pub fn AbsDuration_122(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if ({
        let _lhs: absl_Duration = (*d.borrow()).clone();
        operator_lt_112(_lhs, ({ ZeroDuration_121() }))
    }) {
        ({
...
pub fn Nanoseconds_124(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_104(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Nanoseconds_125(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_104((*n.borrow()), std_ratio______ {}) });
}
pub fn Microseconds_126(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_106(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Microseconds_127(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_106((*n.borrow()), std_ratio______ {}) });
}
pub fn Milliseconds_128(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_107(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Milliseconds_129(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_107((*n.borrow()), std_ratio______ {}) });
}
pub fn Seconds_130(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_108((*n.borrow()), std_ratio______ {}) });
}
pub fn Seconds_131(n: i64) -> absl_Duration {
    let n: Value<i64> = Rc::new(RefCell::new(n));
    return ({ FromInt64_108((*n.borrow()), std_ratio______ {}) });
}
pub fn Minutes_132(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_133(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn Hours_134(n: i32) -> absl_Duration {
    let n: Value<i32> = Rc::new(RefCell::new(n));
    return ({ FromInt64_135(((*n.borrow()) as i64), std_ratio______ {}) });
}
pub fn operator_shl_136(os: Ptr<std::fs::File>, d: absl_Duration) -> Ptr<std::fs::File> {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return os.write_all(
        &([(&({ FormatDuration_137((*d.borrow()).clone()) })
            .iter()
            .take(({ FormatDuration_137((*d.borrow()).clone()) }).len() - 1)
            .map(|&c| c as u8)
...
pub fn operator_cmp_138(lhs: absl_Time, rhs: absl_Time) -> std::cmp::Ordering {
    ...
    return ({
        ...
        operator_cmp_109(_lhs, _rhs)
    });
}
pub fn operator_lt_140(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_lt_112(_lhs, _rhs)
    });
}
pub fn operator_gt_141(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Time = (*rhs.borrow()).clone();
        operator_lt_140(_lhs, (*lhs.borrow()).clone())
    });
}
pub fn operator_ge_142(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*lhs.borrow()).clone();
        operator_lt_140(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_le_143(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*rhs.borrow()).clone();
...
pub fn operator_eq_139(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_eq_110(_lhs, _rhs)
    });
}
pub fn operator_ne_144(lhs: absl_Time, rhs: absl_Time) -> bool {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return !({
        let _lhs: absl_Time = (*lhs.borrow()).clone();
        operator_eq_139(_lhs, (*rhs.borrow()).clone())
    });
}
pub fn operator_add_145(lhs: absl_Time, rhs: absl_Duration) -> absl_Time {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
...
pub fn operator_add_146(lhs: absl_Duration, rhs: absl_Time) -> absl_Time {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
...
pub fn operator_sub_147(lhs: absl_Time, rhs: absl_Duration) -> absl_Time {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
...
pub fn operator_sub_148(lhs: absl_Time, rhs: absl_Time) -> absl_Duration {
    let lhs: Value<absl_Time> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Time> = Rc::new(RefCell::new(rhs));
    return ({
        let _lhs: absl_Duration = (*(*lhs.borrow()).rep_.borrow()).clone();
        let _rhs: absl_Duration = (*(*rhs.borrow()).rep_.borrow()).clone();
        operator_sub_117(_lhs, _rhs)
    });
}
pub fn UnixEpoch_149() -> absl_Time {
    return <absl_Time>::default();
}
pub fn UniversalEpoch_150() -> absl_Time {
    return absl_Time::absl_Time1({
        ({ MakeDuration_151((((-24_i32 * 719162) as i64) * 3600_i64), 0_u32) })
    });
}
pub fn InfiniteFuture_152() -> absl_Time {
    return absl_Time::absl_Time1({ ({ MakeDuration_151(<i64>::MAX, !0_u32) }) });
}
pub fn InfinitePast_153() -> absl_Time {
    return absl_Time::absl_Time1({ ({ MakeDuration_151(<i64>::MIN, !0_u32) }) });
}
pub fn operator_eq_154(a: absl_TimeZone, b: absl_TimeZone) -> bool {
    ...
    return ({
        ...
        operator_eq_97(_lhs, _rhs)
    });
}
pub fn operator_ne_155(a: absl_TimeZone, b: absl_TimeZone) -> bool {
    let a: Value<absl_TimeZone> = Rc::new(RefCell::new(a));
    let b: Value<absl_TimeZone> = Rc::new(RefCell::new(b));
    return ({
        let _lhs: absl_time_internal_cctz_time_zone = (*(*a.borrow()).cz_.borrow()).clone();
        let _rhs: absl_time_internal_cctz_time_zone = (*(*b.borrow()).cz_.borrow()).clone();
        operator_ne_98(_lhs, _rhs)
    });
}
pub fn operator_shl_156(os: Ptr<std::fs::File>, tz: absl_TimeZone) -> Ptr<std::fs::File> {
...
pub fn LoadTimeZone_157(name: Vec<u8>, tz: Ptr<absl_TimeZone>) -> bool {
    ...
    if (*name.borrow()).clone()
        == Ptr::from_string_literal(b"localtime")
            ...
            .collect::<Vec<u8>>()
    {
        let __rhs = absl_TimeZone::absl_TimeZone1({ ({ local_time_zone_158() }) });
    ...
    let b: Value<bool> = Rc::new(RefCell::new(
        ({
            ...
            load_time_zone_159(_name.as_pointer(), (cz.as_pointer()))
        }),
    ));
...
pub fn FixedTimeZone_160(seconds: i32) -> absl_TimeZone {
    ...
    return absl_TimeZone::absl_TimeZone1({
        ({
            ...
            fixed_time_zone_161(_offset.as_pointer())
        })
    });
}
pub fn UTCTimeZone_162() -> absl_TimeZone {
    return absl_TimeZone::absl_TimeZone1({ ({ utc_time_zone_163() }) });
}
pub fn LocalTimeZone_164() -> absl_TimeZone {
    return absl_TimeZone::absl_TimeZone1({ ({ local_time_zone_158() }) });
}
pub fn ToCivilSecond_165(
    t: absl_Time,
    tz: absl_TimeZone,
) -> absl_time_internal_cctz_detail_civil_time_absl_time_internal_second_tag_ {
...
thread_local!();
thread_local!();
pub fn operator_shl_178(os: Ptr<std::fs::File>, t: absl_Time) -> Ptr<std::fs::File> {
    let t: Value<absl_Time> = Rc::new(RefCell::new(t));
    return os.write_all(
        &([(&({ FormatTime_179((*t.borrow()).clone()) })
            .iter()
            .take(({ FormatTime_179((*t.borrow()).clone()) }).len() - 1)
            .map(|&c| c as u8)
            .collect::<Vec<u8>>()[..] as &[u8])]
        .concat()),
    );
}
pub fn MakeDuration_151(hi: i64, lo: Option<u32>) -> absl_Duration {
    let hi: Value<i64> = Rc::new(RefCell::new(hi));
    let lo: Value<u32> = Rc::new(RefCell::new(lo.unwrap_or(0_u32)));
    return absl_Duration::absl_Duration2({ (*hi.borrow()) }, { (*lo.borrow()) });
}
pub fn MakeDuration_180(hi: i64, lo: i64) -> absl_Duration {
    let hi: Value<i64> = Rc::new(RefCell::new(hi));
    let lo: Value<i64> = Rc::new(RefCell::new(lo));
    return ({ MakeDuration_151((*hi.borrow()), Some(((*lo.borrow()) as u32))) });
}
pub fn MakePosDoubleDuration_181(n: f64) -> absl_Duration {
    ...
    return if (((*ticks.borrow()) as i64) < 4000000000) {
        ({ MakeDuration_151((*int_secs.borrow()), Some((*ticks.borrow()))) })
    } else {
        ({
            MakeDuration_180(
...
pub fn MakeNormalizedDuration_105(sec: i64, ticks: i64) -> absl_Duration {
    let sec: Value<i64> = Rc::new(RefCell::new(sec));
    let ticks: Value<i64> = Rc::new(RefCell::new(ticks));
    return if ((*ticks.borrow()) < 0_i64) {
        ({ MakeDuration_180(((*sec.borrow()) - 1_i64), ((*ticks.borrow()) + 4000000000)) })
    } else {
        ({ MakeDuration_180((*sec.borrow()), (*ticks.borrow())) })
    };
}
pub fn GetRepHi_182(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return ({ absl_Duration_HiRepImpl::Get(&(*d.borrow()).rep_hi_.as_pointer()) });
}
pub fn GetRepLo_183(d: absl_Duration) -> u32 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return (*(*d.borrow()).rep_lo_.borrow());
}
pub fn IsInfiniteDuration_184(d: absl_Duration) -> bool {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return (({ GetRepLo_183((*d.borrow()).clone()) }) == !0_u32);
}
pub fn OppositeInfinity_185(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if (({ GetRepHi_182((*d.borrow()).clone()) }) < 0_i64) {
        ({ MakeDuration_151(<i64>::MAX, Some(!0_u32)) })
    } else {
        ({ MakeDuration_151(<i64>::MIN, Some(!0_u32)) })
    };
}
pub fn NegateAndSubtractOne_186(n: i64) -> i64 {
    let n: Value<i64> = Rc::new(RefCell::new(n));
...
pub fn FromUnixDuration_187(d: absl_Duration) -> absl_Time {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return absl_Time::absl_Time1({ (*d.borrow()).clone() });
}
pub fn ToUnixDuration_188(t: absl_Time) -> absl_Duration {
    let t: Value<absl_Time> = Rc::new(RefCell::new(t));
    return (*(*t.borrow()).rep_.borrow()).clone();
}
pub fn FromInt64_133(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return if (((*v.borrow()) <= (<i64>::MAX / 60_i64)) && ((*v.borrow()) >= (<i64>::MIN / 60_i64)))
    {
        ({ MakeDuration_151(((*v.borrow()) * 60_i64), None) })
    } else {
        if ((*v.borrow()) > 0_i64) {
            ({ InfiniteDuration_189() })
        } else {
            ({
                let _d: absl_Duration = ({ InfiniteDuration_189() });
                operator_neg_123(_d)
            })
        }
    };
}
pub fn FromInt64_135(v: i64, _a1: std_ratio______) -> absl_Duration {
    let v: Value<i64> = Rc::new(RefCell::new(v));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return if (((*v.borrow()) <= (<i64>::MAX / 3600_i64))
        && ((*v.borrow()) >= (<i64>::MIN / 3600_i64)))
    {
        ({ MakeDuration_151(((*v.borrow()) * 3600_i64), None) })
    } else {
        if ((*v.borrow()) > 0_i64) {
            ({ InfiniteDuration_189() })
        } else {
...
pub fn IsValidRep64_190(_a0: i32) -> bool {
    let _a0: Value<i32> = Rc::new(RefCell::new(_a0));
    return true;
}
pub fn IsValidRep64_191(_a0: i32) -> bool {
    let _a0: Value<i32> = Rc::new(RefCell::new(_a0));
    return true;
}
pub fn FromChrono_192(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_104(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_193(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_106(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_194(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
...
pub fn FromChrono_195(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_108(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_196(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_133(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn FromChrono_197(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromInt64_135(({ (*d.upgrade().deref()).count() }), std_ratio______ {}) });
}
pub fn ToInt64_198(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Nanoseconds_199((*d.borrow()).clone()) });
}
pub fn ToInt64_200(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Microseconds_201((*d.borrow()).clone()) });
}
pub fn ToInt64_202(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Milliseconds_203((*d.borrow()).clone()) });
}
pub fn ToInt64_204(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Seconds_205((*d.borrow()).clone()) });
}
pub fn ToInt64_206(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Minutes_207((*d.borrow()).clone()) });
}
pub fn ToInt64_208(d: absl_Duration, _a1: std_ratio______) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let _a1: Value<std_ratio______> = Rc::new(RefCell::new(_a1));
    return ({ ToInt64Hours_209((*d.borrow()).clone()) });
}
pub fn operator_lt_112(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return if (({ GetRepHi_182((*lhs.borrow()).clone()) })
        != ({ GetRepHi_182((*rhs.borrow()).clone()) }))
    {
        (({ GetRepHi_182((*lhs.borrow()).clone()) }) < ({ GetRepHi_182((*rhs.borrow()).clone()) }))
    } else {
        if (({ GetRepHi_182((*lhs.borrow()).clone()) }) == <i64>::MIN) {
            (({ GetRepLo_183((*lhs.borrow()).clone()) }).wrapping_add(1_u32)
                < ({ GetRepLo_183((*rhs.borrow()).clone()) }).wrapping_add(1_u32))
        } else {
            (({ GetRepLo_183((*lhs.borrow()).clone()) })
                < ({ GetRepLo_183((*rhs.borrow()).clone()) }))
        }
    };
}
pub fn operator_cmp_109(lhs: absl_Duration, rhs: absl_Duration) -> std::cmp::Ordering {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    let lhs_hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_182((*lhs.borrow()).clone()) })));
    let rhs_hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_182((*rhs.borrow()).clone()) })));
    {
    ...
    let lhs_lo: Value<u32> = Rc::new(RefCell::new(({ GetRepLo_183((*lhs.borrow()).clone()) })));
    let rhs_lo: Value<u32> = Rc::new(RefCell::new(({ GetRepLo_183((*rhs.borrow()).clone()) })));
...
pub fn operator_eq_110(lhs: absl_Duration, rhs: absl_Duration) -> bool {
    let lhs: Value<absl_Duration> = Rc::new(RefCell::new(lhs));
    let rhs: Value<absl_Duration> = Rc::new(RefCell::new(rhs));
    return (({ GetRepHi_182((*lhs.borrow()).clone()) })
        == ({ GetRepHi_182((*rhs.borrow()).clone()) }))
        && (({ GetRepLo_183((*lhs.borrow()).clone()) })
            == ({ GetRepLo_183((*rhs.borrow()).clone()) }));
}
pub fn operator_neg_123(d: absl_Duration) -> absl_Duration {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    return if (({ GetRepLo_183((*d.borrow()).clone()) }) == 0_u32) {
        if (({ GetRepHi_182((*d.borrow()).clone()) }) == <i64>::MIN) {
            ({ InfiniteDuration_189() })
        } else {
            ({ MakeDuration_151(-({ GetRepHi_182((*d.borrow()).clone()) }), None) })
        }
    } else {
        if ({ IsInfiniteDuration_184((*d.borrow()).clone()) }) {
            ({ OppositeInfinity_185((*d.borrow()).clone()) })
        } else {
            ({
                let _hi: i64 =
                    ({ NegateAndSubtractOne_186(({ GetRepHi_182((*d.borrow()).clone()) })) });
                let _lo: i64 = (4000000000 - (({ GetRepLo_183((*d.borrow()).clone()) }) as i64));
                MakeDuration_180(_hi, _lo)
            })
        }
    };
}
pub fn InfiniteDuration_189() -> absl_Duration {
    return ({ MakeDuration_151(<i64>::MAX, Some(!0_u32)) });
}
pub fn FromChrono_210(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_192((d).clone()) });
}
pub fn FromChrono_211(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_193((d).clone()) });
}
pub fn FromChrono_212(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_194((d).clone()) });
}
pub fn FromChrono_213(d: Ptr<std_chrono_duration_long_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_195((d).clone()) });
}
pub fn FromChrono_214(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_196((d).clone()) });
}
pub fn FromChrono_215(d: Ptr<std_chrono_duration_long__std_ratio_______>) -> absl_Duration {
    return ({ FromChrono_197((d).clone()) });
}
pub fn FromUnixNanos_216(ns: i64) -> absl_Time {
    let ns: Value<i64> = Rc::new(RefCell::new(ns));
    return ({ FromUnixDuration_187(({ Nanoseconds_125((*ns.borrow())) })) });
}
pub fn FromUnixMicros_217(us: i64) -> absl_Time {
    let us: Value<i64> = Rc::new(RefCell::new(us));
    return ({ FromUnixDuration_187(({ Microseconds_127((*us.borrow())) })) });
}
pub fn FromUnixMillis_218(ms: i64) -> absl_Time {
    let ms: Value<i64> = Rc::new(RefCell::new(ms));
    return ({ FromUnixDuration_187(({ Milliseconds_129((*ms.borrow())) })) });
}
pub fn FromUnixSeconds_219(s: i64) -> absl_Time {
    let s: Value<i64> = Rc::new(RefCell::new(s));
    return ({ FromUnixDuration_187(({ Seconds_130((*s.borrow())) })) });
}
pub fn FromTimeT_220(t: i64) -> absl_Time {
    let t: Value<i64> = Rc::new(RefCell::new(t));
    return ({ FromUnixDuration_187(({ Seconds_131((*t.borrow())) })) });
}
pub fn ToInt64Nanoseconds_199(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_182((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_182((*d.borrow()).clone()) }) >> 33) == 0_i64)
    {
        return ((((({ GetRepHi_182((*d.borrow()).clone()) }) * 1000_i64) * 1000_i64) * 1000_i64)
            + ((({ GetRepLo_183((*d.borrow()).clone()) }) as i64) / 4));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_118(_lhs, ({ Nanoseconds_124(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Microseconds_201(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_182((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_182((*d.borrow()).clone()) }) >> 43) == 0_i64)
    {
        return (((({ GetRepHi_182((*d.borrow()).clone()) }) * 1000_i64) * 1000_i64)
            + ((({ GetRepLo_183((*d.borrow()).clone()) }) as i64) / (4 * 1000_i64)));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_118(_lhs, ({ Microseconds_126(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Milliseconds_203(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    if (({ GetRepHi_182((*d.borrow()).clone()) }) >= 0_i64)
        && ((({ GetRepHi_182((*d.borrow()).clone()) }) >> 53) == 0_i64)
    {
        return ((({ GetRepHi_182((*d.borrow()).clone()) }) * 1000_i64)
            + ((({ GetRepLo_183((*d.borrow()).clone()) }) as i64) / ((4 * 1000_i64) * 1000_i64)));
    } else {
        return ({
            let _lhs: absl_Duration = (*d.borrow()).clone();
            operator_div_118(_lhs, ({ Milliseconds_128(1) }))
        });
    }
    panic!("ub: non-void function does not return a value")
}
pub fn ToInt64Seconds_205(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_182((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_184((*d.borrow()).clone()) }) {
        return (*hi.borrow());
    }
    if ((*hi.borrow()) < 0_i64) && (({ GetRepLo_183((*d.borrow()).clone()) }) != 0_u32) {
        (*hi.borrow_mut()).prefix_inc();
    }
    return (*hi.borrow());
}
pub fn ToInt64Minutes_207(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_182((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_184((*d.borrow()).clone()) }) {
        return (*hi.borrow());
...
pub fn ToInt64Hours_209(d: absl_Duration) -> i64 {
    let d: Value<absl_Duration> = Rc::new(RefCell::new(d));
    let hi: Value<i64> = Rc::new(RefCell::new(({ GetRepHi_182((*d.borrow()).clone()) })));
    if ({ IsInfiniteDuration_184((*d.borrow()).clone()) }) {
        return (*hi.borrow());
...
pub fn SleepFor_221(duration: absl_Duration) {
    let duration: Value<absl_Duration> = Rc::new(RefCell::new(duration));
    ({ AbslInternalSleepFor_222((*duration.borrow()).clone()) });
}
thread_local!(
    static kNoTimeout_223: Value<u64> = Rc::new(RefCell::new(18446744073709551615));
);
thread_local!(
    static kMaxNanos_224: Value<i64> = Rc::new(RefCell::new(9223372036854775807));
);
#[derive()]
...
pub fn GetOrCreateCurrentThreadIdentity_225() -> Ptr<absl_base_internal_ThreadIdentity> {
    let identity: Value<Ptr<absl_base_internal_ThreadIdentity>> =
        Rc::new(RefCell::new(({ CurrentThreadIdentityIfPresent_226() })));
    if ((((false) || ((*identity.borrow()).is_null())) as i64) != 0) {
        return ({ CreateThreadIdentity_227() });
    }
...
impl absl_synchronization_internal_PerThreadSem {
    fn Init(identity: Ptr<absl_base_internal_ThreadIdentity>) {
        ...
        ({ AbslInternalPerThreadSemInit_228((*identity.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {
    fn Post(identity: Ptr<absl_base_internal_ThreadIdentity>) {
        let identity: Value<Ptr<absl_base_internal_ThreadIdentity>> =
            Rc::new(RefCell::new(identity));
        ({ AbslInternalPerThreadSemPost_229((*identity.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {
    fn Wait(t: absl_synchronization_internal_KernelTimeout) -> bool {
        ...
        return ({ AbslInternalPerThreadSemWait_230((*t.borrow()).clone()) });
    }
}
impl absl_synchronization_internal_PerThreadSem {
...
pub type v8_internal_TryAbortResult = i32;
pub const v8_internal_TryAbortResult_kTaskRemoved: v8_internal_TryAbortResult = 0;
pub const v8_internal_TryAbortResult_kTaskRunning: v8_internal_TryAbortResult = 1;
pub const v8_internal_TryAbortResult_kTaskAborted: v8_internal_TryAbortResult = 2;
thread_local!(
    pub static kInvalidTaskId_235: Value<u64> = Rc::new(RefCell::new(0));
);
#[derive()]
pub struct v8_internal_CancelableTaskManager {  task_id_counter_ : Value<u64 > ,  cancelable_tasks_ : Value<std_unordered_map_unsigned_long_long__v8_internal_Cancelable_ptr__std_hash_unsigned_long_long___std_equal_to_unsigned_long_long___std_allocator_std_pair_const_unsigned_long_long__v8_internal_Cancelable_ptr___ > ,  cancelable_tasks_barrier_ : Value<v8_base_ConditionVariable > ,  mutex_ : Value<v8_base_Mutex > ,  canceled_ : Value<bool > , }
impl v8_internal_CancelableTaskManager {}
impl Default for v8_internal_CancelableTaskManager {
    fn default() -> Self {
        { v8_internal_CancelableTaskManager::v8_internal_CancelableTaskManager() }
    }
}
impl ByteRepr for v8_internal_CancelableTaskManager {
    fn byte_size() -> usize {
        72
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.task_id_counter_.borrow()).to_bytes(&mut buf[0..8]);
        (*self.cancelable_tasks_.borrow()).to_bytes(&mut buf[8..48]);
        (*self.cancelable_tasks_barrier_.borrow()).to_bytes(&mut buf[48..56]);
        (*self.mutex_.borrow()).to_bytes(&mut buf[56..64]);
        (*self.canceled_.borrow()).to_bytes(&mut buf[64..65]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self { task_id_counter_: Rc::new(RefCell::new(<u64 >::from_bytes(&buf[0..8]))), cancelable_tasks_: Rc::new(RefCell::new(<std_unordered_map_unsigned_long_long__v8_internal_Cancelable_ptr__std_hash_unsigned_long_long___std_equal_to_unsigned_long_long___std_allocator_std_pair_const_unsigned_long_long__v8_internal_Cancelable_ptr___ >::from_bytes(&buf[8..48]))), cancelable_tasks_barrier_: Rc::new(RefCell::new(<v8_base_ConditionVariable >::from_bytes(&buf[48..56]))), mutex_: Rc::new(RefCell::new(<v8_base_Mutex >::from_bytes(&buf[56..64]))), canceled_: Rc::new(RefCell::new(<bool >::from_bytes(&buf[64..65]))), }
    }
}
pub type v8_internal_Cancelable_Status = u32;
pub const v8_internal_Cancelable_Status_kWaiting: v8_internal_Cancelable_Status = 0;
pub const v8_internal_Cancelable_Status_kCanceled: v8_internal_Cancelable_Status = 1;
pub const v8_internal_Cancelable_Status_kRunning: v8_internal_Cancelable_Status = 2;
#[derive(Default)]
pub struct v8_internal_Cancelable {
    parent_: Value<Ptr<v8_internal_CancelableTaskManager>>,
    status_: Value<std_atomic_v8_internal_Cancelable_Status_>,
    id_: Value<u64>,
}
impl v8_internal_Cancelable {
    pub fn v8_internal_Cancelable(parent: Ptr<v8_internal_CancelableTaskManager>) -> Self {
        let parent: Value<Ptr<v8_internal_CancelableTaskManager>> = Rc::new(RefCell::new(parent));
        let __this : Value<v8_internal_Cancelable> = Rc::new(RefCell::new(Self { parent_ : Rc::new(RefCell::new(((*parent.borrow()) ).clone())) , status_ : Rc::new(RefCell::new(std_atomic_v8_internal_Cancelable_Status_ :: std_atomic_v8_internal_Cancelable_Status_2 ( {  v8_internal_Cancelable_Status_kWaiting   } , ) )) , id_ : Rc::new(RefCell::new((  { v8_internal_CancelableTaskManagerImpl :: Register ( &(*parent.borrow())  , (this ).clone() , ) } )  )) , } )) ;
        let this: Ptr<v8_internal_Cancelable> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for v8_internal_Cancelable {
    fn byte_size() -> usize {
        32
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.parent_.borrow()).to_bytes(&mut buf[8..16]);
        (*self.status_.borrow()).to_bytes(&mut buf[16..20]);
        (*self.id_.borrow()).to_bytes(&mut buf[24..32]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            parent_: Rc::new(RefCell::new(
                <Ptr<v8_internal_CancelableTaskManager>>::from_bytes(&buf[8..16]),
            )),
            status_: Rc::new(RefCell::new(
                <std_atomic_v8_internal_Cancelable_Status_>::from_bytes(&buf[16..20]),
            )),
            id_: Rc::new(RefCell::new(<u64>::from_bytes(&buf[24..32]))),
        }
    }
}
pub trait v8_internal_CancelableTask {
    fn Run(&self) {
        if ({ v8_internal_CancelableImpl::TryRun(&(*self), None) }) {
            ({ self.RunInternal() });
        }
    }
    fn RunInternal(&self);
}
pub trait v8_internal_CancelableIdleTask {
    fn Run(&self, deadline_in_seconds: f64) {
        let deadline_in_seconds: Value<f64> = Rc::new(RefCell::new(deadline_in_seconds));
        if ({ v8_internal_CancelableImpl::TryRun(&(*self), None) }) {
            ({ self.RunInternal((*deadline_in_seconds.borrow())) });
        }
    }
    fn RunInternal(&self, deadline_in_seconds: f64);
}
#[derive(Default)]
pub struct v8_internal_CancelableFuncTask {
    func_: Value<std_function_void____>,
}
impl v8_internal_CancelableFuncTask {
    pub fn v8_internal_CancelableFuncTask1(
        isolate: Ptr<v8_internal_Isolate>,
        func: std_function_void____,
    ) -> Self {
        let isolate: Value<Ptr<v8_internal_Isolate>> = Rc::new(RefCell::new(isolate));
        let func: Value<std_function_void____> = Rc::new(RefCell::new(func));
        let __this: Value<v8_internal_CancelableFuncTask> = Rc::new(RefCell::new(Self {
            func_: Rc::new(RefCell::new((*func.borrow_mut()).clone())),
        }));
        let this: Ptr<v8_internal_CancelableFuncTask> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
    pub fn v8_internal_CancelableFuncTask2(
        manager: Ptr<v8_internal_CancelableTaskManager>,
        func: std_function_void____,
    ) -> Self {
        let manager: Value<Ptr<v8_internal_CancelableTaskManager>> = Rc::new(RefCell::new(manager));
        let func: Value<std_function_void____> = Rc::new(RefCell::new(func));
        let __this: Value<v8_internal_CancelableFuncTask> = Rc::new(RefCell::new(Self {
            func_: Rc::new(RefCell::new((*func.borrow_mut()).clone())),
        }));
...
impl v8_internal_CancelableTask for v8_internal_CancelableFuncTask {
    fn RunInternal(&self) {
        ({ (*self.func_.borrow_mut())() });
    }
}
impl ByteRepr for v8_internal_CancelableFuncTask {
    fn byte_size() -> usize {
        72
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.func_.borrow()).to_bytes(&mut buf[40..72]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            func_: Rc::new(RefCell::new(<std_function_void____>::from_bytes(
                &buf[40..72],
            ))),
        }
    }
...
#[derive(Default)]
pub struct v8_internal_CancelableIdleFuncTask {
    func_: Value<std_function_void__double__>,
}
impl v8_internal_CancelableIdleFuncTask {
    pub fn v8_internal_CancelableIdleFuncTask1(
        isolate: Ptr<v8_internal_Isolate>,
        func: std_function_void__double__,
    ) -> Self {
        let isolate: Value<Ptr<v8_internal_Isolate>> = Rc::new(RefCell::new(isolate));
        let func: Value<std_function_void__double__> = Rc::new(RefCell::new(func));
        let __this: Value<v8_internal_CancelableIdleFuncTask> = Rc::new(RefCell::new(Self {
            func_: Rc::new(RefCell::new((*func.borrow_mut()).clone())),
        }));
        let this: Ptr<v8_internal_CancelableIdleFuncTask> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
    pub fn v8_internal_CancelableIdleFuncTask2(
        manager: Ptr<v8_internal_CancelableTaskManager>,
        func: std_function_void__double__,
    ) -> Self {
        let manager: Value<Ptr<v8_internal_CancelableTaskManager>> = Rc::new(RefCell::new(manager));
...
impl v8_internal_CancelableIdleTask for v8_internal_CancelableIdleFuncTask {
    fn RunInternal(&self, deadline_in_seconds: f64) {
        let deadline_in_seconds: Value<f64> = Rc::new(RefCell::new(deadline_in_seconds));
        ({ (*self.func_.borrow_mut())((*deadline_in_seconds.borrow())) });
    }
}
impl ByteRepr for v8_internal_CancelableIdleFuncTask {
    fn byte_size() -> usize {
        72
    }
    fn to_bytes(&self, buf: &mut [u8]) {
    ...
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            func_: Rc::new(RefCell::new(<std_function_void__double__>::from_bytes(
                &buf[40..72],
            ))),
        }
    }
}
pub fn MakeCancelableTask_236(
    isolate: Ptr<v8_internal_Isolate>,
    func: std_function_void____,
) -> Option<Value<v8_internal_CancelableTask>> {
    let isolate: Value<Ptr<v8_internal_Isolate>> = Rc::new(RefCell::new(isolate));
    let func: Value<std_function_void____> = Rc::new(RefCell::new(func));
    return ({ make_unique_237(isolate.as_pointer(), func.as_pointer()) });
}
pub fn MakeCancelableTask_238(
    manager: Ptr<v8_internal_CancelableTaskManager>,
    func: std_function_void____,
) -> Option<Value<v8_internal_CancelableTask>> {
    let manager: Value<Ptr<v8_internal_CancelableTaskManager>> = Rc::new(RefCell::new(manager));
    let func: Value<std_function_void____> = Rc::new(RefCell::new(func));
    return ({ make_unique_239(manager.as_pointer(), func.as_pointer()) });
}
pub fn MakeCancelableIdleTask_240(
    isolate: Ptr<v8_internal_Isolate>,
    func: std_function_void__double__,
) -> Option<Value<v8_internal_CancelableIdleTask>> {
    let isolate: Value<Ptr<v8_internal_Isolate>> = Rc::new(RefCell::new(isolate));
    let func: Value<std_function_void__double__> = Rc::new(RefCell::new(func));
    return ({ make_unique_241(isolate.as_pointer(), func.as_pointer()) });
}
pub fn MakeCancelableIdleTask_242(
    manager: Ptr<v8_internal_CancelableTaskManager>,
    func: std_function_void__double__,
) -> Option<Value<v8_internal_CancelableIdleTask>> {
    let manager: Value<Ptr<v8_internal_CancelableTaskManager>> = Rc::new(RefCell::new(manager));
    let func: Value<std_function_void__double__> = Rc::new(RefCell::new(func));
    return ({ make_unique_243(manager.as_pointer(), func.as_pointer()) });
}
#[derive(Clone, Copy, Default, ByteRepr)]
pub struct v8_internal_Isolate;
#[derive(Clone, Copy, Default, ByteRepr)]
...
pub trait v8_internal_CancelableImpl {
    fn id(&self) -> u64;
    fn TryRun(&self, previous: Option<Ptr<v8_internal_Cancelable_Status>>) -> bool;
    fn Cancel(&self) -> bool;
    fn CompareExchangeStatus(
        &self,
        expected: v8_internal_Cancelable_Status,
        desired: v8_internal_Cancelable_Status,
        previous: Option<Ptr<v8_internal_Cancelable_Status>>,
    ) -> bool;
}
impl v8_internal_CancelableImpl for Ptr<v8_internal_Cancelable> {
    fn id(&self) -> u64 {
        return (*(*(*self).upgrade().deref()).id_.borrow());
    }
    fn TryRun(&self, previous: Option<Ptr<v8_internal_Cancelable_Status>>) -> bool {
        let previous: Value<Ptr<v8_internal_Cancelable_Status>> = Rc::new(RefCell::new(
            previous.unwrap_or(Ptr::<v8_internal_Cancelable_Status>::null()),
        ));
        return ({
            let _expected: v8_internal_Cancelable_Status = v8_internal_Cancelable_Status_kWaiting;
            let _desired: v8_internal_Cancelable_Status = v8_internal_Cancelable_Status_kRunning;
            let _previous: Ptr<v8_internal_Cancelable_Status> = (*previous.borrow()).clone();
            v8_internal_CancelableImpl::CompareExchangeStatus(
                self,
                _expected,
                _desired,
                Some(_previous),
    ...
    fn Cancel(&self) -> bool {
        return ({
            v8_internal_CancelableImpl::CompareExchangeStatus(
                self,
                v8_internal_Cancelable_Status_kWaiting,
                v8_internal_Cancelable_Status_kCanceled,
                None,
            )
        });
    }
    fn CompareExchangeStatus(
        ...
        let expected: Value<v8_internal_Cancelable_Status> = Rc::new(RefCell::new(expected));
        let desired: Value<v8_internal_Cancelable_Status> = Rc::new(RefCell::new(desired));
        let previous: Value<Ptr<v8_internal_Cancelable_Status>> = Rc::new(RefCell::new(
            previous.unwrap_or(Ptr::<v8_internal_Cancelable_Status>::null()),
        ));
        let success: Value<bool> = Rc::new(RefCell::new(
            ({
                (*(*(* self ) .upgrade().deref()) . status_ .borrow()) . compare_exchange_strong_pmutv8_internal_Cancelable_Status_v8_internal_Cancelable_Status ( expected .as_pointer()  , (*desired.borrow())  , 4  , 2  , )
            }),
        ));
        if !(*previous.borrow()).is_null() {
            let __rhs = (*expected.borrow());
            (*previous.borrow()).write(__rhs);
        }
        return (*success.borrow());
    }
}
pub trait v8_internal_CancelableTaskManagerImpl {
    fn canceled(&self) -> bool;
}
impl v8_internal_CancelableTaskManagerImpl for Ptr<v8_internal_CancelableTaskManager> {
    fn canceled(&self) -> bool {
        return (*(*(*self).upgrade().deref()).canceled_.borrow());
    }
}

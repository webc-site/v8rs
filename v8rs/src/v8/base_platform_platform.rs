...
thread_local!(
    pub static thread_stack_start_40: Value<AnyPtr> = Rc::new(RefCell::new(AnyPtr::default()));
);
thread_local!(
    pub static thread_reserved_stack_limit_41: Value<AnyPtr> =
        Rc::new(RefCell::new(AnyPtr::default()));
);
impl v8_base_Stack {
    fn GetStackStartUnchecked() -> v8_base_Stack_StackSlot {
        if !(!(*thread_stack_start_40.with(Value::clone).borrow()).is_null()) {
            (*thread_stack_start_40.with(Value::clone).borrow_mut()) = ({
                v8_base_Stack_StackSlotImpl::operator_void__(
                    &Rc::new(RefCell::new(
                        ({ v8_base_Stack::ObtainCurrentThreadStackStart() }),
                    ))
                    .as_pointer(),
                )
            });
        ...
        return v8_base_Stack_StackSlot::v8_base_Stack_StackSlot1({
            (*thread_stack_start_40.with(Value::clone).borrow()).clone()
        });
    }
}
impl v8_base_Stack {
    pub fn GetReservedStackLimit() -> v8_base_Stack_StackSlot {
        if !(!(*thread_reserved_stack_limit_41.with(Value::clone).borrow()).is_null()) {
            (*thread_reserved_stack_limit_41
                .with(Value::clone)
                .borrow_mut()) = ({
                v8_base_Stack_StackSlotImpl::operator_void__(
                    &Rc::new(RefCell::new(
                        ({ v8_base_Stack::ObtainCurrentThreadStackReservedLimit() }),
                    ))
                    .as_pointer(),
                )
            });
        ...
        return v8_base_Stack_StackSlot::v8_base_Stack_StackSlot1({
            (*thread_reserved_stack_limit_41.with(Value::clone).borrow()).clone()
        });
    }
}
impl v8_base_Stack {
    pub fn GetStackStart() -> v8_base_Stack_StackSlot {
        return ({ v8_base_Stack::GetStackStartUnchecked() });
    }
}
impl v8_base_OS {
    pub fn GetCurrentThreadId() -> i32 {
        thread_local!(
            static id_42: Value<i32> =
                Rc::new(RefCell::new(({ v8_base_OS::GetCurrentThreadIdInternal() })));
        );
        return (*id_42.with(Value::clone).borrow());
    }
}

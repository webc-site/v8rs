...
impl v8_base_LsanPageAllocator {
    pub fn v8_base_LsanPageAllocator(page_allocator: PtrDyn<dyn v8_PageAllocator>) -> Self {
        let page_allocator: Value<PtrDyn<dyn v8_PageAllocator>> =
        ...
        let __this: Value<v8_base_LsanPageAllocator> = Rc::new(RefCell::new(Self {
            ...
            allocate_page_size_: Rc::new(RefCell::new(
                ({
                    (*(*(*this.upgrade().deref()).page_allocator_.borrow())
                        .upgrade()
                        .deref())
                    .AllocatePageSize()
                }),
            )),
            commit_page_size_: Rc::new(RefCell::new(
                ({
                    (*(*(*this.upgrade().deref()).page_allocator_.borrow())
                        .upgrade()
                        .deref())
...
impl v8_PageAllocator for v8_base_LsanPageAllocator {
    ...
    fn AllocatePages(
        ...
        let result: Value<AnyPtr> = Rc::new(RefCell::new(
            ({
                (*(*self.page_allocator_.borrow()).upgrade().deref())
                    .AllocatePages_AnyPtr_usize_usize_v8_PageAllocator_Permission(
                        (*hint.borrow()).clone(),
                        (*size.borrow()),
                        (*alignment.borrow()),
                        (*access.borrow()),
    ...
    fn AllocateSharedPages(
        ...
        let size: Value<usize> = Rc::new(RefCell::new(size));
        let original_address: Value<AnyPtr> = Rc::new(RefCell::new(original_address));
        let result: Value<Option<Value<v8_PageAllocator_SharedMemory>>> = Rc::new(RefCell::new(
            ({
                (*(*self.page_allocator_.borrow()).upgrade().deref())
                    .AllocateSharedPages((*size.borrow()), (*original_address.borrow()).clone())
            }),
        ));
    ...
    fn CanAllocateSharedPages(&self) -> bool {
        return ({
            (*(*self.page_allocator_.borrow()).upgrade().deref()).CanAllocateSharedPages()
        });
    }
    fn FreePages(&self, address: AnyPtr, size: usize) -> bool {
        ...
        'loop_: while __do_while || (false) {
            ...
            if ((!(!(!({
                (*(*self.page_allocator_.borrow()).upgrade().deref())
                    .FreePages((*address.borrow()).clone(), (*size.borrow()))
            }))) as i64)
                != 0)
            {
                ({
                    V8_Fatal_10(
                        ...
                        &[
                            (Ptr::from_string_literal(
                                b"page_allocator_->FreePages(address, size)",
                            ))
                            .into(),
                        ],
                    )
    ...
    fn ReleasePages(&self, address: AnyPtr, size: usize, new_size: usize) -> bool {
        ...
        'loop_: while __do_while || (false) {
            __do_while = false;
            if ((!(!(!({
                (*(*self.page_allocator_.borrow()).upgrade().deref()).ReleasePages(
                ...
                != 0)
            {
                ({
                    V8_Fatal_10(
                        Ptr::from_string_literal(b"Check failed: %s."),
                        &[(Ptr::from_string_literal(
                            b"page_allocator_->ReleasePages(address, size, new_size)",
                        ))
                        .into()],
                    )
                });

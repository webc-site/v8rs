...
thread_local!();
#[derive(Default)]
pub struct heap_base_Worklist_const_void_ptrptr_____Segment {
    next_: Value<Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>>,
}
impl heap_base_Worklist_const_void_ptrptr_____Segment {
    pub fn Delete(segment: Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>) {
        let segment: Value<Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>> =
            Rc::new(RefCell::new(segment));
        ({
            Free_69(
                ((*segment.borrow()).clone()
                    as Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>)
                    .to_any(),
            )
        });
    }
}
impl Clone for heap_base_Worklist_const_void_ptrptr_____Segment {
    fn clone(&self) -> Self {
        let __this: Value<heap_base_Worklist_const_void_ptrptr_____Segment> =
            Rc::new(RefCell::new(Self {
                next_: Rc::new(RefCell::new((*self.next_.borrow()).clone())),
            }));
        let this: Ptr<heap_base_Worklist_const_void_ptrptr_____Segment> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for heap_base_Worklist_const_void_ptrptr_____Segment {
    fn byte_size() -> usize {
        16
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.next_.borrow()).to_bytes(&mut buf[8..16]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            next_: Rc::new(RefCell::new(<Ptr<
                heap_base_Worklist_const_void_ptrptr_____Segment,
            >>::from_bytes(&buf[8..16]))),
        }
    }
}
#[derive(Default)]
pub struct heap_base_Worklist_const_void_ptrptr____ {
    lock_: Value<v8_base_Mutex>,
    top_: Value<Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>>,
    size_: Value<std_atomic_unsigned_long_>,
}
impl ByteRepr for heap_base_Worklist_const_void_ptrptr____ {
    fn byte_size() -> usize {
        24
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.lock_.borrow()).to_bytes(&mut buf[0..8]);
        (*self.top_.borrow()).to_bytes(&mut buf[8..16]);
        (*self.size_.borrow()).to_bytes(&mut buf[16..24]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            lock_: Rc::new(RefCell::new(<v8_base_Mutex>::from_bytes(&buf[0..8]))),
            top_: Rc::new(RefCell::new(<Ptr<
                heap_base_Worklist_const_void_ptrptr_____Segment,
            >>::from_bytes(&buf[8..16]))),
            size_: Rc::new(RefCell::new(<std_atomic_unsigned_long_>::from_bytes(
                &buf[16..24],
            ))),
        }
    }
...
#[derive(Default)]
pub struct cppgc_internal_CompactionWorklists {
    movable_slots_worklist_: Value<heap_base_Worklist_const_void_ptrptr____>,
}
impl ByteRepr for cppgc_internal_CompactionWorklists {
    fn byte_size() -> usize {
        24
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.movable_slots_worklist_.borrow()).to_bytes(&mut buf[0..24]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            movable_slots_worklist_: Rc::new(RefCell::new(
                <heap_base_Worklist_const_void_ptrptr____>::from_bytes(&buf[0..24]),
            )),
        }
    }
}
...
pub trait cppgc_internal_CompactionWorklistsImpl {
    fn movable_slots_worklist(&self) -> Ptr<heap_base_Worklist_const_void_ptrptr____>;
    fn ClearForTesting(&self);
}
impl cppgc_internal_CompactionWorklistsImpl for Ptr<cppgc_internal_CompactionWorklists> {
    fn movable_slots_worklist(&self) -> Ptr<heap_base_Worklist_const_void_ptrptr____> {
        return ((*(*self).upgrade().deref())
    ...
    fn ClearForTesting(&self) {
        ({
            heap_base_Worklist_const_void_ptrptr____Impl::Clear(
                &(*(*self).upgrade().deref())
                    .movable_slots_worklist_
                    .as_pointer(),
            )
...
pub trait heap_base_Worklist_const_void_ptrptr____Impl {
    fn Clear(&self);
}
impl heap_base_Worklist_const_void_ptrptr____Impl
    for Ptr<heap_base_Worklist_const_void_ptrptr____>
{
    fn Clear(&self) {
        let guard: Value<v8_base_LockGuard_v8_base_Mutex_> = Rc::new(RefCell::new(
            v8_base_LockGuard_v8_base_Mutex_::v8_base_LockGuard_v8_base_Mutex_1({
        ...
        let _dtor_guard = ScopedDestructor::new(&guard, |__p| __p.destructor());
        ({ (*(*(*self).upgrade().deref()).size_.borrow()).store_u64(0_u64, Some(0)) });
        let current: Value<Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>> = Rc::new(
            RefCell::new((*(*(*self).upgrade().deref()).top_.borrow()).clone()),
        );
        'loop_: while !((*current.borrow()).is_null()) {
            let tmp: Value<Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>> =
                Rc::new(RefCell::new((*current.borrow()).clone()));
            let __rhs = ({
                heap_base_Worklist_const_void_ptrptr_____SegmentImpl::next(&(*current.borrow()))
            });
            (*current.borrow_mut()) = __rhs;
            ({ heap_base_Worklist_const_void_ptrptr_____Segment::Delete((*tmp.borrow()).clone()) });
        }
        (*(*(*self).upgrade().deref()).top_.borrow_mut()) =
            Ptr::<heap_base_Worklist_const_void_ptrptr_____Segment>::null();
    }
}
pub trait heap_base_Worklist_const_void_ptrptr_____SegmentImpl {
    fn next(&self) -> Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>;
}
impl heap_base_Worklist_const_void_ptrptr_____SegmentImpl
    for Ptr<heap_base_Worklist_const_void_ptrptr_____Segment>
{
    fn next(&self) -> Ptr<heap_base_Worklist_const_void_ptrptr_____Segment> {
        return (*(*(*self).upgrade().deref()).next_.borrow()).clone();
    }
}

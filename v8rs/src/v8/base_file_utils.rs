...
pub fn RelativePath_40(exec_path: Ptr<u8>, name: Ptr<u8>) -> Option<Value<Box<[u8]>>> {
    let exec_path: Value<Ptr<u8>> = Rc::new(RefCell::new(exec_path));
    let name: Value<Ptr<u8>> = Rc::new(RefCell::new(name));
    (&(0));
    let basename_start: Value<usize> = Rc::new(RefCell::new(
        (*exec_path.borrow()).to_c_string_iterator().count(),
    ));
    'loop_: while ((*basename_start.borrow()) > 0_usize)
        && (!({
            v8_base_OS::isDirectorySeparator(
                ((*exec_path.borrow())
                    .offset(((*basename_start.borrow()).wrapping_sub(1_usize)) as isize)
                    .read()),
            )
        }))
    {
        (*basename_start.borrow_mut()).prefix_dec();
    }
    let name_length: Value<usize> = Rc::new(RefCell::new(
        (*name.borrow()).to_c_string_iterator().count(),
    ));
    let buffer: Value<Option<Value<Box<[u8]>>>> =
        Rc::new(RefCell::new(Some(Rc::new(RefCell::new(
            (0..((*basename_start.borrow()).wrapping_add((*name_length.borrow())))
                .wrapping_add(1_usize))
                .map(|_| <u8>::default())
                .collect::<Box<[_]>>(),
        )))));
    if ((*basename_start.borrow()) > 0_usize) {
        {
            ((*buffer.borrow()).as_pointer() as Ptr<u8>)
                .to_any()
                .memcpy(
                    &((*exec_path.borrow()).clone() as Ptr<u8>).to_any(),
                    (*basename_start.borrow()) as usize,
                );
            ((*buffer.borrow()).as_pointer() as Ptr<u8>).to_any()
        };
    }
    {
        ((*buffer.borrow())
            .as_pointer()
            .offset((*basename_start.borrow()) as isize) as Ptr<u8>)
            .to_any()
            .memcpy(
                &((*name.borrow()).clone() as Ptr<u8>).to_any(),
                (*name_length.borrow()) as usize,
            );
        ((*buffer.borrow())
            .as_pointer()
            .offset((*basename_start.borrow()) as isize) as Ptr<u8>)

...
thread_local!(
  pub static kFlushDenormToZeroBit_0: Value<i32> = Rc::new(RefCell::new((1 << 24)));
);
...
impl v8_base_FPU {
  pub fn GetFlushDenormals() -> bool {
    ...
    return (((*csr.borrow()) & (*kFlushDenormToZeroBit_0.with(Value::clone).borrow())) != 0);
  }
}
impl v8_base_FPU {
  pub fn SetFlushDenormals(value: bool) {
    ...
    let new_csr: Value<i32> = Rc::new(RefCell::new(if (*value.borrow()) {
      ((*old_csr.borrow()) | (*kFlushDenormToZeroBit_0.with(Value::clone).borrow()))
    } else {
      ((*old_csr.borrow()) & !(*kFlushDenormToZeroBit_0.with(Value::clone).borrow()))
    }));

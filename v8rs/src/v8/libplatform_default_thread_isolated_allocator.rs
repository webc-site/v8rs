...
thread_local!(
    pub static kReturnAddressStackSlotCount_10: Value<i32> = Rc::new(RefCell::new(0));
);
thread_local!(
    pub static kPageSizeBits_11: Value<i32> = Rc::new(RefCell::new(18));
);
thread_local!(
    pub static kRegularPageSize_12: Value<i32> = Rc::new(RefCell::new(262144));
);
thread_local!(
    pub static kMinimumOSPageSize_13: Value<i32> = Rc::new(RefCell::new(16384));
);
...
thread_local!();
pub fn ControlledCrashesAreHarmless_15() -> bool {
    return ((*g_abort_mode_14.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_14.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn DcheckFailuresAreIgnored_16() -> bool {
    return ((*g_abort_mode_14.with(Value::clone).borrow())
        == v8_base_AbortMode_kExitWithSuccessAndIgnoreDcheckFailures)
        || ((*g_abort_mode_14.with(Value::clone).borrow())
            == v8_base_AbortMode_kExitWithFailureAndIgnoreDcheckFailures);
}
pub fn FatalErrorsWithNoSecurityImpactShouldExit_17() -> bool {
    return ((*g_abort_mode_14.with(Value::clone).borrow())
...
#[derive()]
pub struct v8_platform_DefaultThreadIsolatedAllocator {}
impl v8_platform_DefaultThreadIsolatedAllocator {
    pub fn v8_platform_DefaultThreadIsolatedAllocator() -> Self {
        let __this: Value<v8_platform_DefaultThreadIsolatedAllocator> =
            Rc::new(RefCell::new(Self {}));
        let this: Ptr<v8_platform_DefaultThreadIsolatedAllocator> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl v8_ThreadIsolatedAllocator for v8_platform_DefaultThreadIsolatedAllocator {
    fn Allocate(&self, size: usize) -> AnyPtr {
        let size: Value<usize> = Rc::new(RefCell::new(size));
        return malloc_refcount((*size.borrow()));
    }
    fn Free(&self, object: AnyPtr) {
        let object: Value<AnyPtr> = Rc::new(RefCell::new(object));
        free_refcount((*object.borrow()).clone());
    }
    fn Type(&self) -> v8_ThreadIsolatedAllocator_Type {
        ({
            V8_Fatal_40(
                (*kUnreachableCodeMessage_19.with(Value::clone).borrow()).clone(),
                &[],
            )
        });
...
impl Clone for v8_platform_DefaultThreadIsolatedAllocator {
    fn clone(&self) -> Self {
        let __this: Value<v8_platform_DefaultThreadIsolatedAllocator> =
            Rc::new(RefCell::new(Self {}));
        let this: Ptr<v8_platform_DefaultThreadIsolatedAllocator> = __this.as_pointer();
...
impl Default for v8_platform_DefaultThreadIsolatedAllocator {
    fn default() -> Self {
        { v8_platform_DefaultThreadIsolatedAllocator::v8_platform_DefaultThreadIsolatedAllocator() }
    }
}
impl ByteRepr for v8_platform_DefaultThreadIsolatedAllocator {
    fn byte_size() -> usize {
        8
    }
    fn to_bytes(&self, buf: &mut [u8]) {}
...
pub trait v8_platform_DefaultThreadIsolatedAllocatorImpl {
    fn Valid(&self) -> bool;
}
impl v8_platform_DefaultThreadIsolatedAllocatorImpl
    for Ptr<v8_platform_DefaultThreadIsolatedAllocator>
{
    fn Valid(&self) -> bool {
        return false;
    }

...
impl v8_base_CPUImpl for Ptr<v8_base_CPU> {
    ...
    fn DetectFeatures(&self) {
        (*(*(*self).upgrade().deref()).has_jscvt_.borrow_mut()) = true;
        (*(*(*self).upgrade().deref()).has_dot_prod_.borrow_mut()) = true;
        (*(*(*self).upgrade().deref()).has_lse_.borrow_mut()) = true;
        (*(*(*self).upgrade().deref()).has_pmull1q_.borrow_mut()) = true;
        (*(*(*self).upgrade().deref()).has_fp16_.borrow_mut()) = true;
        (*(*(*self).upgrade().deref()).has_sha3_.borrow_mut()) = true;
        let feat_cssc: Value<i64> = Rc::new(RefCell::new(0_i64));
        let feat_cssc_size: Value<usize> = Rc::new(RefCell::new(::std::mem::size_of::<i64>()));
        if (({
            sysctlbyname_37(
                Ptr::from_string_literal(b"hw.optional.arm.FEAT_CSSC"),
                ((feat_cssc.as_pointer()) as Ptr<i64>).to_any(),
                (feat_cssc_size.as_pointer()),
                AnyPtr::default(),
                0_usize,
            )
        }) == -1_i32)
        {
            (*(*(*self).upgrade().deref()).has_cssc_.borrow_mut()) = false;
        } else {
            (*(*(*self).upgrade().deref()).has_cssc_.borrow_mut()) = ((*feat_cssc.borrow()) != 0);
        }
    }
}

...
#[derive(Default)]
pub struct v8_internal_OperationsBarrier_Token {
    outer_: Value<Ptr<v8_internal_OperationsBarrier>>,
}
impl v8_internal_OperationsBarrier_Token {
    pub fn Token_pmutv8_internal_OperationsBarrier_Token(
        other: Ptr<v8_internal_OperationsBarrier_Token>,
    ) -> Self {
        let __this: Value<v8_internal_OperationsBarrier_Token> = Rc::new(RefCell::new(Self {
            outer_: Rc::new(RefCell::new(
                (*(*other.upgrade().deref()).outer_.borrow()).clone(),
            )),
        }));
        let this: Ptr<v8_internal_OperationsBarrier_Token> = __this.as_pointer();
        (*(*other.upgrade().deref()).outer_.borrow_mut()) =
            Ptr::<v8_internal_OperationsBarrier>::null();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
    fn v8_internal_OperationsBarrier_Token1(outer: Ptr<v8_internal_OperationsBarrier>) -> Self {
        let outer: Value<Ptr<v8_internal_OperationsBarrier>> = Rc::new(RefCell::new(outer));
        let __this: Value<v8_internal_OperationsBarrier_Token> = Rc::new(RefCell::new(Self {
...
impl ByteRepr for v8_internal_OperationsBarrier_Token {
    fn byte_size() -> usize {
        8
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.outer_.borrow()).to_bytes(&mut buf[0..8]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            outer_: Rc::new(RefCell::new(
                <Ptr<v8_internal_OperationsBarrier>>::from_bytes(&buf[0..8]),
            )),
        }
    }
}
#[derive(Default)]
pub struct v8_internal_OperationsBarrier {
    mutex_: Value<v8_base_Mutex>,
    release_condition_: Value<v8_base_ConditionVariable>,
    cancelled_: Value<bool>,
    operations_count_: Value<usize>,
}
impl ByteRepr for v8_internal_OperationsBarrier {
    fn byte_size() -> usize {
        32
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.mutex_.borrow()).to_bytes(&mut buf[0..8]);
        (*self.release_condition_.borrow()).to_bytes(&mut buf[8..16]);
        (*self.cancelled_.borrow()).to_bytes(&mut buf[16..17]);
        (*self.operations_count_.borrow()).to_bytes(&mut buf[24..32]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            mutex_: Rc::new(RefCell::new(<v8_base_Mutex>::from_bytes(&buf[0..8]))),
            release_condition_: Rc::new(RefCell::new(<v8_base_ConditionVariable>::from_bytes(
                &buf[8..16],
            ))),
            cancelled_: Rc::new(RefCell::new(<bool>::from_bytes(&buf[16..17]))),
            operations_count_: Rc::new(RefCell::new(<usize>::from_bytes(&buf[24..32]))),
        }
    }
}
...
pub trait v8_internal_OperationsBarrierImpl {
    fn destructor(&self);
    fn TryLock(&self) -> v8_internal_OperationsBarrier_Token;
    fn CancelAndWait(&self);
    fn cancelled(&self) -> bool;
    fn Release(&self);
}
impl v8_internal_OperationsBarrierImpl for Ptr<v8_internal_OperationsBarrier> {
    fn destructor(&self) {
        (&(0));
    }
    fn cancelled(&self) -> bool {
        return (*(*(*self).upgrade().deref()).cancelled_.borrow());
    }
    fn TryLock(&self) -> v8_internal_OperationsBarrier_Token {
        ...
        if (*(*(*self).upgrade().deref()).cancelled_.borrow()) {
            return <v8_internal_OperationsBarrier_Token>::default();
        }
        (*(*(*self).upgrade().deref()).operations_count_.borrow_mut()).prefix_inc();
        return v8_internal_OperationsBarrier_Token::v8_internal_OperationsBarrier_Token1({
            (*self).clone()
        });
    }
    fn CancelAndWait(&self) {
        ...
        let _dtor_guard = ScopedDestructor::new(&guard, |__p| __p.destructor());
        (&(0));
        (*(*(*self).upgrade().deref()).cancelled_.borrow_mut()) = true;
        'loop_: while ((*(*(*self).upgrade().deref()).operations_count_.borrow()) > 0_usize) {
            ({
                let _mutex: Ptr<v8_base_Mutex> = ((*(*self).upgrade().deref()).mutex_.as_pointer());
                v8_base_ConditionVariableImpl::Wait(
                    &(*(*self).upgrade().deref()).release_condition_.as_pointer(),
                    _mutex,
                )
            });
        }
    ...
    fn Release(&self) {
        ...
        let _dtor_guard = ScopedDestructor::new(&guard, |__p| __p.destructor());
        if ((*(*(*self).upgrade().deref()).operations_count_.borrow_mut()).prefix_dec() == 0_usize)
            && (*(*(*self).upgrade().deref()).cancelled_.borrow())
        {
            ({
...
pub trait v8_internal_OperationsBarrier_TokenImpl {
    fn destructor(&self);
    fn operator_assign_pmutv8_internal_OperationsBarrier_Token(
        &self,
        other: Ptr<v8_internal_OperationsBarrier_Token>,
    ) -> Ptr<v8_internal_OperationsBarrier_Token>;
    fn operator__Bool(&self) -> bool;
}
impl v8_internal_OperationsBarrier_TokenImpl for Ptr<v8_internal_OperationsBarrier_Token> {
    fn destructor(&self) {
        if !(*(*(*self).upgrade().deref()).outer_.borrow()).is_null() {
            ({
                v8_internal_OperationsBarrierImpl::Release(
    ...
    fn operator_assign_pmutv8_internal_OperationsBarrier_Token(
        ...
        let __rhs = (*(*other.upgrade().deref()).outer_.borrow()).clone();
        (*(*(*self).upgrade().deref()).outer_.borrow_mut()) = __rhs;
        (*(*other.upgrade().deref()).outer_.borrow_mut()) =
            Ptr::<v8_internal_OperationsBarrier>::null();
        return (*self).clone();
    }
    fn operator__Bool(&self) -> bool {
        return !(!(!(*(*(*self).upgrade().deref()).outer_.borrow()).is_null()));
    }
}

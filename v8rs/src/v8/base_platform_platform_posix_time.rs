...
impl v8_base_PosixTimezoneCache for v8_base_PosixDefaultTimezoneCache {
    fn LocalTimezone(&self, time: f64) -> Ptr<u8> {
        let time: Value<f64> = Rc::new(RefCell::new(time));
        if ({ isnan_40((*time.borrow())) }) {
            return Ptr::from_string_literal(b"");
        }
        let tv: Value<i64> = Rc::new(RefCell::new(
            (({
                floor_8(((*time.borrow()) / ((*msPerSecond_41.with(Value::clone).borrow()) as f64)))
            }) as i64),
        ));
        let tm: Value<Tm> = Rc::new(RefCell::new(Default::default()));
        let t: Value<Ptr<Tm>> = Rc::new(RefCell::new({
            let __res = (tm.as_pointer());
            match jiff::Timestamp::from_second((tv.as_pointer()).read()) {
                Ok(__ts) => {
                ...
                Err(_) => {
                    cpp2rust_errno().write(::libc::EOVERFLOW);
                    Ptr::null()
                }
            }
        }));
        if (!(!(*t.borrow()).is_null()))
            || (!(!(*(*(*t.borrow()).upgrade().deref()).tm_zone.borrow()).is_null()))
        {
            return Ptr::from_string_literal(b"");
        }
        return (*(*(*t.borrow()).upgrade().deref()).tm_zone.borrow()).clone();
    }
    fn LocalTimeOffset(&self, time_ms: f64, is_utc: bool) -> f64 {
        let time_ms: Value<f64> = Rc::new(RefCell::new(time_ms));
        let is_utc: Value<bool> = Rc::new(RefCell::new(is_utc));
        let tv: Value<i64> = Rc::new(RefCell::new({
        ...
        return (({
            let _lhs = {
                let _lhs = (*(*(*t.borrow()).upgrade().deref()).tm_gmtoff.borrow());
                _lhs * ((*msPerSecond_41.with(Value::clone).borrow()) as i64)
            };
            _lhs - ((if ((*(*(*t.borrow()).upgrade().deref()).tm_isdst.borrow()) > 0) {
                (3600 * (*msPerSecond_41.with(Value::clone).borrow()))
            } else {
                0
            }) as i64)
        }) as f64);

...
impl v8_base_Mutex {
    pub fn v8_base_Mutex() -> Self {
        let __this: Value<v8_base_Mutex> = Rc::new(RefCell::new(Self {
            native_handle_: Rc::new(RefCell::new(absl_Mutex::absl_Mutex1())),
        }));
        let this: Ptr<v8_base_Mutex> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
...
impl v8_base_MutexImpl for Ptr<v8_base_Mutex> {
    ...
    fn Lock(&self) {
        ({ absl_MutexImpl::lock(&(*(*self).upgrade().deref()).native_handle_.as_pointer()) });
        ({ v8_base_MutexImpl::AssertUnheldAndMark(self) });
    }
    fn Unlock(&self) {
        ({ v8_base_MutexImpl::AssertHeldAndUnmark(self) });
        ({ absl_MutexImpl::unlock(&(*(*self).upgrade().deref()).native_handle_.as_pointer()) });
    }
    fn TryLock(&self) -> bool {
        if !({
            absl_MutexImpl::try_lock(&(*(*self).upgrade().deref()).native_handle_.as_pointer())
        }) {
            return false;
        }
        ({ v8_base_MutexImpl::AssertUnheldAndMark(self) });
...
impl v8_base_RecursiveMutexImpl for Ptr<v8_base_RecursiveMutex> {
    ...
    fn destructor(&self) {
        (&(0));
        (*self.upgrade().deref()).mutex_.as_pointer().destructor();
    }
    fn Lock(&self) {
        let own_id: Value<i32> = Rc::new(RefCell::new(({ v8_base_OS::GetCurrentThreadId() })));
        if (({ (*(*(*self).upgrade().deref()).thread_id_.borrow()).operator_int_const() })
            == (*own_id.borrow()))
        {
            (*(*(*self).upgrade().deref()).level_.borrow_mut()).postfix_inc();
            return;
        }
        ({ v8_base_MutexImpl::Lock(&(*(*self).upgrade().deref()).mutex_.as_pointer()) });
        (&(0));
        (*(*(*self).upgrade().deref()).thread_id_.borrow_mut()) = (*own_id.borrow());
        (*(*(*self).upgrade().deref()).level_.borrow_mut()) = 1;
    }
    fn Unlock(&self) {
        if (((*(*(*self).upgrade().deref()).level_.borrow_mut()).prefix_dec()) == 0) {
            (*(*(*self).upgrade().deref()).thread_id_.borrow_mut()) = 0;
            ({ v8_base_MutexImpl::Unlock(&(*(*self).upgrade().deref()).mutex_.as_pointer()) });
        }
    }
    fn TryLock(&self) -> bool {
        let own_id: Value<i32> = Rc::new(RefCell::new(({ v8_base_OS::GetCurrentThreadId() })));
        ...
        if ({ v8_base_MutexImpl::TryLock(&(*(*self).upgrade().deref()).mutex_.as_pointer()) }) {
            (&(0));
            (*(*(*self).upgrade().deref()).thread_id_.borrow_mut()) = (*own_id.borrow());
            (*(*(*self).upgrade().deref()).level_.borrow_mut()) = 1;
            return true;

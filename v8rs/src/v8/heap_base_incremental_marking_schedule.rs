...
thread_local!(
    pub static kEstimatedMarkingTime_114: Value<v8_base_TimeDelta> = Rc::new(RefCell::new(
        ({ v8_base_TimeDelta::FromMilliseconds(500_i64) }),
    ));
);
thread_local!(
    pub static kStepSizeWhenNotMakingProgress_115: Value<usize> = Rc::new(RefCell::new(65536));
);
thread_local!(
    static kEphemeronPairsFlushingRatioIncrements_116: Value<f64> = Rc::new(RefCell::new(2.5E-1));
);
#[derive(Default)]
pub struct heap_base_IncrementalMarkingSchedule_StepInfo {
    pub mutator_marked_bytes: Value<usize>,
    pub concurrent_marked_bytes: Value<usize>,
    pub estimated_live_bytes: Value<usize>,
    pub expected_marked_bytes: Value<usize>,
    pub elapsed_time: Value<v8_base_TimeDelta>,
}
impl Clone for heap_base_IncrementalMarkingSchedule_StepInfo {
    fn clone(&self) -> Self {
        let __this: Value<heap_base_IncrementalMarkingSchedule_StepInfo> =
            Rc::new(RefCell::new(Self {
                mutator_marked_bytes: Rc::new(RefCell::new((*self.mutator_marked_bytes.borrow()))),
                concurrent_marked_bytes: Rc::new(RefCell::new(
                    (*self.concurrent_marked_bytes.borrow()),
                )),
                estimated_live_bytes: Rc::new(RefCell::new((*self.estimated_live_bytes.borrow()))),
                expected_marked_bytes: Rc::new(RefCell::new(
                    (*self.expected_marked_bytes.borrow()),
                )),
                elapsed_time: Rc::new(RefCell::new((*self.elapsed_time.borrow()).clone())),
            }));
        let this: Ptr<heap_base_IncrementalMarkingSchedule_StepInfo> = __this.as_pointer();
        Rc::try_unwrap(__this).ok().unwrap().into_inner()
    }
}
impl ByteRepr for heap_base_IncrementalMarkingSchedule_StepInfo {
    fn byte_size() -> usize {
        40
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.mutator_marked_bytes.borrow()).to_bytes(&mut buf[0..8]);
        (*self.concurrent_marked_bytes.borrow()).to_bytes(&mut buf[8..16]);
        (*self.estimated_live_bytes.borrow()).to_bytes(&mut buf[16..24]);
        (*self.expected_marked_bytes.borrow()).to_bytes(&mut buf[24..32]);
        (*self.elapsed_time.borrow()).to_bytes(&mut buf[32..40]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            mutator_marked_bytes: Rc::new(RefCell::new(<usize>::from_bytes(&buf[0..8]))),
            concurrent_marked_bytes: Rc::new(RefCell::new(<usize>::from_bytes(&buf[8..16]))),
            estimated_live_bytes: Rc::new(RefCell::new(<usize>::from_bytes(&buf[16..24]))),
            expected_marked_bytes: Rc::new(RefCell::new(<usize>::from_bytes(&buf[24..32]))),
            elapsed_time: Rc::new(RefCell::new(<v8_base_TimeDelta>::from_bytes(&buf[32..40]))),
        }
    }
}
#[derive(Default)]
pub struct heap_base_IncrementalMarkingSchedule {
    incremental_marking_start_time_: Value<v8_base_TimeTicks>,
    mutator_thread_marked_bytes_: Value<usize>,
    concurrently_marked_bytes_: Value<std_atomic_unsigned_long_>,
    last_estimated_live_bytes_: Value<usize>,
    ephemeron_pairs_flushing_ratio_target_: Value<f64>,
    current_step_: Value<heap_base_IncrementalMarkingSchedule_StepInfo>,
    min_marked_bytes_per_step_: Value<usize>,
    predictable_schedule_: Value<bool>,
    elapsed_time_override_: Value<std_optional_v8_base_TimeDelta_>,
    last_concurrently_marked_bytes_: Value<usize>,
    last_concurrently_marked_bytes_update_: Value<v8_base_TimeTicks>,
}
impl heap_base_IncrementalMarkingSchedule {
    fn heap_base_IncrementalMarkingSchedule(
        min_marked_bytes_per_step: usize,
        predictable_schedule: bool,
    ) -> Self {
        let min_marked_bytes_per_step: Value<usize> =
            Rc::new(RefCell::new(min_marked_bytes_per_step));
        let predictable_schedule: Value<bool> = Rc::new(RefCell::new(predictable_schedule));
        let __this: Value<heap_base_IncrementalMarkingSchedule> = Rc::new(RefCell::new(Self {
            incremental_marking_start_time_: Rc::new(RefCell::new(
                v8_base_TimeTicks::v8_base_TimeTicks2(),
            )),
            mutator_thread_marked_bytes_: Rc::new(RefCell::new(0_usize)),
            concurrently_marked_bytes_: Rc::new(RefCell::new(
                std_atomic_unsigned_long_::std_atomic_unsigned_long_1({ 0_u64 }),
            )),
            last_estimated_live_bytes_: Rc::new(RefCell::new(0_usize)),
            ephemeron_pairs_flushing_ratio_target_: Rc::new(RefCell::new(2.5E-1)),
            current_step_: Rc::new(RefCell::new(
                <heap_base_IncrementalMarkingSchedule_StepInfo>::default(),
            )),
            min_marked_bytes_per_step_: Rc::new(RefCell::new(
                (*min_marked_bytes_per_step.borrow()),
            )),
            predictable_schedule_: Rc::new(RefCell::new((*predictable_schedule.borrow()))),
            elapsed_time_override_: Rc::new(RefCell::new(
                std_optional_v8_base_TimeDelta_::std_optional_v8_base_TimeDelta_1(),
            )),
            last_concurrently_marked_bytes_: Rc::new(RefCell::new(0_usize)),
            last_concurrently_marked_bytes_update_: Rc::new(RefCell::new(
                v8_base_TimeTicks::v8_base_TimeTicks2(),
            )),
        }));
        let this: Ptr<heap_base_IncrementalMarkingSchedule> = __this.as_pointer();
        if (*(*this.upgrade().deref()).predictable_schedule_.borrow()) {
            ({
                (*(*this.upgrade().deref()).elapsed_time_override_.borrow()).emplace(
                    kTimeDeltaForPredictableSchedule_117
                        .with(Value::clone)
                        .as_pointer(),
                )
            });
...
impl ByteRepr for heap_base_IncrementalMarkingSchedule {
    fn byte_size() -> usize {
        128
    }
    fn to_bytes(&self, buf: &mut [u8]) {
        (*self.incremental_marking_start_time_.borrow()).to_bytes(&mut buf[0..8]);
        (*self.mutator_thread_marked_bytes_.borrow()).to_bytes(&mut buf[8..16]);
        (*self.concurrently_marked_bytes_.borrow()).to_bytes(&mut buf[16..24]);
        (*self.last_estimated_live_bytes_.borrow()).to_bytes(&mut buf[24..32]);
        (*self.ephemeron_pairs_flushing_ratio_target_.borrow()).to_bytes(&mut buf[32..40]);
        (*self.current_step_.borrow()).to_bytes(&mut buf[40..80]);
        (*self.min_marked_bytes_per_step_.borrow()).to_bytes(&mut buf[80..88]);
        (*self.predictable_schedule_.borrow()).to_bytes(&mut buf[88..89]);
        (*self.elapsed_time_override_.borrow()).to_bytes(&mut buf[96..112]);
        (*self.last_concurrently_marked_bytes_.borrow()).to_bytes(&mut buf[112..120]);
        (*self.last_concurrently_marked_bytes_update_.borrow()).to_bytes(&mut buf[120..128]);
    }
    fn from_bytes(buf: &[u8]) -> Self {
        Self {
            incremental_marking_start_time_: Rc::new(RefCell::new(
                <v8_base_TimeTicks>::from_bytes(&buf[0..8]),
            )),
            mutator_thread_marked_bytes_: Rc::new(RefCell::new(<usize>::from_bytes(&buf[8..16]))),
            concurrently_marked_bytes_: Rc::new(RefCell::new(
                <std_atomic_unsigned_long_>::from_bytes(&buf[16..24]),
            )),
            last_estimated_live_bytes_: Rc::new(RefCell::new(<usize>::from_bytes(&buf[24..32]))),
            ephemeron_pairs_flushing_ratio_target_: Rc::new(RefCell::new(<f64>::from_bytes(
                &buf[32..40],
            ))),
            current_step_: Rc::new(RefCell::new(
                <heap_base_IncrementalMarkingSchedule_StepInfo>::from_bytes(&buf[40..80]),
            )),
            min_marked_bytes_per_step_: Rc::new(RefCell::new(<usize>::from_bytes(&buf[80..88]))),
            predictable_schedule_: Rc::new(RefCell::new(<bool>::from_bytes(&buf[88..89]))),
            elapsed_time_override_: Rc::new(RefCell::new(
                <std_optional_v8_base_TimeDelta_>::from_bytes(&buf[96..112]),
            )),
            last_concurrently_marked_bytes_: Rc::new(RefCell::new(<usize>::from_bytes(
                &buf[112..120],
            ))),
            last_concurrently_marked_bytes_update_: Rc::new(RefCell::new(
                <v8_base_TimeTicks>::from_bytes(&buf[120..128]),
            )),
        }
    }
}
thread_local!(
    pub static kTimeDeltaForPredictableSchedule_117: Value<v8_base_TimeDelta> = Rc::new(
        RefCell::new(({ v8_base_TimeDelta::FromMilliseconds(1_i64) })),
    );
);
impl heap_base_IncrementalMarkingSchedule {
    pub fn Create(
        predictable_schedule: Option<bool>,
    ) -> Option<Value<heap_base_IncrementalMarkingSchedule>> {
        let predictable_schedule: Value<bool> =
            Rc::new(RefCell::new(predictable_schedule.unwrap_or(false)));
        return Ptr::alloc(
            heap_base_IncrementalMarkingSchedule::heap_base_IncrementalMarkingSchedule(
                { 0_usize },
                { (*predictable_schedule.borrow()) },
            ),
        )
        .to_owned_opt();
    }
...
impl heap_base_IncrementalMarkingSchedule {
    pub fn CreateWithMarkedBytesPerStepForTesting(
        min_marked_bytes_per_step: usize,
        predictable_schedule: Option<bool>,
    ) -> Option<Value<heap_base_IncrementalMarkingSchedule>> {
        let min_marked_bytes_per_step: Value<usize> =
...
thread_local!();
pub trait heap_base_IncrementalMarkingScheduleImpl {
    fn NotifyIncrementalMarkingStart(&self);
    fn NotifyConcurrentMarkingStart(&self);
    fn AddMutatorThreadMarkedBytes(&self, marked_bytes: usize);
    fn AddConcurrentlyMarkedBytes(&self, marked_bytes: usize);
    fn GetNextIncrementalStepDuration(&self, estimated_live_bytes: usize) -> usize;
    fn GetCurrentStepInfo(&self) -> heap_base_IncrementalMarkingSchedule_StepInfo;
    fn ShouldFlushEphemeronPairs(&self) -> bool;
    fn GetTimeSinceLastConcurrentMarkingUpdate(&self) -> v8_base_TimeDelta;
    fn min_marked_bytes_per_step(&self) -> usize;
    fn SetElapsedTimeForTesting(&self, elapsed_time: v8_base_TimeDelta);
    fn GetElapsedTimeSinceMarkingStart(&self) -> v8_base_TimeDelta;
    fn GetOverallMarkedBytes(&self) -> usize;
    fn GetConcurrentlyMarkedBytes(&self) -> usize;
}
impl heap_base_IncrementalMarkingScheduleImpl for Ptr<heap_base_IncrementalMarkingSchedule> {
    fn min_marked_bytes_per_step(&self) -> usize {
    ...
    fn NotifyIncrementalMarkingStart(&self) {
        if !({
            v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl::IsNull(
                &(*(*self).upgrade().deref())
        ...
        (*(*(*self).upgrade().deref())
            .incremental_marking_start_time_
            .borrow_mut()) = ({ v8_base_TimeTicks::Now() });
    }
    fn NotifyConcurrentMarkingStart(&self) {
        if !({
            v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl::IsNull(
    ...
    fn AddMutatorThreadMarkedBytes(&self, marked_bytes: usize) {
        let marked_bytes: Value<usize> = Rc::new(RefCell::new(marked_bytes));
        {
            let rhs_0 = (*(*(*self).upgrade().deref())
                .mutator_thread_marked_bytes_
                .borrow())
            .wrapping_add((*marked_bytes.borrow()));
            (*(*(*self).upgrade().deref())
                .mutator_thread_marked_bytes_
                .borrow_mut()) = rhs_0
        };
    ...
    fn AddConcurrentlyMarkedBytes(&self, marked_bytes: usize) {
        ...
        ({
            (*(*(*self).upgrade().deref())
                ...
                .borrow())
            .fetch_add_u64(((*marked_bytes.borrow()) as u64), Some(0))
        });
    }
    fn GetOverallMarkedBytes(&self) -> usize {
        return (*(*(*self).upgrade().deref())
            ...
            .borrow())
        .wrapping_add(
            ({ heap_base_IncrementalMarkingScheduleImpl::GetConcurrentlyMarkedBytes(self) }),
        );
    }
    fn GetConcurrentlyMarkedBytes(&self) -> usize {
        return (({
    ...
    fn GetElapsedTimeSinceMarkingStart(&self) -> v8_base_TimeDelta {
        if ({ (*(*(*self).upgrade().deref()).elapsed_time_override_.borrow()).has_value() }) {
            let elapsed_time: Value<v8_base_TimeDelta> = Rc::new(RefCell::new(
                (*(*(*(*self).upgrade().deref()).elapsed_time_override_.borrow())
                    .upgrade()
                    .deref())
                .clone(),
            ));
            if (*(*(*self).upgrade().deref()).predictable_schedule_.borrow()) {
                (*(*(*self).upgrade().deref())
                    .elapsed_time_override_
                    .borrow_mut()) = (*kTimeDeltaForPredictableSchedule_117
                    .with(Value::clone)
                    .borrow())
                .clone();
            } else {
                ({ (*(*(*self).upgrade().deref()).elapsed_time_override_.borrow()).reset() });
            }
            return (*elapsed_time.borrow()).clone();
        }
        return ({
            v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl :: operator_sub_v8_base_TimeTicks_const ( &Rc::new(RefCell::new((  { v8_base_TimeTicks::Now ( ) } ) )) .as_pointer()  , ((*(*(* self ) .upgrade().deref()) . incremental_marking_start_time_ .borrow()) ).clone()  , )
        });
    }
    fn GetCurrentStepInfo(&self) -> heap_base_IncrementalMarkingSchedule_StepInfo {
        return (*(*(*self).upgrade().deref()).current_step_.borrow()).clone();
    }
    fn GetNextIncrementalStepDuration(&self, estimated_live_bytes: usize) -> usize {
        let estimated_live_bytes: Value<usize> = Rc::new(RefCell::new(estimated_live_bytes));
        (*(*(*self).upgrade().deref())
            .last_estimated_live_bytes_
            .borrow_mut()) = (*estimated_live_bytes.borrow());
        (&(0));
        let elapsed_time: Value<v8_base_TimeDelta> = Rc::new(RefCell::new(
            ({ heap_base_IncrementalMarkingScheduleImpl::GetElapsedTimeSinceMarkingStart(self) }),
        ));
        let last_marked_bytes: Value<usize> = Rc::new(RefCell::new(
            ({
                heap_base_IncrementalMarkingSchedule_StepInfoImpl::marked_bytes(
                    &(*(*self).upgrade().deref()).current_step_.as_pointer(),
                )
            }),
        ));
        let actual_marked_bytes: Value<usize> = Rc::new(RefCell::new(
            ({ heap_base_IncrementalMarkingScheduleImpl::GetOverallMarkedBytes(self) }),
        ));
        let expected_marked_bytes: Value<usize> = Rc::new(RefCell::new(
            (({
                ceil_118(
                    ((((*estimated_live_bytes.borrow()) as f64)
                        * ({
                            v8_base_TimeDeltaImpl::InMillisecondsF(&elapsed_time.as_pointer())
                        }))
                        / ({
                            v8_base_TimeDeltaImpl::InMillisecondsF(
                                &kEstimatedMarkingTime_114.with(Value::clone).as_pointer(),
                            )
                        })),
                )
            }) as usize),
        ));
        (*(*(*self).upgrade().deref()).current_step_.borrow_mut()) =
            heap_base_IncrementalMarkingSchedule_StepInfo {
                mutator_marked_bytes: Rc::new(RefCell::new(
                    (*(*(*self).upgrade().deref())
                        .mutator_thread_marked_bytes_
                        .borrow()),
                )),
                ...
                estimated_live_bytes: Rc::new(RefCell::new((*estimated_live_bytes.borrow()))),
                expected_marked_bytes: Rc::new(RefCell::new((*expected_marked_bytes.borrow()))),
                elapsed_time: Rc::new(RefCell::new((*elapsed_time.borrow()).clone())),
            };
        if ((*actual_marked_bytes.borrow()) >= (*last_marked_bytes.borrow()))
            && (((*actual_marked_bytes.borrow()).wrapping_sub((*last_marked_bytes.borrow())))
                < 65536)
        {
            return ({
                let __tmp_0: Value<u64> = Rc::new(RefCell::new((65536 as u64)));
        ...
        if ((*expected_marked_bytes.borrow()) < (*actual_marked_bytes.borrow())) {
            return (*(*(*self).upgrade().deref())
                .min_marked_bytes_per_step_
                .borrow());
        }
        return ({
            ...
            let __tmp_1: Value<u64> = Rc::new(RefCell::new(
                ((*expected_marked_bytes.borrow()).wrapping_sub((*actual_marked_bytes.borrow()))
                    as u64),
            ));
            (if __tmp_0.as_pointer().read() >= __tmp_1.as_pointer().read() {
                __tmp_0.as_pointer()
    ...
    fn ShouldFlushEphemeronPairs(&self) -> bool {
        if ((({ heap_base_IncrementalMarkingScheduleImpl::GetOverallMarkedBytes(self) }) as f64)
            < ((*(*(*self).upgrade().deref())
        ...
        (*(*(*self).upgrade().deref())
            .ephemeron_pairs_flushing_ratio_target_
            .borrow_mut()) += (*kEphemeronPairsFlushingRatioIncrements_116
            .with(Value::clone)
            .borrow());
        return true;
    }
    fn GetTimeSinceLastConcurrentMarkingUpdate(&self) -> v8_base_TimeDelta {
        let current_concurrently_marked_bytes: Value<usize> = Rc::new(RefCell::new(
            ({ heap_base_IncrementalMarkingScheduleImpl::GetConcurrentlyMarkedBytes(self) }),
        ));
        if ((*current_concurrently_marked_bytes.borrow())
            > (*(*(*self).upgrade().deref())
                ...
                .borrow()))
        {
            (*(*(*self).upgrade().deref())
                .last_concurrently_marked_bytes_
                .borrow_mut()) = (*current_concurrently_marked_bytes.borrow());
            (*(*(*self).upgrade().deref())
                .last_concurrently_marked_bytes_update_
                .borrow_mut()) = ({ v8_base_TimeTicks::Now() });
            return v8_base_TimeDelta::v8_base_TimeDelta1();
        }
        if ({
            v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl::IsNull(
                &(*(*self).upgrade().deref())
        ...
        return ({
            v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl :: operator_sub_v8_base_TimeTicks_const ( &Rc::new(RefCell::new((  { v8_base_TimeTicks::Now ( ) } ) )) .as_pointer()  , ((*(*(* self ) .upgrade().deref()) . last_concurrently_marked_bytes_update_ .borrow()) ).clone()  , )
        });
    }
    fn SetElapsedTimeForTesting(&self, elapsed_time: v8_base_TimeDelta) {
        let elapsed_time: Value<v8_base_TimeDelta> = Rc::new(RefCell::new(elapsed_time));
        ({
            (*(*(*self).upgrade().deref()).elapsed_time_override_.borrow())
                .emplace(elapsed_time.as_pointer())
        });
    }
}
pub trait heap_base_IncrementalMarkingSchedule_StepInfoImpl {
    fn marked_bytes(&self) -> usize;
    fn scheduled_delta_bytes(&self) -> i64;
    fn is_behind_expectation(&self) -> bool;
}
impl heap_base_IncrementalMarkingSchedule_StepInfoImpl
    for Ptr<heap_base_IncrementalMarkingSchedule_StepInfo>
{
    fn marked_bytes(&self) -> usize {
        return (*(*(*self).upgrade().deref()).mutator_marked_bytes.borrow()).wrapping_add(
            (*(*(*self).upgrade().deref())
    ...
    fn scheduled_delta_bytes(&self) -> i64 {
        return ((((({ heap_base_IncrementalMarkingSchedule_StepInfoImpl::marked_bytes(self) })
            as i64) as u64)
            .wrapping_sub(((*(*(*self).upgrade().deref()).expected_marked_bytes.borrow()) as u64)))
            as i64);
    }
    fn is_behind_expectation(&self) -> bool {
        return (({
            heap_base_IncrementalMarkingSchedule_StepInfoImpl::scheduled_delta_bytes(self)
        }) < 0_i64);
    }
}
...
pub trait v8_base_time_internal_TimeBase_v8_base_TimeTicks_Impl {
    fn IsNull(&self) -> bool;
    fn operator_sub_v8_base_TimeTicks_const(&self, other: v8_base_TimeTicks) -> v8_base_TimeDelta;
    ...
    for Ptr<v8_base_time_internal_TimeBase_v8_base_TimeTicks_>
{
    ...
    fn operator_sub_v8_base_TimeTicks_const(&self, other: v8_base_TimeTicks) -> v8_base_TimeDelta {
        let other: Value<v8_base_TimeTicks> = Rc::new(RefCell::new(other));
        return ({
            v8_base_TimeDelta::FromMicroseconds(
                ((*(*(*self).upgrade().deref()).us_.borrow()) - (*(*other.borrow()).us_.borrow())),
            )
        });
    }
    fn operator_add(&self, delta: v8_base_TimeDelta) -> v8_base_TimeTicks {
